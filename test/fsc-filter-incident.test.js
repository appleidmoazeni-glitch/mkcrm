...
test('legacy category rows resolve only through the canonical GUID catalog mapping',async()=>{const db=store();delete db.collection('fifoAllocations').rows[2].officialProductCategoryGuid;const result=await fsc.fifoLines(db,{fifoDatasetId:'F-C',categoryGuid:'NB-G'},accounting);assert.equal(result.total,1);assert.equal(result.list[0].officialProductCategoryGuid,'NB-G');assert.equal(result.list[0].categoryIdentityResolution,'canonical-name-legacy-fallback');assert.equal(result.legacyCategoryFallbackRows,1);assert.equal(result.categoryFilterAudit.authority,'official-product-category-guid');});
