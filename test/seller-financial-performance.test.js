...
test('human audit DTO presents canonical purchase evidence and zero reconciliation without browser calculation',async()=>{
  const db=dbSeed();await service.buildReadModel(db,{},accounting);const drill=await service.lineDrilldown(db,'LINE-1',manager);
  assert.equal(drill.humanAudit.readOnly,true);assert.equal(drill.humanAudit.transaction.type,'SALE');assert.equal(drill.humanAudit.transaction.canonicalSellerId,'11701013');
  assert.equal(drill.humanAudit.financial.provenanceStatus,'PROVEN');assert.equal(drill.humanAudit.financial.costSourceType,'OFFICIAL_PURCHASE_LAYER');
  assert.equal(drill.humanAudit.costSources.length,1);assert.equal(drill.humanAudit.costSources[0].purchaseInvoiceNumber,7001);assert.equal(drill.humanAudit.costSources[0].allocatedCostExact,'2100000000.00');
  assert.deepEqual({sale:drill.humanAudit.reconciliation.saleDifferenceExact,cost:drill.humanAudit.reconciliation.costDifferenceExact,profit:drill.humanAudit.reconciliation.profitDifferenceExact},{sale:'0.00',cost:'0.00',profit:'0.00'});
  assert.equal(drill.humanAudit.reconciliation.saleWithinTolerance,true);assert.equal(drill.humanAudit.reconciliation.costWithinTolerance,true);assert.equal(drill.humanAudit.reconciliation.profitWithinTolerance,true);
});

test('human audit DTO distinguishes opening, partial, unknown, multi-allocation, and return evidence',()=>{
  const base={saleLineIdentity:'SL-6-158-001-10A1507821',saleInvoiceType:6,saleInvoiceNumber:158,itemCode:'10A1507821',quantityExact:'-1.000000',grossSaleAmountExact:'-5000000.00',fifoCostExact:'-2800000.00',actualFifoProfitExact:'-2200000.00'};
  const returnAudit=service._humanAuditView(base,[{sourceType:'sale_return_reversal',returnInvoiceNo:158,originSaleInvoiceNo:3410,originalSaleLineId:'SL-2-3410-002-10A1507821',originalAllocationId:'ALLOC-ORIGINAL',quantityExact:'-1.000000',allocatedCostAmountExact:'-2800000.00'}],{profitProvenanceStatus:'PROVEN',costSourceType:'OFFICIAL_PURCHASE_LAYER',provenanceSources:[{allocationId:'ALLOC-RETURN',sourceType:'OFFICIAL_PURCHASE_LAYER',allocatedQty:'-1.000000',allocatedCostExact:'-2800000.00',returnProvenance:{returnInvoiceNumber:158,originSaleInvoiceNumber:3410,originSaleLineId:'SL-2-3410-002-10A1507821',reversedAllocationId:'ALLOC-ORIGINAL',linkageSource:'official-reference',linkageQuality:'exact'}}],provenanceReconciliation:{requiredQtyExact:'-1.000000',provenQtyExact:'-1.000000',unknownQtyExact:'0.000000',allocatedCostExact:'-2800000.00',saleValueExact:'-5000000.00',fifoProfitExact:'-2200000.00',quantityConserved:true,moneyConserved:true}});
  assert.equal(returnAudit.returnReversal.isReturn,true);assert.equal(returnAudit.returnReversal.originalInvoiceNumber,3410);assert.equal(returnAudit.returnReversal.reversalInvoiceNumber,158);assert.equal(returnAudit.returnReversal.reversedCostExact,'-2800000.00');
  const opening=service._humanAuditView({...base,saleInvoiceType:2},[],{profitProvenanceStatus:'PROVEN',costSourceType:'APPROVED_OPENING_ACCOUNTING_COST',provenanceSources:[{sourceType:'APPROVED_OPENING_ACCOUNTING_COST',openingDatasetId:'OPENING-1',openingEvidenceId:'OE-1',allocatedQty:'1.000000',allocatedCostExact:'2800000.00'}],provenanceReconciliation:{requiredQtyExact:'1.000000',provenQtyExact:'1.000000',unknownQtyExact:'0.000000',allocatedCostExact:'2800000.00',saleValueExact:'5000000.00',fifoProfitExact:'2200000.00',quantityConserved:true,moneyConserved:true}});
  assert.equal(opening.financial.costSourceType,'APPROVED_OPENING_ACCOUNTING_COST');assert.equal(opening.costSources[0].openingDatasetId,'OPENING-1');
  const partial=service._humanAuditView({...base,saleInvoiceType:2,fifoCostExact:null,actualFifoProfitExact:null},[{unknownReason:'negative_inventory_chronology'}],{profitProvenanceStatus:'PARTIAL',costSourceType:'UNKNOWN',provenanceSources:[{sourceType:'OFFICIAL_PURCHASE_LAYER',allocatedQty:'1.000000',allocatedCostExact:'100.00'},{sourceType:'UNKNOWN',allocatedQty:'1.000000',allocatedCostExact:null,unknownReason:'negative_inventory_chronology'}],provenanceReconciliation:{requiredQtyExact:'2.000000',provenQtyExact:'1.000000',unknownQtyExact:'1.000000',allocatedCostExact:null,saleValueExact:'500.00',fifoProfitExact:null,quantityConserved:true,moneyConserved:false}});
  assert.equal(partial.allocationCoverage.provenQuantityExact,'1.000000');assert.equal(partial.allocationCoverage.unresolvedQuantityExact,'1.000000');assert.equal(partial.allocationCoverage.knownAllocationCostExact,'100.00');assert.deepEqual(partial.financial.canonicalReasons,['negative_inventory_chronology']);
  const unknown=service._humanAuditView({...base,saleInvoiceType:2,fifoCostExact:null,actualFifoProfitExact:null},[{unknownReason:'PRE_OPENING_PERIOD'}],{profitProvenanceStatus:'UNKNOWN',costSourceType:'UNKNOWN',provenanceSources:[{sourceType:'UNKNOWN',allocatedQty:'1.000000',allocatedCostExact:null,unknownReason:'PRE_OPENING_PERIOD'}],provenanceReconciliation:{requiredQtyExact:'1.000000',provenQtyExact:'0.000000',unknownQtyExact:'1.000000',allocatedCostExact:null,saleValueExact:'500.00',fifoProfitExact:null,quantityConserved:true,moneyConserved:false}});
  assert.equal(unknown.financial.fifoCostExact,null);assert.equal(unknown.financial.fifoProfitExact,null);assert.equal(unknown.financial.unavailableMessage,'هزینه قابل اثبات نیست');assert.ok(unknown.financial.canonicalReasons.includes('PRE_OPENING_PERIOD'));
  const multi=service._humanAuditView({...base,saleInvoiceType:2},[],{profitProvenanceStatus:'PROVEN',costSourceType:'MULTI_PURCHASE_LAYER',provenanceSources:[{sourceType:'OFFICIAL_PURCHASE_LAYER',allocationId:'A1',allocatedCostExact:'40.00'},{sourceType:'OFFICIAL_PURCHASE_LAYER',allocationId:'A2',allocatedCostExact:'60.00'}],provenanceReconciliation:{requiredQtyExact:'2.000000',provenQtyExact:'2.000000',unknownQtyExact:'0.000000',allocatedCostExact:'100.00',saleValueExact:'500.00',fifoProfitExact:'400.00',quantityConserved:true,moneyConserved:true}});
  assert.equal(multi.costSources.length,2);assert.equal(multi.allocationCoverage.knownAllocationCostExact,'100.00');
});

test('canonical Seller Financial UI uses explicit run-scoped read-only human audit and retains collapsed raw evidence',()=>{
  const source=fs.readFileSync(path.join(__dirname,'..','public','assets','app.js'),'utf8');const start=source.lastIndexOf('async function drill(line)');const end=source.indexOf('async function loadRuns()',start);const drillSource=source.slice(start,end);
  assert.match(drillSource,/r\.humanAudit/);assert.match(drillSource,/runScopedUrl/);assert.match(drillSource,/نمایش اطلاعات فنی خام/);assert.match(drillSource,/<details>/);assert.match(drillSource,/هزینه قابل اثبات نیست/);assert.match(drillSource,/برگشت \/ معکوس‌سازی فروش/);assert.doesNotMatch(drillSource,/method\s*:\s*['"]POST['"]/);assert.doesNotMatch(drillSource,/\/\s*10\b/);
});

test('human audit drill-down performs zero financial business writes',async()=>{
  const db=dbSeed();await service.buildReadModel(db,{},accounting);const names=['sellerFinancialPerformanceRuns','sellerFinancialPerformanceLines','sellerFinancialPerformanceSummaries','fifoProfitFacts','fifoAllocations','manualCostResolutions','profitAdjustments','savedProfitLedgerEntries'];const before=Object.fromEntries(names.map(name=>[name,structuredClone(db.collection(name).rows)]));
  await service.lineDrilldown(db,'LINE-1',manager);
  for(const name of names)assert.deepEqual(db.collection(name).rows,before[name],name);
});
