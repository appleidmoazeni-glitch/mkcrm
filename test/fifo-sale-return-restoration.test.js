...
test('FIFO Candidate UI explains sale, return, cost source and net effect without raw JSON',()=>{
  ...
  assert.match(view,/generation!==allocationGeneration/);
  assert.match(view,/!box\.isConnected/);
  ...
  assert.match(finalRegistry,/\{id:'fifo-shadow-validation',label:'به‌روزرسانی FIFO'\}/);
