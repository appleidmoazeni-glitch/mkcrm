...
(()=>{
  ...
  function message(text,error=false){const box=q('#fgMessage');if(box)box.innerHTML=`<div class="${error?'error':'success'}">${safe(text)}</div>`;}
  function actions(row,type){if(row.historicalFrozen||row.status==='historical_frozen')return '<span class="muted">تاریخی و منجمد</span>';const base=type==='policy'?'/api/accounting/commission-policies':type==='mapping'?'/api/accounting/profit-ledger/categories':'/api/accounting/profit-ledger/rates';const buttons=[];if(EDIT.includes(userRole())&&row.status==='draft')buttons.push(`<button class="mini fg-transition" data-base="${base}" data-id="${safe(row.policyVersionId||row.mappingId||row.rateVersionId)}" data-action="submit" data-revision="${safe(row.revision)}">ارسال برای تأیید</button>`);if(APPROVE.includes(userRole())&&row.status==='pending')buttons.push(`<button class="mini fg-transition" data-base="${base}" data-id="${safe(row.policyVersionId||row.mappingId||row.rateVersionId)}" data-action="approve" data-revision="${safe(row.revision)}">تأیید مستقل</button>`);if(APPROVE.includes(userRole())&&row.status==='approved'&&type==='policy')buttons.push(`<button class="mini fg-transition" data-base="${base}" data-id="${safe(row.policyVersionId)}" data-action="retire" data-revision="${safe(row.revision)}">بازنشسته‌کردن</button>`);return buttons.join(' ')||'—';}
  function bindTransitions(refresh){document.querySelectorAll('.fg-transition').forEach(button=>button.onclick=async()=>{try{await api(`${button.dataset.base}/${encodeURIComponent(button.dataset.id)}/${button.dataset.action}`,{method:'POST',body:JSON.stringify({revision:Number(button.dataset.revision),reason:q('#fgActionReason')?.value||''})});await refresh();}catch(error){message(`${error.code} ${error.message}`,true);}});}
  ...
  function statusFa(status){return({draft:'پیش‌نویس',pending:'در انتظار تأیید',approved:'مصوب',rejected:'ردشده',cancelled:'لغوشده',retired:'بازنشسته',historical_frozen:'تاریخی و منجمد'})[status]||status||'نامشخص';}
  ...
  async function mappingPage(){
    ...
    try{
      ...
      if(q('#fgmGroup'))q('#fgmGroup').innerHTML=(groups.list||[]).filter(row=>row.officialEvidence).map(row=>`<option value="${safe(row.officialProductCategoryIdentity)}" data-guid="${safe(row.officialProductCategoryGuid)}" data-number="${safe(row.officialProductCategoryNumber)}" data-name="${safe(row.officialProductCategoryName)}">${safe(row.officialProductCategoryNumber)} — ${safe(row.officialProductCategoryName)}</option>`).join('');
      const list=q('#fgmList');if(!list)return;list.innerHTML=`<table class="table"><thead><tr><th>دسته رسمی کالا</th><th>سبد نرخ</th><th>سیاست</th><th>اثر</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${(mappings.list||[]).map(row=>`<tr><td>${row.officialProductCategoryNumber?`${safe(row.officialProductCategoryNumber)} — `:''}<b>${safe(row.officialProductCategoryName||(row.policyVersionId==='LEGACY_PRE_POLICY'?'رکورد تاریخی':'دسته ثبت‌شده'))}</b><details><summary>جزئیات حسابرسی</summary>${safe(row.officialProductCategoryIdentity||row.identityValue||'—')}</details></td><td>${safe(row.commissionRatePool)}</td><td><details><summary>سیاست ثبت‌شده</summary>${safe(row.policyVersionId||'—')}</details></td><td>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td><td>${safe(statusFa(row.status))} / بازبینی ${safe(row.revision)}</td><td>${actions(row,'mapping')}</td></tr>`).join('')||'<tr><td colspan="6">رکوردی وجود ندارد.</td></tr>'}</tbody></table>`;
      bindTransitions(mappingPage);
      if(q('#fgmCreate'))q('#fgmCreate').onclick=async()=>{const policyVersionId=q('#fgmPolicy').value,option=q('#fgmGroup').selectedOptions[0];if(!policyVersionId){message('ابتدا یک سیاست پورسانت مصوب را انتخاب کنید.',true);return;}if(!option){message('گروه اصلی رسمی قابل انتخاب وجود ندارد.',true);return;}try{await api('/api/accounting/profit-ledger/categories',{method:'POST',body:JSON.stringify({policyVersionId,groupPathIdentity:option.value,officialProductCategoryIdentity:option.value,officialProductCategoryGuid:option.dataset.guid,officialProductCategoryNumber:option.dataset.number,officialProductCategoryName:option.dataset.name,commissionRatePool:q('#fgmPool').value,effectiveFrom:q('#fgmFrom').value,effectiveTo:q('#fgmTo').value,description:q('#fgmDescription').value})});await mappingPage();}catch(error){message(`${error.code} ${error.message}`,true);}};
  ...
  async function ratePage(){
    ...
    try{
      ...
      const list=q('#fgrList');if(!list)return;list.innerHTML=`<table class="table"><thead><tr><th>فروشنده / دامنه</th><th>دسته / سبد نرخ</th><th>نرخ</th><th>سیاست</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${(rates.list||[]).map(row=>`<tr><td>${safe(row.sellerIdentity)} / ${safe(scopeFa(row.rateScope))}</td><td>${safe(row.officialProductCategoryName||row.commissionRatePool)}</td><td>${safe(row.rate)}</td><td><details><summary>سیاست ثبت‌شده</summary>${safe(row.policyVersionId||'—')}</details></td><td>${safe(statusFa(row.status))} / بازبینی ${safe(row.revision)}</td><td>${actions(row,'rate')}</td></tr>`).join('')||'<tr><td colspan="6">رکوردی وجود ندارد.</td></tr>'}</tbody></table>`;
