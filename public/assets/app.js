...
(()=>{
  ...
  async function loadOptions(){const r=await api('/api/accounting/seller-financial-performance/filters');preserveSelections('#sfSeller',optionRows(r.sellers));preserveSelections('#sfCategory',optionRows(r.categories,'guid','label'));preserveSelections('#sfPool',optionRows(r.ratePools));preserveSelections('#sfStore',optionRows(r.stores));}
  async function loadTotals(){const r=await api('/api/accounting/seller-financial-performance/totals?'+params());q('#sfCards').innerHTML=[card('فاکتور',number(r.invoiceCount)),card('ردیف',number(r.lineCount),`${number(r.unavailableLineCount)} پورسانت ناموجود`),card('فروش ناخالص',toman(r.saleValueExact),'تومان'),card('سود FIFO اثبات‌شده',toman(r.provenFifoProfitExact),`${number(r.provenProfitCoveragePercent)}٪ پوشش ردیف؛ سود کل نیست`),card('سود پورسانتی',toman(r.commissionableProfitExact),'تومان'),card('پورسانت مقدماتی',toman(r.preliminaryCommissionExact),'تومان؛ غیرقابل پرداخت'),card('هزینه نامشخص',number(r.unknownCostLineCount),'ردیف')].join('');}
  ...
  async function pageRenderer(){
    ...
    try{statusCache=await api('/api/accounting/seller-financial-performance/status');}catch(error){if(error.code==='SELLER_FINANCIAL_READ_MODEL_DISABLED')return legacyPage();setPage('عملکرد مالی فروشندگان',`<main class="main-content"><div class="error">${safe(error.code)} ${safe(error.message)}</div></main>`);return;}
    ...
    try{await Promise.all([loadBuildContext(),loadOptions()]);await refresh();}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}
  }
  ...
  const inheritedRoute=window.route||route;
  window.route=route=async function(){const current=location.hash.slice(1)||firstAllowedPage();if(current===PAGE)return pageRenderer();return inheritedRoute.apply(this,arguments);};
  try{renderMenu();}catch{}
})();
