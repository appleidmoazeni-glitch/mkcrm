...
(()=>{
  ...
  async function rebuild(){if(!confirm('Candidate جدید فقط از منابع immutable و approved ساخته شود؟'))return;try{const r=await api('/api/accounting/seller-financial-performance/rebuild',{method:'POST',body:JSON.stringify({batchSize:500,maxAttempts:3})});q('#sfStatus').innerHTML=`<div class="info">Job ${safe(r.jobId)} شروع شد. صفحه را بعداً Refresh کنید.</div>`;}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  async function pageRenderer(){
    ...
    q('#sfApply').onclick=()=>{page=1;refresh();};q('#sfView').onchange=()=>{page=1;refresh();};q('#sfPrev').onclick=()=>{page=Math.max(1,page-1);refresh();};q('#sfNext').onclick=()=>{page++;refresh();};q('#sfRebuild')&&(q('#sfRebuild').onclick=rebuild);
    ...
    try{await loadOptions();await refresh();}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}
  }
