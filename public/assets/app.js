...
async function searchStockRows(q,limit=1000,stockNumber=''){return await api(`/api/legacy/stock/search?q=${encodeURIComponent(q)}&limit=${limit}${stockNumber?`&stockNumber=${encodeURIComponent(stockNumber)}`:''}`)}
async function searchAllItemsRows(q,limit=200,forceLive=false){return await api(`/api/items/search-all?q=${encodeURIComponent(q)}&limit=${limit}&pages=${forceLive?300:25}${forceLive?'&forceLive=1':''}`)}
...
(function(){
  ...
  function bindSupplierPicker(inputId, listId, onPick){
    ...
    const search=debounce(async()=>{const q=input.value.trim(); if(q.length<2){box.style.display='none'; return;} const my=++seq; box.style.display='block'; box.innerHTML='<div class="floating-empty">در حال جستجو در شایگان...</div>'; const r=await supplierSearch(q,80); if(my!==seq)return; last=r.list||[]; render(last);},260);
    input.addEventListener('input', search);
...
(function(){
  ...
  window.openInventoryCardexModal = async function(item){
    ...
    document.getElementById('inlineCardexClose').onclick=()=>modal.style.display='none';
...
(function(){
  ...
  function installSaleCustomerHardSearch(){
    ...
    async function runCustomerSearch(term){
      ...
      if(q.length<2){resultBox.style.display='none';resultBox.innerHTML='';return;}
      resultBox.style.display='block'; resultBox.innerHTML='<div class="floating-empty">در حال جستجوی مشتری...</div>';
      try{
        const r=await api(`/api/customers/search?q=${encodeURIComponent(q)}&limit=30`);
        last=r.list||[];
        resultBox.innerHTML=customerResultHtml(last);
        resultBox.querySelectorAll('.customer-pick').forEach(el=>el.onclick=()=>{const c=last[Number(el.dataset.i)]; fillSaleCustomer(c); const si=qs('#saleCustomerSearch'); if(si) si.value=customerLabel(c); resultBox.style.display='none';});
      }catch(e){resultBox.innerHTML=`<div class="floating-empty error">خطا در جستجوی مشتری: ${safe(e.message||e)}</div>`;}
    }
    function bindLive(id){
      const el=qs('#'+id); if(!el || el.dataset.customerHardBound==='1') return;
      el.dataset.customerHardBound='1';
      el.addEventListener('input',()=>{clearTimeout(timer); const q=el.value.trim(); timer=setTimeout(()=>runCustomerSearch(q),260);});
      el.addEventListener('blur',()=>{const q=el.value.trim(); if(q.length>=3) setTimeout(()=>runCustomerSearch(q),80);});
    }
...
(function(){
  ...
  async function openInvoiceView(invNo, invType=2){
    ...
    modal.style.display='block'; qs('#closeInvModal').onclick=()=>modal.style.display='none'; modal.querySelector('.mk-modal-backdrop').onclick=()=>modal.style.display='none';
...
(function(){
  ...
  window.pageSaleSnapshot=async function(){
    ...
    $q('#saleSnapStart').onclick=async()=>{const m=$q('#saleSnapMsg');m.innerHTML='<div class="info">در حال ساخت Sale Snapshot. این کار ممکن است چند دقیقه طول بکشد...</div>';try{const r=await postJson('/api/sale-snapshot/start',{dateFrom:$q('#saleSnapFrom').value,dateTo:$q('#saleSnapTo').value,maxPages:Number($q('#saleSnapPages').value||300),maxDetailInvoices:Number($q('#saleSnapDetails').value||0),reset:!!$q('#saleSnapReset')?.checked});m.innerHTML=`<div class="success">Sale Snapshot ساخته شد: ${safe(r.snapshotId)} | حالت: ${safe(r.mode||'')} | فاکتور: ${money(r.invoiceHeadersFound)} | Body: ${money(r.invoiceBodiesLoaded)} | ردیف فروش: ${money(r.saleLinesParsed)}</div>`;await renderSaleSnapshots();await renderSaleSnapshotStatus(r.snapshotId);}catch(e){m.innerHTML=`<div class="error">${safe(e.message||e)}</div>`;}};
