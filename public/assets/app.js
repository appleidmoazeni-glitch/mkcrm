...
(function(){
  ...
  function renderSellerPerfReport(base, out){
    ...
    const statusBadge=status=>status==='calculated'?'<span class="success">قطعی</span>':status==='partial'?'<span class="warn">ناقص / بخش پوشش‌داده‌شده</span>':'<span class="muted">نامشخص</span>';
    const coverage=sm.coverage||r.coverage||{};
    const coverageWarning=`<div class="${sm.profitStatus==='unknown'?'error':'warn'}"><b>وضعیت سود: ${statusBadge(sm.profitStatus||'unknown')}</b><br>پوشش تعداد: ${fmt(coverage.coveredQtyPercent||0)}٪ | پوشش مبلغ فروش: ${fmt(coverage.coveredSalesPercent||0)}٪ | ردیف محاسبه‌شده: ${fmt(coverage.calculatedLines||0)} | ناقص: ${fmt(coverage.partialLines||0)} | نامشخص: ${fmt(coverage.unknownLines||0)}<br>منبع لایه‌های خرید ممکن است فقط تأمین‌کنندگان منتخب Supplier Sleep را پوشش دهد؛ سود و پورسانت تا تکمیل تاریخچه خرید قطعی نیست.</div>`;
    ...
    let html=generated;
    html+=`<div class="info">منبع: ${hesc(r.source||'')} | گزارش پایه از aggregateهای Sale Snapshot در Mongo خوانده می‌شود. | گروه کالا: ${r.meta?.itemGroup==='notebook'?'فقط نوت‌بوک؛ کد کالا با ۱':'همه کالاها'}</div>${coverageWarning}`;
    html+=`<div class="kpis"><div class="kpi"><b>${fmt(sm.invoiceCount)}</b><span>تعداد فاکتور</span></div><div class="kpi"><b>${fmt(sm.saleRial)}</b><span>جمع فروش</span></div><div class="kpi"><b>${valueOrUnknown(sm.fifoProfit,sm.profitStatus)}</b><span>سود FIFO پوشش‌داده‌شده</span></div><div class="kpi"><b>${sm.avgProfitPct==null?'نامشخص':fmt(sm.avgProfitPct)+'%'}</b><span>ROI بخش پوشش‌داده‌شده</span></div><div class="kpi"><b>${sm.suggestedCommission==null?'متوقف':fmt(sm.suggestedCommission)}</b><span>پورسانت نهایی</span></div><div class="kpi warn"><b>${fmt(sm.unmatchedSaleRial)}</b><span>فروش با قیمت تمام‌شده نامشخص</span></div></div>`;
    html+=`<h5>جدول خلاصه فروشندگان</h5><table class="table"><thead><tr><th>نام</th><th>جاری کارکنان</th><th>فروشگاه</th><th>تعداد فاکتور</th><th>جمع فروش</th><th>سود</th><th>ROI</th><th>پوشش فروش</th><th>وضعیت</th></tr></thead><tbody>${(r.sellers||[]).map(x=>`<tr><td>${hesc(x.sellerName||x.sellerKey)}</td><td>${hesc(x.employeeAccountNumber||'ندارد')}</td><td>${hesc(x.sellerStore||'')}</td><td>${fmt(x.invoiceCount)}</td><td>${fmt(x.saleRial)}</td><td>${valueOrUnknown(x.fifoProfit,x.profitStatus)}</td><td>${x.avgProfitPct==null?'نامشخص':fmt(x.avgProfitPct)+'%'}</td><td>${fmt(x.coverage?.coveredSalesPercent||0)}٪</td><td>${statusBadge(x.profitStatus||'unknown')}</td></tr>`).join('')}</tbody></table>`;
    html+=`<h5>ریز فاکتورها</h5><table class="table"><thead><tr><th>شماره فاکتور</th><th>تاریخ</th><th>فروشنده</th><th>مشتری</th><th>فروش</th><th>سود</th><th>ROI</th><th>پوشش</th><th>وضعیت</th></tr></thead><tbody>${(r.invoices||[]).slice(0,1000).map(x=>`<tr><td>${invLink(x.invoiceNo)}</td><td>${hesc(window.normDate?normDate(x.invoiceDate):x.invoiceDate||'')}</td><td>${hesc(x.sellerName||x.sellerKey)}</td><td>${hesc(x.customerName||'')}</td><td>${fmt(x.saleRial)}</td><td>${valueOrUnknown(x.fifoProfit,x.profitStatus)}</td><td>${x.profitPct==null?'نامشخص':fmt(x.profitPct)+'%'}</td><td>${fmt(x.coverage?.coveredSalesPercent||0)}٪</td><td>${statusBadge(x.profitStatus||'unknown')}</td></tr>`).join('')}</tbody></table>`;
    html+=`<h5>جدول پورسانت پیشنهادی</h5><table class="table"><thead><tr><th>نام فروشنده</th><th>جمع فروش</th><th>سود پوشش‌داده‌شده</th><th>نرخ</th><th>پورسانت</th><th>وضعیت</th></tr></thead><tbody>${(r.sellers||[]).map(x=>`<tr><td>${hesc(x.sellerName||x.sellerKey)}</td><td>${fmt(x.saleRial)}</td><td>${valueOrUnknown(x.fifoProfit,x.profitStatus)}</td><td>${hesc(String(x.commissionRate??''))}</td><td>${x.suggestedCommission==null?'متوقف تا تکمیل پوشش':fmt(x.suggestedCommission)}</td><td>${statusBadge(x.profitStatus||'unknown')}</td></tr>`).join('')}</tbody></table>`;
