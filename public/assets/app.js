...
(function(){
  ...
  async function loadResolutions(){
    ...
    try{
      ...
      box.innerHTML=`<table class="table"><thead><tr><th>Resolution</th><th>کالا</th><th>هزینه</th><th>بازه</th><th>منبع</th><th>وضعیت</th><th>ایجادکننده / تأییدکننده</th><th>Audit</th><th></th></tr></thead><tbody>${(report.list||[]).map(row=>`<tr>
        ...
        <td>${safe(row.sourceType)}<br><small>${safe(row.resolutionScope||'item')} ${safe(row.purchaseLineIdentity||'')}</small></td><td>${safe(row.status)}</td>
  ...
  window.pageManualCostResolution=async function(){
    ...
    q('#mcClear').onclick=()=>{selectedResolutionId='';selectedResolutionRevision=0;q('#mcResolutionId').textContent='Resolution جدید';['#mcItemCode','#mcItemGuid','#mcCost','#mcPurchaseDataset','#mcPurchaseLine','#mcTargetQuantity','#mcFrom','#mcTo','#mcAttachment','#mcReason','#mcNotes'].forEach(id=>{q(id).value='';});q('#mcScope').value='item';};
    q('#mcSave').onclick=async()=>{
      const body={revision:selectedResolutionRevision,itemCode:q('#mcItemCode').value,itemGuid:q('#mcItemGuid').value,manualCost:q('#mcCost').value,resolutionScope:q('#mcScope').value,purchaseDatasetId:q('#mcPurchaseDataset').value,purchaseLineIdentity:q('#mcPurchaseLine').value,targetQuantityExact:q('#mcTargetQuantity').value,effectiveFrom:q('#mcFrom').value,effectiveTo:q('#mcTo').value,sourceType:q('#mcSource').value,currency:'IRR',reason:q('#mcReason').value,notes:q('#mcNotes').value,attachment:q('#mcAttachment').value?{reference:q('#mcAttachment').value}:null};
      try{
...
(()=>{
  ...
  async function loadTotals(){const r=await api('/api/accounting/seller-financial-performance/totals?'+params());q('#sfCards').innerHTML=[card('فاکتور',number(r.invoiceCount)),card('ردیف',number(r.lineCount),`${number(r.unavailableLineCount)} پورسانت ناموجود`),card('فروش ناخالص',toman(r.saleValueExact),'تومان'),card(r.profitCoverageComplete?'سود واقعی FIFO':'سود Known FIFO',toman(r.profitCoverageComplete?r.actualFifoProfitExact:r.knownFifoProfitExact),r.profitCoverageComplete?'تومان؛ immutable':'تومان؛ سود کل ناموجود است'),card('سود پورسانتی',toman(r.commissionableProfitExact),'تومان'),card('پورسانت مقدماتی',toman(r.preliminaryCommissionExact),'تومان؛ غیرقابل پرداخت'),card('هزینه نامشخص',number(r.unknownCostLineCount),'ردیف')].join('');}
...
(()=>{
  ...
  async function fifoAuditPage(){
    ...
    const loadDrill=async line=>{drill.innerHTML='<div class="muted">در حال خواندن زنجیره Allocation…</div>';try{const data=await api(`/api/accounting/seller-financial-performance/lines/${encodeURIComponent(line)}/drilldown`);drill.innerHTML=`<div class="card"><div class="card-header"><h5>FIFO Drill-down — ${safe(data.line.saleLineIdentity)}</h5></div><div class="card-body"><div class="info">FIFO Dataset: ${safe(data.line.fifoDatasetId)} | Sale Snapshot: ${safe(data.line.saleSnapshotId)} | Coverage: ${safe(data.line.costCoverageStatus)}</div><h5>FIFO Allocations → Purchase Layer / Purchase Invoice</h5><pre>${safe(JSON.stringify(data.source.allocations||[],null,2))}</pre><h5>Manual Cost Evidence</h5><pre>${safe(JSON.stringify(data.source.manualCostEvidence||[],null,2))}</pre><h5>Cost exception / unknown reason</h5><pre>${safe(JSON.stringify({costCoverageStatus:data.line.costCoverageStatus,blockers:data.line.blockers||[],unknownCostReason:data.line.unknownCostReason||null},null,2))}</pre></div></div>`;}catch(error){drill.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}};
