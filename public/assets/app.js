...
/* Phase D — governed Commissionable Profit adjustment batches. The page is
   intentionally non-payable and never edits Actual FIFO Profit. */
(()=>{
  const PAGE='profit-adjustment-review',ALLOWED=['admin','accounting','manager'];
  const safe=value=>esc(value==null?'':String(value)),q=selector=>document.querySelector(selector),role=()=>userRole();
  const canEdit=()=>['admin','accounting'].includes(role()),canApprove=()=>['admin','manager'].includes(role());
  let candidateRows=[],selected=new Set(),currentBatch=null;
  ...
  function money(value){return value==null?'—':Number(value).toLocaleString('fa-IR');}
  function shell(){setPage('تعدیلات سود مبنای پورسانت',`<main class="main-content"><div class="warn"><b>غیرقابل پرداخت</b> — این workflow فقط سود مبنای پورسانت و دفتر سود ذخیره‌شده را تغییر می‌دهد. سود واقعی FIFO غیرقابل تغییر است.</div><section class="card"><h3>۱. فیلتر و Candidate Set</h3><div class="actions"><input id="pdFrom" placeholder="از تاریخ" value="14050401"><input id="pdTo" placeholder="تا تاریخ" value="14050431"><input id="pdSeller" placeholder="شناسه فروشنده"><input id="pdCategory" placeholder="Product Category"><select id="pdPool"><option value="">Rate Pool</option><option>NOTEBOOK</option><option>COMPONENT</option></select><input id="pdStore" placeholder="فروشگاه"><input id="pdSaleMin" placeholder="حداقل فروش فاکتور"><input id="pdSaleMax" placeholder="حداکثر فروش فاکتور"><input id="pdProfitMin" placeholder="حداقل سود FIFO"><input id="pdProfitMax" placeholder="حداکثر سود FIFO"><button class="btn" id="pdSearch">جست‌وجوی محلی</button></div><div id="pdCandidates" class="muted">فیلتر را اجرا کنید.</div></section><section class="card"><h3>۲. انتخاب و Preview</h3><div class="actions"><select id="pdClass"><option>BUSINESS</option><option>TEST</option></select><input id="pdPeriod" value="140504" placeholder="دوره"><select id="pdMode"><option value="high_profit_reduction">کاهش سود پُرسود</option><option value="low_profit_subsidy">یارانه فاکتور کم‌سود</option></select><input id="pdTarget" placeholder="سود مبنای هدف هر فاکتور (ریال)"><input id="pdReason" placeholder="دلیل مستند"><button class="mini" id="pdDraft">ذخیره Draft</button><button class="btn" id="pdPreview">Preview</button><button class="btn" id="pdSubmit" disabled>ارسال برای تأیید</button><button class="mini" id="pdCancel" disabled>لغو Draft</button></div><div id="pdPreviewBox" class="muted">Preview اجباری است و با هر تغییر انتخاب یا هدف باطل می‌شود.</div></section><section class="card"><h3>۳. تاریخچه، تأیید مستقل و Posting</h3><div id="pdHistory" class="muted">در حال بارگذاری…</div></section></main>`);}
  function filters(){const map={dateFrom:'pdFrom',dateTo:'pdTo',sellerIdentity:'pdSeller',productCategory:'pdCategory',ratePool:'pdPool',store:'pdStore',invoiceSaleMin:'pdSaleMin',invoiceSaleMax:'pdSaleMax',actualProfitMin:'pdProfitMin',actualProfitMax:'pdProfitMax'};const p=new URLSearchParams({completeCostOnly:'true'});for(const[key,id]of Object.entries(map)){const value=q('#'+id)?.value.trim();if(value)p.set(key,value);}return p;}
  function invalidate(){if(currentBatch?.status==='previewed')currentBatch=null;q('#pdSubmit')&&(q('#pdSubmit').disabled=true);q('#pdPreviewBox')&&(q('#pdPreviewBox').innerHTML='<div class="warn">انتخاب یا هدف تغییر کرد؛ Preview جدید الزامی است.</div>');}
  function candidateTable(){const grouped=new Map();for(const row of candidateRows){if(!grouped.has(row.saleInvoiceIdentity))grouped.set(row.saleInvoiceIdentity,[]);grouped.get(row.saleInvoiceIdentity).push(row);}q('#pdCandidates').innerHTML=`<div class="info">${candidateRows.length} ردیف از Run فعال. فقط ردیف complete-cost با Policy/Mapping/Rate/Discount معتبر قابل Preview است.</div>${[...grouped].map(([invoice,lines])=>`<details><summary>فاکتور ${safe(invoice)} — ${lines.length} ردیف — فروش ${money(lines[0].invoiceGrossSaleAmountExact||lines.reduce((s,x)=>s+Number(x.grossSaleAmountExact||0),0))}</summary><div style="overflow:auto"><table class="table"><thead><tr><th></th><th>کالا</th><th>دسته / Pool</th><th>سود واقعی FIFO</th><th>تخفیف رسمی</th><th>سود مبنای فعلی</th><th>Eligibility</th></tr></thead><tbody>${lines.map(row=>`<tr><td><input type="checkbox" class="pd-line" data-line="${safe(row.saleLineIdentity)}" ${selected.has(row.saleLineIdentity)?'checked':''}></td><td>${safe(row.itemCode)}<br>${safe(row.itemDescription)}</td><td>${safe(row.officialProductCategoryName)}<br>${safe(row.commissionRatePool)}</td><td>${money(row.actualFifoProfitExact)}</td><td>${money(Number(row.lineDiscountExact||0)+Number(row.allocatedInvoiceDiscountExact||0))}</td><td>${money(row.baseCommissionableProfitExact)}</td><td>${row.eligibilityBlockers.length?`<span class="error">${safe(row.eligibilityBlockers.join('، '))}</span>`:'مجاز'}</td></tr>`).join('')}</tbody></table></div></details>`).join('')||'<div class="warn">Candidate موجود نیست.</div>'}`;document.querySelectorAll('.pd-line').forEach(input=>input.onchange=()=>{input.checked?selected.add(input.dataset.line):selected.delete(input.dataset.line);invalidate();});}
  async function search(){q('#pdCandidates').textContent='در حال خواندن Read Model محلی…';try{const r=await api('/api/accounting/profit-adjustment-batches/candidates?'+filters());candidateRows=r.list||[];selected.clear();currentBatch=null;candidateTable();}catch(error){q('#pdCandidates').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  function selectedRows(){return candidateRows.filter(row=>selected.has(row.saleLineIdentity));}
  function draftPayload(){const rows=selectedRows();if(!rows.length)throw new Error('حداقل یک ردیف را انتخاب کنید.');const categories=new Set(rows.map(row=>row.officialProductCategoryIdentity)),pools=new Set(rows.map(row=>row.commissionRatePool)),policies=new Set(rows.map(row=>row.policyVersionId));if(categories.size!==1||pools.size!==1||policies.size!==1)throw new Error('هر Batch باید دقیقاً یک Product Category، Rate Pool و Policy داشته باشد.');return{classification:q('#pdClass').value,policyVersionId:[...policies][0],accountingPeriod:q('#pdPeriod').value,sellerIdentity:rows[0].sellerIdentity,officialProductCategoryIdentity:[...categories][0],officialProductCategoryName:rows[0].officialProductCategoryName,commissionRatePool:[...pools][0],filterSnapshot:Object.fromEntries(filters()),candidateLineIds:candidateRows.map(row=>row.saleLineIdentity),selectedInvoiceIds:[...new Set(rows.map(row=>row.saleInvoiceIdentity))],selectedLineIds:rows.map(row=>row.saleLineIdentity),adjustmentMode:q('#pdMode').value,targetMethod:'target_commissionable_profit_per_invoice',targetValueExact:q('#pdTarget').value};}
  async function createDraft(){try{currentBatch=(await api('/api/accounting/profit-adjustment-batches',{method:'POST',body:JSON.stringify(draftPayload())})).batch;q('#pdPreviewBox').innerHTML=`<div class="info">Draft ${safe(currentBatch.batchId)} ذخیره شد. هنوز هیچ Adjustment یا Ledger entry ایجاد نشده است.</div>`;q('#pdCancel').disabled=false;await history();return currentBatch;}catch(error){q('#pdPreviewBox').innerHTML=`<div class="error">${safe(error.code||'')} ${safe(error.message)}</div>`;throw error;}}
  async function preview(){try{if(!currentBatch||!['draft','previewed'].includes(currentBatch.status))await createDraft();const payload={revision:currentBatch.revision,selectedLineIds:[...selected],targetValueExact:q('#pdTarget').value,allowSaleValueFallback:false};const r=await api(`/api/accounting/profit-adjustment-batches/${encodeURIComponent(currentBatch.batchId)}/preview`,{method:'POST',body:JSON.stringify(payload)});currentBatch=r.batch;q('#pdPreviewBox').innerHTML=`<div class="info"><b>Preview ${safe(currentBatch.previewFingerprint)}</b><br>سود ذخیره‌شده: ${money(currentBatch.expectedCreditExact)} ریال | سود مصرف‌شده: ${money(currentBatch.expectedDebitExact)} ریال<br>مانده قبل: ${money(currentBatch.poolBalanceBeforeExact)} | رزروهای pending: ${money(currentBatch.pendingDebitReservationsExact)} | مانده قابل استفاده پس از Preview: ${money(currentBatch.poolBalanceAfterPreviewExact)}</div><div style="overflow:auto"><table class="table"><thead><tr><th>فاکتور</th><th>سود مبنای فعلی</th><th>هدف</th><th>تعدیل</th><th>روش تخصیص</th></tr></thead><tbody>${(r.invoicePreviews||[]).map(row=>`<tr><td>${safe(row.saleInvoiceIdentity)}</td><td>${money(row.baseCommissionableProfitExact)}</td><td>${money(row.targetCommissionableProfitExact)}</td><td>${money(row.adjustmentAmountExact)}</td><td>${safe(row.allocationMethod)}</td></tr>`).join('')}</tbody></table></div>`;q('#pdSubmit').disabled=false;q('#pdCancel').disabled=false;await history();}catch(error){q('#pdPreviewBox').innerHTML=`<div class="error">${safe(error.code||'')} ${safe(error.message)}</div>`;}}
  async function action(id,action,revision){const reason=q('#pdReason').value.trim();if(!reason){q('#pdPreviewBox').innerHTML='<div class="error">دلیل مستند الزامی است.</div>';return;}try{const r=await api(`/api/accounting/profit-adjustment-batches/${encodeURIComponent(id)}/${action}`,{method:'POST',body:JSON.stringify({revision,reason})});if(currentBatch?.batchId===id)currentBatch=r.batch;q('#pdPreviewBox').innerHTML=`<div class="info">عملیات ${safe(action)} ثبت شد؛ وضعیت ${safe(r.batch?.status)}.</div>`;await history();}catch(error){q('#pdPreviewBox').innerHTML=`<div class="error">${safe(error.code||'')} ${safe(error.message)}</div>`;}}
  async function history(){try{const [r,s]=await Promise.all([api('/api/accounting/profit-adjustment-batches?pageSize=100'),api('/api/accounting/profit-adjustment-batches/monthly-statement?accountingPeriod='+encodeURIComponent(q('#pdPeriod')?.value||'140504'))]);q('#pdHistory').innerHTML=`<div class="info">دفتر ماهانه: ${(s.statements||[]).map(row=>`${safe(row.pool)} مانده ${money(row.closingBalanceExact)} / رزرو ${money(row.pendingDebitReservationsExact)} / قابل استفاده ${money(row.availableApprovedBalanceExact)}`).join(' | ')}</div><div style="overflow:auto"><table class="table"><thead><tr><th>Batch</th><th>Classification / Creator</th><th>دسته / Pool</th><th>Credit / Debit</th><th>Status / revision</th><th>اثر فعلی</th><th>Workflow</th></tr></thead><tbody>${(r.list||[]).map(row=>{const buttons=[];if(canEdit()&&row.status==='previewed')buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="submit" data-revision="${row.revision}">Submit</button>`);if(canEdit()&&['draft','previewed'].includes(row.status))buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="cancel" data-revision="${row.revision}">Cancel</button>`);if(canApprove()&&row.status==='pending')buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="approve" data-revision="${row.revision}">Approve + Post</button> <button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="reject" data-revision="${row.revision}">Reject</button>`);if(canApprove()&&row.status==='post_failed_retryable')buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="retry-post" data-revision="${row.revision}">Retry idempotent post</button>`);if(canEdit()&&row.status==='posted'&&row.reversalRequest?.status!=='pending')buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="request-reversal" data-revision="${row.revision}">درخواست Reversal</button>`);if(canApprove()&&row.status==='posted'&&row.reversalRequest?.status==='pending')buttons.push(`<button class="mini pd-action" data-id="${safe(row.batchId)}" data-action="approve-reversal" data-revision="${row.revision}">تأیید مستقل Reversal</button>`);return`<tr><td>${safe(row.batchId)}<br><small>${safe(row.previewFingerprint||'بدون Preview')}</small></td><td>${safe(row.classification)}<br>${safe(row.createdBy?.username)}<br>${safe(row.createdAt)}</td><td>${safe(row.officialProductCategoryName)}<br>${safe(row.commissionRatePool)}</td><td>${money(row.expectedCreditExact)} / ${money(row.expectedDebitExact)}</td><td>${safe(row.status)} r${row.revision}</td><td>${row.status==='posted'?'در محاسبات فعال؛ reversal برای cleanup لازم است':'بدون اثر مالی'}</td><td>${buttons.join(' ')||'—'}<details><summary>Audit</summary><pre>${safe(JSON.stringify(row.auditLog||[],null,2))}</pre></details></td></tr>`;}).join('')||'<tr><td colspan="7">Batch موجود نیست.</td></tr>'}</tbody></table></div>`;document.querySelectorAll('.pd-action').forEach(button=>button.onclick=()=>action(button.dataset.id,button.dataset.action,Number(button.dataset.revision)));}catch(error){q('#pdHistory').innerHTML=`<div class="error">${safe(error.code||'')} ${safe(error.message)}</div>`;}}
  async function render(){if(!ALLOWED.includes(role())){setPage('عدم دسترسی','<main class="main-content"><div class="error">این صفحه برای نقش فروشنده قابل مشاهده نیست.</div></main>');return;}shell();q('#pdSearch').onclick=search;q('#pdDraft').onclick=createDraft;q('#pdPreview').onclick=preview;q('#pdSubmit').onclick=()=>currentBatch&&action(currentBatch.batchId,'submit',currentBatch.revision);q('#pdCancel').onclick=()=>currentBatch&&action(currentBatch.batchId,'cancel',currentBatch.revision);for(const id of ['pdClass','pdPeriod','pdMode','pdTarget'])q('#'+id).onchange=invalidate;await history();}
  window.__profitAdjustmentBatchPage=render;const inheritedRoute=window.route||route;window.route=route=async function(){const page=location.hash.slice(1)||firstAllowedPage();if(page===PAGE)return render();return inheritedRoute.apply(this,arguments);};
})();
...
(()=>{
  ...
  function actions(row,type){if(row.historicalFrozen||row.status==='historical_frozen')return '<span class="muted">تاریخی و منجمد</span>';const base=type==='policy'?'/api/accounting/commission-policies':type==='mapping'?'/api/accounting/profit-ledger/categories':'/api/accounting/profit-ledger/rates';const id=recordId(row,type);if(!id)return '—';const buttons=[];if(EDIT.includes(userRole())&&row.status==='draft')buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="submit" data-revision="${safe(row.revision)}">ارسال برای تأیید</button>`);if(APPROVE.includes(userRole())&&row.status==='pending'){buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="approve" data-revision="${safe(row.revision)}">تأیید مستقل</button>`);buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="reject" data-revision="${safe(row.revision)}">رد کردن</button>`);}if(APPROVE.includes(userRole())&&row.status==='approved'&&type==='policy')buttons.push(`<button class="mini fg-transition" data-record-type="policy" data-base="${base}" data-id="${safe(id)}" data-action="retire" data-revision="${safe(row.revision)}">بازنشسته‌کردن</button>`);return buttons.join(' ')||'—';}
  ...
  async function mappingPage(){
    ...
    try{
      const [groups,mappings]=await Promise.all([api('/api/accounting/governance/group-review?periodFrom=14050401&periodTo=14050431&pageSize=500'),api('/api/accounting/profit-ledger/categories?pageSize=500')]);
      const officialGroups=(groups.list||[]).filter(row=>row.officialEvidence&&row.officialProductCategoryGuid);
      if(q('#fgmGroup'))q('#fgmGroup').innerHTML=officialGroups.map(row=>`<option value="${safe(row.officialProductCategoryIdentity)}" data-guid="${safe(row.officialProductCategoryGuid)}" data-number="${safe(row.officialProductCategoryNumber)}" data-name="${safe(row.officialProductCategoryName)}">${safe(row.officialProductCategoryNumber)} — ${safe(row.officialProductCategoryName)}</option>`).join('');
      const createMapping=async group=>{const policyVersionId=q('#fgmPolicy').value;if(!policyVersionId){message('ابتدا یک سیاست پورسانت مصوب را انتخاب کنید.',true);return;}if(!group?.officialProductCategoryGuid){message('هویت GUID گروه اصلی رسمی موجود نیست.',true);return;}try{const result=await api('/api/accounting/profit-ledger/categories',{method:'POST',body:JSON.stringify({policyVersionId,groupPathIdentity:group.officialProductCategoryIdentity,officialProductCategoryIdentity:group.officialProductCategoryIdentity,officialProductCategoryGuid:group.officialProductCategoryGuid,officialProductCategoryNumber:group.officialProductCategoryNumber,officialProductCategoryName:group.officialProductCategoryName,commissionRatePool:q('#fgmPool').value,effectiveFrom:q('#fgmFrom').value,effectiveTo:q('#fgmTo').value,description:q('#fgmDescription').value})});message(result.openedExisting?`پیش‌نویس باز موجود شد و رکورد جدید ساخته نشد: ${result.mapping?.mappingId||'شناسه نامشخص'}`:`پیش‌نویس واقعی ایجاد شد: ${result.mapping?.mappingId||'شناسه نامشخص'} / بازبینی ${result.mapping?.revision||'—'}`);await mappingPage();}catch(error){message(`${error.code} ${error.message}`,true,error.code);}};
      const renderRows=()=>{const list=q('#fgmList');if(!list)return;const policyVersionId=q('#fgmPolicy')?.value||'',effectiveFrom=q('#fgmFrom')?.value||'',effectiveTo=q('#fgmTo')?.value||'';const activeStatuses=new Set(['draft','pending','returned','approved']);const rows=officialGroups.map(group=>{const matches=(mappings.list||[]).filter(row=>activeStatuses.has(row.status)&&row.policyVersionId===policyVersionId&&row.officialProductCategoryGuid===group.officialProductCategoryGuid&&row.effectiveFrom===effectiveFrom&&(row.effectiveTo||'')===effectiveTo);if(matches.length===1)return{kind:'persisted',group,row:matches[0]};if(matches.length>1)return{kind:'conflict',group,matches};return{kind:'projection',group};});list.innerHTML=`<div class="info">ردیف ثبت‌نشده فقط تصویر گروه رسمی است و تا ایجاد پیش‌نویس، mappingId، revision یا اقدام ارسال ندارد.</div><table class="table"><thead><tr><th>دسته رسمی کالا</th><th>سبد نرخ</th><th>سیاست</th><th>اثر</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${rows.map(entry=>{const group=entry.group;if(entry.kind==='persisted'){const row=entry.row;return `<tr data-row-type="persisted" data-mapping-id="${safe(row.mappingId)}"><td>${safe(group.officialProductCategoryNumber)} — <b>${safe(group.officialProductCategoryName)}</b><details><summary>جزئیات حسابرسی</summary>GUID: ${safe(group.officialProductCategoryGuid)}<br>mappingId: ${safe(row.mappingId)}</details></td><td>${safe(row.commissionRatePool)}</td><td>${safe(row.policyVersionId)}</td><td>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td><td>${safe(statusFa(row.status))} / بازبینی ${safe(row.revision)}</td><td>${actions(row,'mapping')}</td></tr>`;}if(entry.kind==='conflict')return `<tr data-row-type="conflict"><td>${safe(group.officialProductCategoryNumber)} — <b>${safe(group.officialProductCategoryName)}</b><br><small>GUID: ${safe(group.officialProductCategoryGuid)}</small></td><td colspan="3" class="error">چند Mapping باز با هویت و دوره یکسان: ${entry.matches.map(row=>safe(row.mappingId)).join('، ')}</td><td>تعارض داده</td><td>نیازمند تصمیم انسانی</td></tr>`;return `<tr data-row-type="projection" data-category-guid="${safe(group.officialProductCategoryGuid)}"><td>${safe(group.officialProductCategoryNumber)} — <b>${safe(group.officialProductCategoryName)}</b><br><small>GUID: ${safe(group.officialProductCategoryGuid)}</small></td><td>${safe(q('#fgmPool')?.value||'UNRESOLVED')}</td><td>${safe(policyVersionId||'انتخاب‌نشده')}</td><td>${safe(effectiveFrom)} تا ${safe(effectiveTo||'باز')}</td><td>${safe(statusFa('unregistered'))}</td><td>${editable&&policyVersionId?`<button class="mini fgm-create-projection" data-guid="${safe(group.officialProductCategoryGuid)}">ایجاد پیش‌نویس</button>`:'—'}</td></tr>`;}).join('')||'<tr><td colspan="6">گروه رسمی قابل نمایش وجود ندارد.</td></tr>'}</tbody></table><details><summary>سوابق کامل نگاشت و رکورد تاریخی (${safe((mappings.list||[]).length)})</summary><table class="table"><tbody>${(mappings.list||[]).map(row=>`<tr data-row-type="persisted-history" data-mapping-id="${safe(row.mappingId)}"><td>${safe(row.officialProductCategoryName||row.identityValue||'—')}</td><td>${safe(row.mappingId||'—')}</td><td>${safe(statusFa(row.status))}</td><td>بازبینی ${safe(row.revision)}</td></tr>`).join('')}</tbody></table></details>`;list.querySelectorAll('.fgm-create-projection').forEach(button=>button.onclick=()=>createMapping(officialGroups.find(group=>group.officialProductCategoryGuid===button.dataset.guid)));bindTransitions(mappingPage);};
      const rerender=()=>renderRows();if(q('#fgmPolicy')){const policyRender=q('#fgmPolicy').onchange;q('#fgmPolicy').onchange=()=>{policyRender?.();rerender();};}['fgmFrom','fgmTo','fgmPool'].forEach(id=>{if(q(`#${id}`))q(`#${id}`).onchange=rerender;});renderRows();
      if(q('#fgmCreate'))q('#fgmCreate').onclick=()=>{const option=q('#fgmGroup').selectedOptions[0];if(!option){message('گروه اصلی رسمی قابل انتخاب وجود ندارد.',true);return;}return createMapping({officialProductCategoryIdentity:option.value,officialProductCategoryGuid:option.dataset.guid,officialProductCategoryNumber:option.dataset.number,officialProductCategoryName:option.dataset.name});};
...
/* Phase D terminal route binding. */
(()=>{
  const inheritedRoute=window.route||route;
  window.route=route=async function(){
    const current=location.hash.slice(1)||firstAllowedPage();
    if(current==='profit-adjustment-review'&&typeof window.__profitAdjustmentBatchPage==='function')return window.__profitAdjustmentBatchPage();
    return inheritedRoute.apply(this,arguments);
  };
})();
