...
(()=>{
  ...
  function page(title,body){setPage(title,`<main class="main-content"><div class="warn">سود FIFO تغییرناپذیر است؛ این صفحات فقط حاکمیت Policy، دسته و نرخ را مدیریت می‌کنند و هیچ تأیید خودکاری ندارند.</div>${body}<div id="fgMessage"></div></main>`);}
  ...
  async function policyPage(){
    const editable=EDIT.includes(userRole());page('سیاست‌های پورسانت',`${editable?`<div class="card"><div class="card-header"><h5>Policy جدید</h5></div><div class="card-body"><div class="row four"><div class="form-group"><label>نام</label><input id="fgpName"></div><div class="form-group"><label>دوره شمسی</label><input id="fgpPeriod" placeholder="140504" maxlength="6"></div><div class="form-group"><label>از تاریخ</label><input id="fgpFrom" placeholder="14050401" maxlength="8"></div><div class="form-group"><label>تا تاریخ</label><input id="fgpTo" placeholder="14050431" maxlength="8"></div></div><div class="form-group"><label>شرح (اختیاری)</label><textarea id="fgpDescription"></textarea></div><button class="btn" id="fgpCreate">ایجاد Draft</button></div></div>`:''}<div class="card"><div class="card-header"><h5>نسخه‌های Policy</h5></div><div class="card-body">${reasonBox()}<div id="fgpList">در حال بارگذاری…</div></div></div>`);
    try{const result=await api('/api/accounting/commission-policies');q('#fgpList').innerHTML=`<table class="table"><thead><tr><th>Policy</th><th>دوره و اثر</th><th>وضعیت</th><th>هویت‌ها</th><th>اقدام</th></tr></thead><tbody>${(result.list||[]).map(row=>`<tr><td><b>${safe(row.name)}</b><br><small>${safe(row.policyVersionId)}</small></td><td>${safe(row.accountingPeriod)}<br>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td><td>${safe(row.status)} / r${safe(row.revision)}</td><td>سازنده: ${safe(row.createdBy?.username)}<br>تأییدکننده: ${safe(row.approvedBy?.username||'—')}</td><td>${actions(row,'policy')}</td></tr>`).join('')||'<tr><td colspan="5">رکوردی وجود ندارد.</td></tr>'}</tbody></table>`;bindTransitions(policyPage);if(q('#fgpCreate'))q('#fgpCreate').onclick=async()=>{try{await api('/api/accounting/commission-policies',{method:'POST',body:JSON.stringify({name:q('#fgpName').value,accountingPeriod:q('#fgpPeriod').value,effectiveFrom:q('#fgpFrom').value,effectiveTo:q('#fgpTo').value,description:q('#fgpDescription').value})});await policyPage();}catch(error){message(`${error.code} ${error.message}`,true);}};}catch(error){message(`${error.code} ${error.message}`,true);}
  }
  async function mappingPage(){
    const editable=EDIT.includes(userRole());page('تعریف پورسانت گروه کالا',`${editable?`<div class="card"><div class="card-header"><h5>نگاشت Product Category رسمی به Rate Pool</h5></div><div class="card-body"><div class="info">Product Category همان Main Group رسمی شایگان است. نرخ در این فرم تعریف نمی‌شود.</div><div class="row four"><div class="form-group"><label>Policy مصوب</label><select id="fgmPolicy"></select></div><div class="form-group"><label>Main Group رسمی</label><select id="fgmGroup"></select></div><div class="form-group"><label>Commission Rate Pool</label><select id="fgmPool"><option>NOTEBOOK</option><option>COMPONENT</option><option>CUSTOM</option><option>NON_COMMISSIONABLE</option><option>UNRESOLVED</option></select></div><div class="form-group"><label>شرح (اختیاری)</label><input id="fgmDescription"></div></div><div class="row four"><div class="form-group"><label>از تاریخ</label><input id="fgmFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ</label><input id="fgmTo" value="14050431"></div><div class="form-group"><label>&nbsp;</label><button class="btn" id="fgmCreate">ایجاد Draft</button></div></div></div></div>`:''}<div class="card"><div class="card-header"><h5>سوابق دسته و Pool</h5></div><div class="card-body">${reasonBox()}<div id="fgmList">در حال بارگذاری…</div></div></div>`);
    try{const [policies,groups,mappings]=await Promise.all([api('/api/accounting/commission-policies'),api('/api/accounting/governance/group-review?periodFrom=14050401&periodTo=14050431&pageSize=500'),api('/api/accounting/profit-ledger/categories?pageSize=500')]);if(q('#fgmPolicy'))q('#fgmPolicy').innerHTML=(policies.selectable||[]).map(row=>`<option value="${safe(row.policyVersionId)}">${safe(row.name)} — ${safe(row.accountingPeriod)}</option>`).join('');if(q('#fgmGroup'))q('#fgmGroup').innerHTML=(groups.list||[]).filter(row=>row.officialEvidence).map(row=>`<option value="${safe(row.officialProductCategoryIdentity)}" data-guid="${safe(row.officialProductCategoryGuid)}" data-number="${safe(row.officialProductCategoryNumber)}" data-name="${safe(row.officialProductCategoryName)}">${safe(row.officialProductCategoryNumber)} — ${safe(row.officialProductCategoryName)}</option>`).join('');q('#fgmList').innerHTML=`<table class="table"><thead><tr><th>Product Category رسمی</th><th>Rate Pool</th><th>Policy</th><th>اثر</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${(mappings.list||[]).map(row=>`<tr><td>${safe(row.officialProductCategoryNumber)} — <b>${safe(row.officialProductCategoryName||row.identityValue)}</b></td><td>${safe(row.commissionRatePool)}</td><td>${safe(row.policyVersionId||'—')}</td><td>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td><td>${safe(row.status)} / r${safe(row.revision)}</td><td>${actions(row,'mapping')}</td></tr>`).join('')||'<tr><td colspan="6">رکوردی وجود ندارد.</td></tr>'}</tbody></table>`;bindTransitions(mappingPage);if(q('#fgmCreate'))q('#fgmCreate').onclick=async()=>{const option=q('#fgmGroup').selectedOptions[0];try{await api('/api/accounting/profit-ledger/categories',{method:'POST',body:JSON.stringify({policyVersionId:q('#fgmPolicy').value,groupPathIdentity:option.value,officialProductCategoryIdentity:option.value,officialProductCategoryGuid:option.dataset.guid,officialProductCategoryNumber:option.dataset.number,officialProductCategoryName:option.dataset.name,commissionRatePool:q('#fgmPool').value,effectiveFrom:q('#fgmFrom').value,effectiveTo:q('#fgmTo').value,description:q('#fgmDescription').value})});await mappingPage();}catch(error){message(`${error.code} ${error.message}`,true);}};}catch(error){message(`${error.code} ${error.message}`,true);}
  }
  async function ratePage(){
    const editable=EDIT.includes(userRole());page('نرخ‌های پورسانت',`${editable?`<div class="card"><div class="card-header"><h5>Rate Version جدید</h5></div><div class="card-body"><div class="info">ترتیب قطعی: seller/category ← seller/rate-pool ← category default ← rate-pool default. نرخ Missing هرگز صفر نیست.</div><div class="row four"><div class="form-group"><label>Policy مصوب</label><select id="fgrPolicy"></select></div><div class="form-group"><label>فروشنده (* برای پیش‌فرض)</label><input id="fgrSeller" value="*"></div><div class="form-group"><label>Scope</label><select id="fgrScope"><option value="rate_pool">Rate Pool</option><option value="product_category">Product Category</option></select></div><div class="form-group"><label>نرخ اعشاری</label><input id="fgrRate" placeholder="0.20"></div></div><div class="row four"><div class="form-group"><label>Rate Pool</label><select id="fgrPool"><option>NOTEBOOK</option><option>COMPONENT</option><option>CUSTOM</option><option>NON_COMMISSIONABLE</option></select></div><div class="form-group"><label>شناسه Product Category</label><input id="fgrCategoryId"></div><div class="form-group"><label>نام Product Category</label><input id="fgrCategoryName"></div><div class="form-group"><label>شرح/مرجع (اختیاری)</label><input id="fgrDescription"></div></div><div class="row four"><div class="form-group"><label>از تاریخ</label><input id="fgrFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ</label><input id="fgrTo" value="14050431"></div><div class="form-group"><label>&nbsp;</label><button class="btn" id="fgrCreate">ایجاد Draft</button></div></div></div></div>`:''}<div class="card"><div class="card-header"><h5>نسخه‌های نرخ</h5></div><div class="card-body">${reasonBox()}<div id="fgrList">در حال بارگذاری…</div></div></div>`);
    try{const [policies,rates]=await Promise.all([api('/api/accounting/commission-policies'),api('/api/accounting/profit-ledger/rates?pageSize=500')]);if(q('#fgrPolicy'))q('#fgrPolicy').innerHTML=(policies.selectable||[]).map(row=>`<option value="${safe(row.policyVersionId)}">${safe(row.name)} — ${safe(row.accountingPeriod)}</option>`).join('');q('#fgrList').innerHTML=`<table class="table"><thead><tr><th>فروشنده / Scope</th><th>Category / Pool</th><th>نرخ</th><th>Policy</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${(rates.list||[]).map(row=>`<tr><td>${safe(row.sellerIdentity)} / ${safe(row.rateScope)}</td><td>${safe(row.officialProductCategoryName||row.commissionRatePool)}</td><td>${safe(row.rate)}</td><td>${safe(row.policyVersionId||'—')}</td><td>${safe(row.status)} / r${safe(row.revision)}</td><td>${actions(row,'rate')}</td></tr>`).join('')||'<tr><td colspan="6">رکوردی وجود ندارد.</td></tr>'}</tbody></table>`;bindTransitions(ratePage);if(q('#fgrCreate'))q('#fgrCreate').onclick=async()=>{const scope=q('#fgrScope').value;try{await api('/api/accounting/profit-ledger/rates',{method:'POST',body:JSON.stringify({policyVersionId:q('#fgrPolicy').value,sellerIdentity:q('#fgrSeller').value,rateScope:scope,commissionRatePool:scope==='rate_pool'?q('#fgrPool').value:'',officialProductCategoryIdentity:scope==='product_category'?q('#fgrCategoryId').value:'',officialProductCategoryName:scope==='product_category'?q('#fgrCategoryName').value:'',rate:q('#fgrRate').value,effectiveFrom:q('#fgrFrom').value,effectiveTo:q('#fgrTo').value,reason:q('#fgrDescription').value})});await ratePage();}catch(error){message(`${error.code} ${error.message}`,true);}};}catch(error){message(`${error.code} ${error.message}`,true);}
  }
