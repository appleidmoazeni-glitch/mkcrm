...
(()=>{
  ...
  function shell(){setPage('عملکرد فروشنده',`<main class="main-content">
    ...
    <div class="actions"><button class="btn" id="sfApply">اعمال فیلتر</button><select id="sfView"><option value="lines">نمای ردیف فروش</option><option value="invoices">نمای فاکتور</option></select><button class="btn" id="sfSavePreset">ذخیره Preset</button><select id="sfPreset"><option value="">Preset ذخیره‌شده</option></select><button class="mini" id="sfLoadPreset">بارگذاری</button><button class="mini" id="sfFreshness">بررسی تازگی</button>${['admin','accounting'].includes(userRole())?'<button class="btn green" id="sfRebuild">ساخت Read Model</button>':''}</div>
  ...
  async function pageRenderer(){
    ...
    if(!statusCache.enabled)return legacyPage();shell();q('#sfStatus').textContent=`Run فعال: ${statusCache.activeRunId||'موجود نیست'} | ${statusCache.buildLocked?'در حال ساخت':'آماده'} | NON-PAYABLE`;renderPresets();
    ...
    q('#sfFreshness').onclick=async()=>{try{const r=await api('/api/accounting/seller-financial-performance/freshness');q('#sfStatus').innerHTML=`<div class="${r.stale?'warn':'success'}">${r.stale?'Read Model قدیمی است و باید rebuild شود.':'Read Model با منابع فعلی یکسان است.'}</div>`;}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.message)}</div>`;}};
