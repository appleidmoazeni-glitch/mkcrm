...
(()=>{
  ...
  function actions(row,type){if(row.historicalFrozen||row.status==='historical_frozen')return '<span class="muted">تاریخی و منجمد</span>';const base=type==='policy'?'/api/accounting/commission-policies':type==='mapping'?'/api/accounting/profit-ledger/categories':'/api/accounting/profit-ledger/rates';const id=recordId(row,type);if(!id)return '—';const buttons=[];if(EDIT.includes(userRole())&&row.status==='draft')buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="submit" data-revision="${safe(row.revision)}">ارسال برای تأیید</button>`);if(APPROVE.includes(userRole())&&row.status==='pending')buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="approve" data-revision="${safe(row.revision)}">تأیید مستقل</button>`);if(APPROVE.includes(userRole())&&row.status==='approved'&&type==='policy')buttons.push(`<button class="mini fg-transition" data-record-type="policy" data-base="${base}" data-id="${safe(id)}" data-action="retire" data-revision="${safe(row.revision)}">بازنشسته‌کردن</button>`);return buttons.join(' ')||'—';}
