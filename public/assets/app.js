...
/* 0.9.19.71 Accounting Governance UI. All suggestions remain human-review
   candidates; approval is role-separated and no source accounting dataset is
   mutated by these screens. */
(()=>{
  const PAGES={'commission-category-governance':'Category Mapping Governance','commission-rate-governance':'Commission Rate Governance','saved-profit-opening-governance':'Saved-Profit Opening Balances','commission-export-readiness':'Commission Export Readiness'};
  ...
  function workflow(row,kind){const buttons=[];if(canEdit()&&['draft','returned'].includes(row.status)){buttons.push('submit','cancel');}if(canApprove()&&row.status==='pending'){buttons.push('approve','reject','return');}return buttons.map(action=>`<button class="mini ag-action" data-kind="${kind}" data-id="${safe(row.mappingId||row.rateVersionId||row.openingBalanceId)}" data-action="${action}" data-revision="${Number(row.revision)}">${action}</button>`).join(' ')||'—';}
  ...
  function bindActions(reload){document.querySelectorAll('.ag-action').forEach(button=>button.onclick=()=>evidenceAction(button,reload));}
  let groupFilters={search:'',minSaleValue:''};
  async function categories(){shell('commission-category-governance',`<div class="actions"><input id="agGroupSearch" placeholder="Group, item or invoice" value="${safe(groupFilters.search)}"><input id="agGroupMinValue" placeholder="Minimum Tir sale value" value="${safe(groupFilters.minSaleValue)}"><button class="mini" id="agGroupFilter">Filter</button>${canEdit()?'<button class="btn" id="agRefresh">Refresh official Item/Get catalogue</button>':''}</div>`);try{const reviewUrl=`/api/accounting/governance/group-review?periodFrom=14050401&periodTo=14050431&pageSize=500&search=${encodeURIComponent(groupFilters.search)}&minSaleValue=${encodeURIComponent(groupFilters.minSaleValue)}`;const [review,mappings]=await Promise.all([api(reviewUrl),api('/api/accounting/profit-ledger/categories?pageSize=500')]);q('#agBody').innerHTML=`<div class="info">Official group rows ${safe(review.total)} | unresolved lines ${safe(review.catalog?.unresolvedLineCount)} | suggestions are never approved automatically.</div>${table(['Official group / hierarchy','Affected items / invoices','Tir metrics / projected gain','Suggestion / evidence','Current mapping','Action'],(review.list||[]).map(row=>`<tr><td>${safe(row.groupNumber)} — ${safe(row.groupName)}<br><small>${safe(row.groupIdentity)}<br>${safe(row.parentGroupNumber)} ${safe(row.parentGroupName)} | ${safe(row.hierarchyStatus)}</small></td><td>${safe(row.itemCount)} items<details><summary>Samples</summary>Items: ${safe((row.representativeItems||[]).join(', '))}<br>Invoices: ${safe((row.representativeInvoices||[]).join(', '))}</details></td><td>${safe(row.saleLineCount)} lines / ${safe(row.saleQuantity)} qty<br>${safe(row.saleValue)} IRR<br>profit ${safe(row.knownFifoProfit)} / unknown ${safe(row.unknownCostValue)}<br><b>Projected +${safe(row.projectedCoverageGainPercent)}%</b></td><td>${safe(row.suggestedCommissionCategory)}<br><small>${safe(row.suggestionEvidence)}</small></td><td>${safe(row.currentMappingStatus)} / ${safe(row.currentCommissionCategory)}</td><td>${canEdit()&&row.groupIdentity!=='UNRESOLVED'?`<button class="mini ag-map-create" data-identity="${safe(row.groupIdentity)}" data-suggestion="${safe(row.suggestedCommissionCategory)}">Create draft</button>`:'—'}</td></tr>`))}<h4>Governed mapping history</h4>${table(['Mapping','Identity / Category','Period','Evidence','Status / revision','Workflow / audit'],(mappings.list||[]).map(row=>`<tr><td>${safe(row.mappingId)}</td><td>${safe(row.identityType)}: ${safe(row.identityValue)}<br><b>${safe(row.commissionCategory)}</b></td><td>${safe(row.effectiveFrom)}–${safe(row.effectiveTo||'open')}</td><td>${safe(row.sourceReference)}<br>${safe(row.reason)}</td><td>${safe(row.status)} r${safe(row.revision)}</td><td>${workflow(row,'mapping')}<details><summary>Audit</summary><pre>${safe(JSON.stringify(row.auditLog||[],null,2))}</pre></details></td></tr>`))}`;bindActions(categories);document.querySelectorAll('.ag-map-create').forEach(button=>button.onclick=async()=>{const commissionCategory=prompt('Category: NOTEBOOK / COMPONENT / OTHER / SERVICE / NON_COMMISSIONABLE',button.dataset.suggestion==='UNKNOWN'?'COMPONENT':button.dataset.suggestion)||'';const sourceReference=prompt('Evidence reference:')||'';const reason=prompt('Reason:')||'';const evidenceUnavailableReason=prompt('Evidence unavailable reason (if no attachment):')||'';if(!commissionCategory||!sourceReference||!reason||!evidenceUnavailableReason)return;try{await api('/api/accounting/profit-ledger/categories',{method:'POST',body:JSON.stringify({groupPathIdentity:button.dataset.identity,commissionCategory,effectiveFrom:'14050401',effectiveTo:'14050431',source:'official-group-human-review',sourceReference,reason,evidenceMetadata:{evidenceUnavailableReason}})});await categories();}catch(error){alert(`${error.code||''} ${error.message}`);}});if(q('#agGroupFilter'))q('#agGroupFilter').onclick=()=>{groupFilters={search:q('#agGroupSearch').value,minSaleValue:q('#agGroupMinValue').value};categories();};if(q('#agRefresh'))q('#agRefresh').onclick=async()=>{if(!confirm('فقط Item/Get رسمی به‌صورت read-only خوانده شود؟'))return;try{await api('/api/accounting/governance/group-catalog/refresh',{method:'POST',body:JSON.stringify({pageSize:100,maxPages:1000})});await categories();}catch(error){alert(`${error.code||''} ${error.message}`);}};}catch(error){q('#agBody').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  async function rates(){shell('commission-rate-governance',canEdit()?'<div class="actions"><button class="btn" id="agRateCreate">Create rate draft</button><button class="mini" id="agSeed">Prepare Tir 14% / 20% pending evidence</button></div>':'');try{const [r,matrix]=await Promise.all([api('/api/accounting/profit-ledger/rates?pageSize=500'),api('/api/accounting/governance/rate-review?periodFrom=14050401&periodTo=14050431&pageSize=500')]);q('#agBody').innerHTML=`<div class="info">Blank or missing rate is never zero. Global (*) and seller overrides are versioned separately.</div><h4>Tir seller/category review matrix</h4>${table(['Seller / category','Tir scope','Proposed rate','Evidence / ambiguity','Status'],(matrix.list||[]).map(row=>`<tr><td>${safe(row.sellerIdentity)} ${safe(row.sellerName)}<br>${safe(row.commissionCategory)}</td><td>${safe(row.saleLineCount)} lines<br>${safe(row.saleValue)} IRR<br>${safe(row.mappingStatus)}</td><td>${safe(row.proposedRate??'MISSING')}<br>${safe(row.rateClassification)}</td><td>${safe(row.evidence)}<br>${safe(row.ambiguityNotes)}</td><td>${safe(row.rateStatus)}</td></tr>`))}<h4>Versioned rate history</h4>${table(['Version','Scope','Period / rate','Contract / evidence','Status','Workflow / audit'],(r.list||[]).map(row=>`<tr><td>${safe(row.rateVersionId)}</td><td>${safe(row.sellerIdentity)} / ${safe(row.commissionCategory)}</td><td>${safe(row.effectiveFrom)}–${safe(row.effectiveTo||'open')}<br><b>${safe(row.rate)}</b></td><td>${safe(row.contractType)}<br>${safe(row.sourceReference)}<br>${safe(row.reason)}</td><td>${safe(row.status)} r${safe(row.revision)}</td><td>${workflow(row,'rate')}<details><summary>Audit</summary><pre>${safe(JSON.stringify(row.auditLog||[],null,2))}</pre></details></td></tr>`))}`;bindActions(rates);if(q('#agRateCreate'))q('#agRateCreate').onclick=async()=>{const sellerIdentity=prompt('Seller identity or * for global:','*')||'';const commissionCategory=prompt('Category:','COMPONENT')||'';const rate=prompt('Exact decimal rate (e.g. 0.20):')||'';const sourceReference=prompt('Contract/workbook evidence reference:')||'';const reason=prompt('Reason:')||'';const evidenceUnavailableReason=prompt('Evidence unavailable reason (if no attachment):')||'';if(!sellerIdentity||rate===''||!sourceReference||!reason||!evidenceUnavailableReason)return;try{await api('/api/accounting/profit-ledger/rates',{method:'POST',body:JSON.stringify({sellerIdentity,commissionCategory,rate,effectiveFrom:'14050401',effectiveTo:'14050431',contractType:'human-review-candidate',sourceDocumentType:'accounting evidence',sourceReference,reason,evidenceMetadata:{evidenceUnavailableReason}})});await rates();}catch(error){alert(`${error.code||''} ${error.message}`);}};if(q('#agSeed'))q('#agSeed').onclick=async()=>{try{await api('/api/accounting/profit-ledger/rates/seed-tir',{method:'POST',body:'{}'});await rates();}catch(error){alert(`${error.code||''} ${error.message}`);}};}catch(error){q('#agBody').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  ...
  async function readiness(){shell('commission-export-readiness');try{const r=await api('/api/accounting/governance/readiness?periodFrom=14050401&periodTo=14050431');const m=r.metrics;q('#agBody').innerHTML=`<div class="${r.normalExportReady?'info':'warn'}"><b>Normal export: ${r.normalExportReady?'READY':'BLOCKED (HTTP 409)'}</b> — Profit / ROI / Commission remain disabled.</div><div class="row"><div class="info">Official group coverage<br><b>${safe(m.officialGroupSaleValuePercent)}% value</b><br>${safe(m.officialGroupLinePercent)}% lines</div><div class="info">Approved mapped value<br><b>${safe(m.mappedSaleValuePercent)}%</b><br>Pending ${safe(m.pendingMappedSaleValue)} IRR<br>Projected ${safe(m.projectedMappedSaleValuePercent)}%</div><div class="info">Approved mapped lines<br><b>${safe(m.mappedLinePercent)}%</b><br>Pending ${safe(m.pendingMappedLineCount)}<br>Projected ${safe(m.projectedMappedLinePercent)}%</div><div class="info">Unknown category<br><b>${safe(m.unknownCategoryValuePercent)}% value</b><br>${safe(m.unknownCategoryLinePercent)}% lines</div><div class="info">Approved rate coverage<br><b>${safe(m.approvedRateCoveragePercent)}%</b></div><div class="info">Cost coverage<br>Complete ${safe(m.completeCostValuePercent)}%<br>Partial ${safe(m.partialCostValuePercent)}%<br>Unknown ${safe(m.unknownCostValuePercent)}%</div><div class="info">Unresolved controls<br>Discount ${safe(m.unresolvedDiscountLineCount)}<br>Seller ${safe(m.unresolvedSellerLineCount)}</div></div><h4>Missing-rate sellers</h4><pre>${safe(JSON.stringify(r.missingRateSellers,null,2))}</pre><h4>Exceptional-rate candidates</h4><pre>${safe(JSON.stringify(r.exceptionalRateCandidates,null,2))}</pre><h4>Opening balances</h4><pre>${safe(JSON.stringify(r.openingReadiness,null,2))}</pre><h4>Machine-readable blockers</h4><pre>${safe(JSON.stringify(r.blockers,null,2))}</pre>`;}catch(error){q('#agBody').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
