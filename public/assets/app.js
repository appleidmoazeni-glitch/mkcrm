...
(()=>{
  ...
  async function pageRenderer(){
    ...
    try{await Promise.all([loadBuildContext(),loadRunSelector()]);if(!isCurrent())return;if(selectedReadRun()){await loadOptions();await refresh();}else q('#sfStatus').innerHTML='<div class="warn">Read Model تکمیل‌شده‌ای برای مشاهده وجود ندارد.</div>';}catch(error){if(isCurrent())q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}
  }
