...
(function(){
    ...
    try{
      ...
      document.querySelectorAll('.mc-resolve').forEach(button=>button.onclick=async()=>{
        ...
        try{
          ...
          const canFinalize=canDecide&&suggestion.reviewOnly!==true&&suggestion.financialAuthority!==false&&!['EXACT_OFFICIAL_PURCHASE_LAYER','SOURCE_HISTORY_INCOMPLETE','PENDING_PURCHASE_PRICE','PURCHASE_RETURN_CONFLICT','CONFLICT_REQUIRES_REVIEW'].includes(suggestion.sourceClass);
  ...
  async function loadOpeningCandidates(){
    ...
    try{
      ...
      box.innerHTML=`<div class="warn">شواهد پیش از Approval فقط برای Review و Impact Preview قابل مشاهده‌اند و هرگز مستقیماً FIFO را تغییر نمی‌دهند.</div><table class="table"><thead><tr><th>Dataset</th><th>تاریخ مبنا</th><th>استخراج</th><th>Approval</th><th>کالا</th><th>Source Calls</th><th>Fingerprint</th><th></th></tr></thead><tbody>${(report.list||[]).map(row=>`<tr><td><code>${safe(row.datasetId)}</code></td><td>${safe(row.openingDate)}</td><td>${safe(row.status)}</td><td><b>${safe(openingStatus(row.approvalStatus))}</b><br><small>Revision ${safe(row.revision||1)}</small></td><td>${number(row.completedItemCount||0)} / ${number(row.itemCount||0)}<br><small>Incomplete: ${number(row.incompleteItemCount||0)}</small></td><td>${number(row.sourceCallCount||0)}</td><td><code>${safe((row.datasetFingerprint||'').slice(0,16))}</code></td><td><button class="mini mc-opening-detail" data-id="${safe(row.datasetId)}">جزئیات</button> ${['incomplete','failed'].includes(row.status)&&['admin','accounting'].includes(userRole())?`<button class="mini mc-opening-action" data-action="resume" data-id="${safe(row.datasetId)}">Resume</button>`:''} ${row.approvalStatus==='validated'&&['admin','accounting'].includes(userRole())?`<button class="mini mc-opening-action" data-action="submit" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">ارسال برای تأیید</button>`:''} ${row.approvalStatus==='pending'&&['admin','manager'].includes(userRole())?`<button class="mini mc-opening-action" data-action="approve" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">تأیید مستقل</button> <button class="mini mc-opening-action" data-action="reject" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">رد</button> <button class="mini mc-opening-action" data-action="defer" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">تعویق</button>`:''}</td></tr>`).join('')||'<tr><td colspan="8">Dataset موجود نیست.</td></tr>'}</tbody></table>`;
      document.querySelectorAll('.mc-opening-detail').forEach(button=>button.onclick=async()=>{try{const detail=await json(`/api/accounting/opening-accounting-evidence/candidates/${encodeURIComponent(button.dataset.id)}?limit=5000`);q('#mcOpeningDetail').innerHTML=`<div class="info"><b>${safe(detail.dataset.datasetId)}</b> | ${safe(openingStatus(detail.dataset.approvalStatus))} | Evidence ${number(detail.rows.length)} | Progress ${number(detail.progress.length)} | Sale-line Preview ${number(detail.eligibility.length)}</div><div class="row four"><div>VALIDATED: <b>${number(detail.dataset.statusCounts?.VALIDATED_CANDIDATE||0)}</b></div><div>NO STOCK: <b>${number(detail.dataset.statusCounts?.NO_OPENING_STOCK||0)}</b></div><div>INCOMPLETE: <b>${number(detail.dataset.statusCounts?.INCOMPLETE_SOURCE||0)}</b></div><div>Fingerprint: <code>${safe(detail.dataset.datasetFingerprint||'')}</code></div><div>Preview: <b>${safe(detail.impactPreview?.authorityClass)}</b></div><div>Resolvable Lines: <b>${number(detail.impactPreview?.resolvableLines||0)}</b></div><div>Resolvable Qty: <b>${safe(detail.impactPreview?.resolvableQuantityExact||'0')}</b></div><div>Sale Exposure: <b>${safe(detail.impactPreview?.saleExposureExact||'0')} ریال</b></div></div><details open><summary>شواهد کالا</summary><table class="table"><thead><tr><th>کالا</th><th>وضعیت</th><th>Opening Qty</th><th>Unit Cost</th><th>Total</th><th>انبار</th></tr></thead><tbody>${(detail.rows||[]).map(row=>`<tr><td>${safe(row.itemCode)}<br><small>${safe(row.itemDescription)}</small></td><td>${safe(row.status)}</td><td>${safe(row.openingQuantityExact||'0')}</td><td>${safe(row.openingUnitCostExact||'0')}</td><td>${safe(row.openingTotalValueExact||'0')}</td><td>${number(row.queriedWarehouseCount||0)}</td></tr>`).join('')}</tbody></table></details><details><summary>Sale-line Eligibility / Impact Preview</summary><table class="table"><thead><tr><th>Invoice / Line</th><th>Date</th><th>Item</th><th>Unknown</th><th>Opening Eligible</th><th>Remaining</th><th>خرید قبلی / بعدی</th><th>Classification</th></tr></thead><tbody>${(detail.eligibility||[]).map(row=>`<tr><td>${safe(row.saleInvoiceNo)} / ${safe(row.saleRow)}</td><td>${safe(row.saleDate)}</td><td>${safe(row.itemCode)}</td><td>${safe(row.unknownQuantityExact)}</td><td>${safe(row.openingEligibleQuantityExact)}</td><td>${safe(row.remainingUnknownQuantityExact)}</td><td>${number(row.earlierOfficialPurchaseCount||0)} / ${number(row.laterPurchaseCount||0)}</td><td>${safe(row.classification)}</td></tr>`).join('')}</tbody></table></details><details><summary>Audit مستقل Approval</summary><pre>${safe(JSON.stringify(detail.approvalHistory||[],null,2))}</pre></details>`;}catch(error){q('#mcOpeningDetail').innerHTML=`<div class="error">${safe(error.message)}</div>`;}});
      document.querySelectorAll('.mc-opening-action').forEach(button=>button.onclick=async()=>{let reason='';if(['approve','reject','defer'].includes(button.dataset.action)){reason=prompt('دلیل تصمیم مستقل را ثبت کنید:')||'';if(!reason)return;}try{await json(`/api/accounting/opening-accounting-evidence/candidates/${encodeURIComponent(button.dataset.id)}/${button.dataset.action}`,{method:'POST',body:JSON.stringify({revision:Number(button.dataset.revision||0),reason})});await loadOpeningCandidates();}catch(error){q('#mcOpeningDetail').innerHTML=`<div class="error">${safe(error.message)}</div>`;}});
    }catch(error){box.innerHTML=`<div class="error">${safe(error.message)}</div>`;}
  }
