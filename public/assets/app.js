...
(()=>{
  ...
  function actions(row,type){if(row.historicalFrozen||row.status==='historical_frozen')return '<span class="muted">تاریخی و منجمد</span>';const base=type==='policy'?'/api/accounting/commission-policies':type==='mapping'?'/api/accounting/profit-ledger/categories':'/api/accounting/profit-ledger/rates';const id=recordId(row,type);if(!id)return '—';const buttons=[];const editableDraft=type==='mapping'?['draft','returned'].includes(row.status):row.status==='draft';if(EDIT.includes(userRole())&&editableDraft){buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="submit" data-revision="${safe(row.revision)}">ارسال برای تأیید</button>`);if(type==='mapping')buttons.push(`<button class="mini fg-transition" data-record-type="mapping" data-base="${base}" data-id="${safe(id)}" data-action="cancel" data-revision="${safe(row.revision)}">لغو پیش‌نویس</button>`);}if(APPROVE.includes(userRole())&&row.status==='pending'){buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="approve" data-revision="${safe(row.revision)}">تأیید مستقل</button>`);buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="reject" data-revision="${safe(row.revision)}">رد کردن</button>`);}if(APPROVE.includes(userRole())&&row.status==='approved'&&type==='policy')buttons.push(`<button class="mini fg-transition" data-record-type="policy" data-base="${base}" data-id="${safe(id)}" data-action="retire" data-revision="${safe(row.revision)}">بازنشسته‌کردن</button>`);return buttons.join(' ')||'—';}
  ...
  async function ratePage(){
    const editable=EDIT.includes(userRole());page('نرخ‌های پورسانت',`${editable?`<div class="card"><div class="card-header"><h5>نسخه نرخ جدید</h5></div><div class="card-body"><div class="info">ترتیب قطعی اعمال نرخ: فروشنده/دسته ← فروشنده/سبد نرخ ← پیش‌فرض دسته ← پیش‌فرض سبد نرخ. نرخ ناموجود هرگز صفر نیست.</div><div id="fgrPolicyState"><div class="muted">در حال دریافت سیاست‌های مصوب…</div></div><div class="row four"><div class="form-group"><label>سیاست مصوب</label><select id="fgrPolicy" disabled><option>در حال بارگذاری…</option></select></div><div class="form-group"><label>فروشنده (* برای پیش‌فرض)</label><input id="fgrSeller" value="*"></div><div class="form-group"><label>دامنه نرخ</label><select id="fgrScope"><option value="rate_pool">سبد نرخ</option><option value="product_category">دسته رسمی کالا</option></select></div><div class="form-group"><label>نرخ اعشاری</label><input id="fgrRate" placeholder="0.20"></div></div><div class="row four"><div class="form-group"><label>سبد نرخ</label><select id="fgrPool"><option>NOTEBOOK</option><option>COMPONENT</option><option>CUSTOM</option><option>NON_COMMISSIONABLE</option></select></div><div class="form-group"><label>شناسه دسته رسمی</label><input id="fgrCategoryId"></div><div class="form-group"><label>نام دسته رسمی</label><input id="fgrCategoryName"></div><div class="form-group"><label>شرح/مرجع (اختیاری)</label><input id="fgrDescription"></div></div><div class="row four"><div class="form-group"><label>از تاریخ</label><input id="fgrFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ</label><input id="fgrTo" value="14050431"></div><div class="form-group"><label>&nbsp;</label><button class="btn" id="fgrCreate" disabled>ایجاد پیش‌نویس</button><div class="small muted">تا انتخاب یک سیاست مصوب، ثبت غیرفعال است.</div></div></div></div></div>`:''}<div class="card"><div class="card-header"><h5>نسخه‌های نرخ</h5></div><div class="card-body">${reasonBox()}<div id="fgrList">در حال بارگذاری…</div></div></div>`);
    let policies;
    try{policies=await api('/api/accounting/commission-policies');configurePolicySelector({selectId:'fgrPolicy',createId:'fgrCreate',stateId:'fgrPolicyState',policies,refresh:ratePage});}
    catch(error){policyLoadError('fgrPolicyState',error,ratePage);return;}if(!q('#fgrList'))return;
    try{
      const rates=await api('/api/accounting/profit-ledger/rates?pageSize=500');
      const list=q('#fgrList');if(!list)return;list.innerHTML=`<div class="info">این جدول فقط نسخه‌های نرخ ذخیره‌شده را نمایش می‌دهد؛ Candidate محاسباتی هرگز پیش‌نویس تلقی نمی‌شود.</div><table class="table"><thead><tr><th>فروشنده / دامنه</th><th>دسته / سبد نرخ</th><th>نرخ</th><th>سیاست</th><th>وضعیت</th><th>اقدام</th></tr></thead><tbody>${(rates.list||[]).map(row=>`<tr data-row-type="persisted" data-rate-version-id="${safe(row.rateVersionId)}"><td>${safe(row.sellerIdentity)} / ${safe(scopeFa(row.rateScope))}</td><td>${safe(row.officialProductCategoryName||row.commissionRatePool)}</td><td>${safe(row.rate)}</td><td><details><summary>سیاست ثبت‌شده</summary>${safe(row.policyVersionId||'—')}</details></td><td>${safe(statusFa(row.status))} / بازبینی ${safe(row.revision)}<details><summary>شناسه نسخه</summary>${safe(row.rateVersionId)}</details></td><td>${actions(row,'rate')}</td></tr>`).join('')||'<tr><td colspan="6">رکورد ذخیره‌شده‌ای وجود ندارد.</td></tr>'}</tbody></table>`;
      bindTransitions(ratePage);
      if(q('#fgrCreate'))q('#fgrCreate').onclick=async()=>{const policyVersionId=q('#fgrPolicy').value,scope=q('#fgrScope').value;if(!policyVersionId){message('ابتدا یک سیاست پورسانت مصوب را انتخاب کنید.',true);return;}try{await api('/api/accounting/profit-ledger/rates',{method:'POST',body:JSON.stringify({policyVersionId,sellerIdentity:q('#fgrSeller').value,rateScope:scope,commissionRatePool:scope==='rate_pool'?q('#fgrPool').value:'',officialProductCategoryIdentity:scope==='product_category'?q('#fgrCategoryId').value:'',officialProductCategoryName:scope==='product_category'?q('#fgrCategoryName').value:'',rate:q('#fgrRate').value,effectiveFrom:q('#fgrFrom').value,effectiveTo:q('#fgrTo').value,reason:q('#fgrDescription').value})});await ratePage();}catch(error){message(`${error.code} ${error.message}`,true);}};
    }catch(error){const list=q('#fgrList');if(!list)return;list.innerHTML=`<div class="error">دریافت نسخه‌های نرخ ناموفق بود: ${safe(error.code||'')} ${safe(error.message)} <button class="btn" id="fgrRetry">تلاش دوباره</button></div>`;q('#fgrRetry').onclick=ratePage;}
  }
