...
(()=>{
  ...
  async function loadTotals(){const r=await api('/api/accounting/seller-financial-performance/totals?'+params());q('#sfCards').innerHTML=[card('فاکتور',number(r.invoiceCount)),card('ردیف',number(r.lineCount),`${number(r.unavailableLineCount)} پورسانت ناموجود`),card('فروش ناخالص',toman(r.saleValueExact),'تومان'),card('سود واقعی FIFO',toman(r.actualFifoProfitExact),'تومان؛ immutable'),card('سود پورسانتی',toman(r.commissionableProfitExact),'تومان'),card('پورسانت مقدماتی',toman(r.preliminaryCommissionExact),'تومان؛ غیرقابل پرداخت'),card('هزینه نامشخص',number(r.unknownCostLineCount),'ردیف')].join('');}
