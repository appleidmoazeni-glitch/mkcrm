...
(function(){
  const typeLabels={2:'فاکتور فروش',3:'فاکتور خرید',6:'برگشت از فروش',7:'برگشت از خرید'};
  const docTypeLabel=t=>typeLabels[Number(t)]||`نوع سند پشتیبانی‌نشده (${String(t||'نامشخص')})`;
  const invoiceNoOf=x=>String(x.InvNo??x.InvoiceNumber??'').trim();
  const invoiceTypeOf=x=>Number(x.InvTyp??x.InvoiceType??0)||0;
  ...
  window.openTypedTurnoverDocument=async function(identity,trigger=document.activeElement){
    const invNo=String(identity?.invNo||''),invType=Number(identity?.invType||0);
    ...
    modal.innerHTML=`<div class="mk-modal-backdrop"></div><div class="mk-modal-card"><div class="mk-modal-head"><h3>${esc(docTypeLabel(invType))} شماره ${esc(invNo)}</h3><button class="mini" id="closeTurnoverDocument">بستن</button></div><div id="turnoverDocumentBody"><div class="info">در حال خواندن سند...</div></div></div>`;
    ...
    if(!invNo||!typeLabels[invType]){$('#turnoverDocumentBody').innerHTML='<div class="error">نوع و شماره معتبر سند برای بازکردن لازم است؛ سند دیگری به‌عنوان fallback باز نشد.</div>';return;}
    try{const r=await api(`/api/invoices/${encodeURIComponent(invNo)}?invType=${invType}`);const inv=(r.list||[]).find(x=>invoiceNoOf(x)===invNo&&invoiceTypeOf(x)===invType);if(!inv){$('#turnoverDocumentBody').innerHTML=`<div class="warn">${esc(docTypeLabel(invType))} شماره ${esc(invNo)} پیدا نشد.</div>`;return;}const lines=invoiceBody(inv);$('#turnoverDocumentBody').innerHTML=`<div class="info">تاریخ: ${esc(toJalaliDisplay(inv.InvDate||''))} | حساب: ${esc(inv.AccountName||'')} | InvTyp=${invType}</div><div class="table-scroll"><table class="table"><thead><tr><th>کد کالا</th><th>نام</th><th>تعداد</th><th>قیمت</th>${invType===2?'<th>کاردکس</th>':''}</tr></thead><tbody>${lines.map(x=>{const code=x.ItemNumber||x.ItemCode||'';return `<tr><td>${esc(code)}</td><td>${esc(x.ItemDescription||x.ItemDesc||'')}</td><td>${fmt(x.Quan||x.Quantity||0)}</td><td>${fmt(x.Price||0)}</td>${invType===2?`<td><button class="mini invoice-item-kardex" data-code="${esc(code)}">مشاهده کاردکس</button></td>`:''}</tr>`}).join('')}</tbody></table></div><div id="invoiceKardexPanel"></div>`;modal.querySelectorAll('.invoice-item-kardex').forEach(b=>b.onclick=async()=>{const panel=$('#invoiceKardexPanel'),code=b.dataset.code;panel.innerHTML='<div class="info">در حال خواندن کاردکس...</div>';try{const kr=await api(`/api/invoices/${encodeURIComponent(invNo)}/items/${encodeURIComponent(code)}/kardex?invType=2`);if(!kr.ok){panel.innerHTML=`<div class="error">${esc(kr.error||'خطا')}</div>`;return;}panel.innerHTML=`<h4>کاردکس ${esc(code)}</h4>${(kr.rows||[]).length?`<table class="table"><thead><tr><th>تاریخ</th><th>نوع</th><th>سند</th><th>ورود</th><th>خروج</th><th>مانده</th><th>خرید/تمام‌شده</th><th>فروش</th></tr></thead><tbody>${kr.rows.map(x=>`<tr><td>${esc(toJalaliDisplay(x.date||''))}</td><td>${esc(docTypeLabel(x.invoiceType))}</td><td>${esc(x.invoiceNumber||'')}</td><td>${fmt(x.inQty)}</td><td>${fmt(x.outQty)}</td><td>${fmt(x.remainQty)}</td><td>${fmt(x.costPrice)}</td><td>${fmt(x.salePrice)}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">ردیفی برای این کالا وجود ندارد.</div>'}`;}catch(e){panel.innerHTML=`<div class="error">${esc(e.message||e)}</div>`;}});}
  ...
  window.pageTurnover=async function(){
    ...
    const access=await api('/api/account-access/my'),input=$('#turnoverAccountSearch'),box=$('#turnoverAccountResults'),allowed=$('#allowedAccountsBox');let selected=null,suggestions=[],searchSeq=0,turnoverSeq=0,searchAbort=null,turnoverAbort=null;
    ...
    function selectAccount(a,source='search'){search.cancel();invalidateSearch();selected={accountNumber:String(a.accountNumber||''),accountName:a.accountName||'',moinNumber:a.moinNumber||'',accountGuid:accountGuid(a),source};input.value=accountLabel(selected);$('#turnoverPicked').value=accountLabel(selected);closeSearch();box.innerHTML='';}
