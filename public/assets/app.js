...
(()=>{
  ...
  async function drill(line){const box=q('#sfDrill'),runId=selectedReadRun();if(!box)return;box.innerHTML='<div class="muted">در حال بارگذاری زنجیره حسابرسی…</div>';try{const r=await api(runScopedUrl(`/api/accounting/seller-financial-performance/lines/${encodeURIComponent(line)}/drilldown`));if(!isCurrent()||runId!==selectedReadRun())return;box.innerHTML=`<div class="actions"><button class="mini" id="sfCloseDrill">بستن (Esc)</button></div><div class="info">Line ${safe(r.line.saleLineIdentity)} — Run ${safe(r.runId)}<br>FIFO ${safe(r.line.fifoDatasetId)} | Snapshot ${safe(r.line.saleSnapshotId)} | Policy ${safe(r.line.policyVersionId||'ناموجود')} | Mapping ${safe(r.line.categoryMappingId||'ناموجود')} | Rate ${safe(r.line.rateVersionId||'ناموجود')}<br>Fingerprint ${safe(r.line.lineFingerprint)}</div><h5>FIFO allocations</h5><pre>${safe(JSON.stringify(r.source.allocations||[],null,2))}</pre><h5>Manual Cost Evidence</h5><pre>${safe(JSON.stringify(r.source.manualCostEvidence||[],null,2))}</pre><h5>Approved/Pending adjustments</h5><pre>${safe(JSON.stringify(r.source.adjustments||[],null,2))}</pre><h5>Saved-profit ledger</h5><pre>${safe(JSON.stringify(r.source.savedEntries||[],null,2))}</pre><h5>Invoice discount fact</h5><pre>${safe(JSON.stringify(r.source.discountFact||null,null,2))}</pre>`;q('#sfCloseDrill').onclick=closeDrill;}catch(error){if(isCurrent())box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
