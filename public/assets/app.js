...
(function(){
  ...
  window.pageSellerProfit = pageSellerProfit = async function(){
    ...
    try{ const sellers=await api('/api/reports/seller-sales-profit/sellers'); const sel=$('#spSeller'); const seen=new Set(); (sellers.list||[]).forEach(x=>{const key=String(x.sellerKey||x.employeeAccountNumber||'').trim();if(!key||seen.has(key))return;seen.add(key);sel.add(new Option(`${x.fullName||x.employeeAccountName||key} - ${x.employeeAccountNumber||key}`,key));}); }catch{}
