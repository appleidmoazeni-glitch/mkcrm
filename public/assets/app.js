...
/* One read-only freshness presentation shared by FIFO-derived consumers. */
window.renderCanonicalFifoFreshness=async function(selector,consumerFifoDatasetId=''){
  const box=document.querySelector(selector);if(!box)return;
  try{
    const response=await fetch('/api/accounting/fifo-shadow/freshness?consumerFifoDatasetId='+encodeURIComponent(consumerFifoDatasetId),{credentials:'include',cache:'no-store',headers:{Accept:'application/json'}});
    const value=await response.json();if(!response.ok||value.ok===false)throw new Error(value.code||value.error||response.statusText);
    const stale=value.sourceFreshnessStatus==='STALE'||value.sellerFinancialStale;
    box.innerHTML=`<div class="${stale?'warn':'success'}"><div class="row three"><div><small>آخرین به‌روزرسانی FIFO</small><br><b>${esc(value.lastFifoUpdatedAt?toJalaliDisplay(value.lastFifoUpdatedAt):'—')}</b></div><div><small>محاسبات FIFO تا تاریخ</small><br><b>${esc(value.calculationCutoff||'—')}</b></div><div><small>وضعیت FIFO</small><br><b>${esc(value.sellerFinancialMessage||value.fifoStatus||'نامشخص')}</b></div></div></div>`;
  }catch(error){box.innerHTML=`<div class="warn">وضعیت تازگی FIFO قابل دریافت نیست: ${esc(error.message||error)}</div>`;}
};

/* Candidate Seller Financial consumer — same #seller-profit route, no FIFO or
   payroll activation. Kept last so historical registries cannot replace it. */
(()=>{
  const PAGE='seller-profit',ALLOWED=['admin','accounting','manager','purchase'];
  const safe=value=>esc(value==null?'':String(value));
  const money=value=>value==null?'ناموجود':Number(value).toLocaleString('fa-IR',{maximumFractionDigits:2});
  const api=async(url,options={})=>{const response=await fetch(url,{credentials:'include',cache:'no-store',headers:{Accept:'application/json','Content-Type':'application/json'},...options});const body=await response.json().catch(()=>({ok:false,error:'پاسخ نامعتبر'}));if(!response.ok||body.ok===false){const error=new Error(body.error||response.statusText);error.code=body.code||'';throw error;}return body;};
  let generation=0,drillGeneration=0,pageNumber=1;
  const q=id=>document.querySelector(id),multi=id=>[...(q(id)?.selectedOptions||[])].map(row=>row.value).filter(Boolean).join(',');
  async function renderFifoFreshness(sourceFifoDatasetId=''){
    const box=q('#csfFifoFreshness');if(!box)return;
    try{const value=await api('/api/accounting/fifo-shadow/freshness?consumerFifoDatasetId='+encodeURIComponent(sourceFifoDatasetId));const stale=value.sellerFinancialStale||value.sourceFreshnessStatus==='STALE';box.innerHTML=`<div class="${stale?'warn':'success'}"><div class="row three"><div><small>آخرین به‌روزرسانی FIFO</small><br><b>${safe(value.lastFifoUpdatedAt?toJalaliDisplay(value.lastFifoUpdatedAt):'—')}</b></div><div><small>محاسبات FIFO تا تاریخ</small><br><b>${safe(value.calculationCutoff||'—')}</b></div><div><small>وضعیت FIFO</small><br><b>${safe(value.sellerFinancialMessage||value.fifoStatus||'نامشخص')}</b></div></div></div>`;}catch(error){box.innerHTML=`<div class="warn">وضعیت تازگی FIFO قابل دریافت نیست: ${safe(error.code||error.message)}</div>`;}
  }
  function query(){const p=new URLSearchParams({runId:q('#csfRun')?.value||'',page:String(pageNumber),pageSize:'50',dateFrom:q('#csfFrom')?.value||'',dateTo:q('#csfTo')?.value||'',sellerIdentity:q('#csfSeller')?.value||'',categoryGuid:q('#csfCategory')?.value||'',itemSearch:q('#csfItem')?.value||'',invoiceNumber:q('#csfInvoice')?.value||'',provenanceStatus:multi('#csfProvenance'),supplier:q('#csfSupplier')?.value||'',lineAmountMin:q('#csfSaleMin')?.value||'',lineAmountMax:q('#csfSaleMax')?.value||'',fifoProfitMin:q('#csfProfitMin')?.value||'',fifoProfitMax:q('#csfProfitMax')?.value||'',marginMin:q('#csfMarginMin')?.value||'',marginMax:q('#csfMarginMax')?.value||''});for(const[key,value]of[...p])if(!value)p.delete(key);return p;}
  const rowHtml=row=>`<tr><td>${row.saleInvoiceType===6?'<b>برگشت از فروش</b>':'فروش'}<br>${safe(row.saleDate)} / ${safe(row.saleInvoiceNumber)}</td><td>${safe(row.sellerName||row.sellerIdentity)}</td><td>${safe(row.itemCode)}<br><small>${safe(row.itemDescription)}</small></td><td>${safe(row.officialProductCategoryName)}<br><small>${safe(row.commissionRatePool)}</small></td><td>${safe(row.quantityExact)}</td><td>${money(row.grossSaleAmountExact)}</td><td>${money(row.fifoCostExact)}</td><td>${money(row.actualFifoProfitExact)}<br><small>${safe(row.fifoMarginExact??'—')}</small></td><td>${safe(row.profitProvenanceStatus)}<br><small>${safe(row.costSourceType)}</small></td><td><button class="mini csfDrill" data-line="${safe(row.saleLineIdentity)}">زنجیره مأخذ</button></td></tr>`;
  async function drill(line){const current=++drillGeneration,box=q('#csfDrill'),runId=q('#csfRun').value;box.innerHTML='<div class="muted">در حال خواندن مأخذ هزینه…</div>';try{const data=await api(`/api/accounting/seller-financial-performance/lines/${encodeURIComponent(line)}/drilldown?runId=${encodeURIComponent(runId)}`);if(current!==drillGeneration||location.hash.slice(1)!==PAGE)return;const r=data.line,ret=r.returnLinkage;box.innerHTML=`<div class="card"><div class="card-header"><h5>زنجیره حسابرسی ${safe(r.saleLineIdentity)}</h5></div><div class="card-body"><div class="info">Candidate ${safe(data.runId)} | FIFO ${safe(r.fifoDatasetId)} | Snapshot ${safe(r.saleSnapshotId)}<br>وضعیت: ${safe(r.profitProvenanceStatus)} | هزینه: ${money(r.fifoCostExact)} | سود: ${money(r.actualFifoProfitExact)}</div>${ret?`<h5>پیوند برگشت از فروش</h5><div class="success">فروش مبدأ ${safe(ret.originSaleInvoiceNumber)} / ردیف ${safe(ret.originSaleLineIdentity)} ← برگشت ${safe(r.saleInvoiceNumber)}<br>تعداد برگشتی ${safe(ret.returnedQuantityExact)} | هزینه معکوس ${money(ret.reversedCostExact)} | Allocation بازیابی‌شده ${safe(ret.restoredAllocationId)}</div>`:''}<h5>مأخذ خرید و تخصیص FIFO</h5><div style="overflow:auto"><table class="table"><thead><tr><th>فاکتور خرید</th><th>تأمین‌کننده</th><th>تعداد</th><th>هزینه</th><th>نوع مأخذ</th></tr></thead><tbody>${(data.source.costProvenance||[]).map(source=>`<tr><td>${safe(source.purchaseInvoiceNumber||'—')}</td><td>${safe(source.supplierName||source.supplierIdentity||'—')}</td><td>${safe(source.allocatedQty)}</td><td>${money(source.allocatedCostExact)}</td><td>${safe(source.sourceType)}</td></tr>`).join('')}</tbody></table></div><details><summary>جزئیات فنی و Fingerprint</summary><pre>${safe(JSON.stringify({lineFingerprint:r.lineFingerprint,provenanceFingerprint:r.provenanceFingerprint,allocations:data.source.allocations},null,2))}</pre></details></div></div>`;}catch(error){if(current===drillGeneration)box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  async function refresh(){const current=++generation,p=query().toString();q('#csfResult').innerHTML='<div class="muted">در حال خواندن Candidate Read Model…</div>';try{const [totals,lines,categories]=await Promise.all([api('/api/accounting/seller-financial-performance/totals?'+p),api('/api/accounting/seller-financial-performance/lines?'+p),api('/api/accounting/seller-financial-performance/category-totals?'+p)]);if(current!==generation||location.hash.slice(1)!==PAGE)return;q('#csfCards').innerHTML=[['فروش ناخالص',totals.grossSalesExact],['برگشت از فروش',totals.saleReturnEffectExact],['فروش خالص مالی',totals.financialNetSalesExact],['هزینه FIFO اثبات‌شده',totals.knownFifoCostExact],['سود FIFO اثبات‌شده',totals.provenFifoProfitExact],['پوشش مبلغ فروش',`${totals.provenProfitCoveragePercent}%`],['مبلغ PARTIAL',totals.partialExposureSaleValueExact],['مبلغ UNKNOWN',totals.unknownExposureSaleValueExact]].map(([label,value])=>`<div class="info"><b>${safe(label)}</b><br>${safe(String(value??'ناموجود'))}</div>`).join('');q('#csfResult').innerHTML=`<div class="info">${safe(lines.total)} ردیف — Candidate، غیرفعال و غیرقابل پرداخت</div><div style="overflow:auto"><table class="table"><thead><tr><th>فروش/برگشت</th><th>فروشنده</th><th>کالا</th><th>Product Category</th><th>تعداد</th><th>فروش/اثر برگشت</th><th>هزینه FIFO</th><th>سود/Margin</th><th>مأخذ</th><th></th></tr></thead><tbody>${(lines.list||[]).map(rowHtml).join('')||'<tr><td colspan="10">نتیجه‌ای وجود ندارد.</td></tr>'}</tbody></table></div><div class="actions"><button class="mini" id="csfPrev">قبلی</button><span>صفحه ${pageNumber} / ${safe(lines.total)} ردیف</span><button class="mini" id="csfNext">بعدی</button></div><details><summary>تجمیع Product Category در همین Scope</summary><table class="table"><thead><tr><th>فروشنده</th><th>دسته رسمی</th><th>فروش ناخالص</th><th>برگشت</th><th>فروش خالص</th><th>سود اثبات‌شده</th><th>UNKNOWN/PARTIAL</th></tr></thead><tbody>${(categories.list||[]).map(row=>`<tr><td>${safe(row.sellerName||row.sellerIdentity)}</td><td>${safe(row.officialProductCategoryName)}</td><td>${money(row.grossSalesExact)}</td><td>${money(row.saleReturnValueExact)}</td><td>${money(row.financialNetSalesExact)}</td><td>${money(row.provenFifoProfitExact)}</td><td>${money(row.unknownOrPartialSaleValueExact)}</td></tr>`).join('')}</tbody></table></details>`;q('#csfPrev').onclick=()=>{pageNumber=Math.max(1,pageNumber-1);refresh();};q('#csfNext').onclick=()=>{pageNumber++;refresh();};q('#csfResult').querySelectorAll('.csfDrill').forEach(button=>button.onclick=()=>drill(button.dataset.line));}catch(error){if(current===generation)q('#csfResult').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  async function render(){if(!ALLOWED.includes(userRole()))return setPage('عدم دسترسی','<main class="main-content"><div class="error">دسترسی مجاز نیست.</div></main>');setPage('عملکرد مالی فروشندگان',`<main class="main-content"><div id="csfFifoFreshness"><div class="info">در حال بررسی تازگی FIFO…</div></div><div class="warn"><b>CANDIDATE / NON-ACTIVE / NON-PAYROLL</b><br>این صفحه فقط مصرف‌کننده Candidate FIFO است؛ Profit نامشخص صفر نیست و هیچ پورسانت قابل پرداختی تولید نمی‌شود.</div><div class="actions"><select id="csfRun"></select><button class="btn" id="csfApply">اعمال فیلتر</button><span class="muted">ساخت فقط از مسیر canonical و Active FIFO مجاز است.</span></div><div class="card"><div class="card-body"><div class="row four"><div class="form-group"><label>از تاریخ شمسی</label><input id="csfFrom"></div><div class="form-group"><label>تا تاریخ شمسی</label><input id="csfTo"></div><div class="form-group"><label>فروشنده</label><select id="csfSeller"><option value="">همه فروشندگان</option></select></div><div class="form-group"><label>Product Category رسمی</label><select id="csfCategory"><option value="">همه دسته‌ها</option></select></div></div><div class="row four"><div class="form-group"><label>کالا</label><input id="csfItem"></div><div class="form-group"><label>شماره فاکتور</label><input id="csfInvoice"></div><div class="form-group"><label>Provenance</label><select id="csfProvenance" multiple><option>PROVEN</option><option>PARTIAL</option><option>UNKNOWN</option></select></div><div class="form-group"><label>تأمین‌کننده</label><input id="csfSupplier"></div></div><details><summary>فیلتر مبلغ و Margin</summary><div class="row three"><div><input id="csfSaleMin" placeholder="فروش حداقل ریال"><input id="csfSaleMax" placeholder="فروش حداکثر ریال"></div><div><input id="csfProfitMin" placeholder="سود حداقل ریال"><input id="csfProfitMax" placeholder="سود حداکثر ریال"></div><div><input id="csfMarginMin" placeholder="Margin حداقل"><input id="csfMarginMax" placeholder="Margin حداکثر"></div></div></details></div></div><div id="csfCards" style="display:flex;gap:8px;flex-wrap:wrap"></div><div id="csfResult"></div><div id="csfDrill"></div></main>`);try{const runs=await api('/api/accounting/seller-financial-performance/runs?pageSize=50');const candidates=(runs.list||[]).filter(row=>row.status==='completed'&&row.activationStatus==='validated-candidate');q('#csfRun').innerHTML=candidates.map(row=>`<option value="${safe(row.runId)}">${safe(row.runId)} — FIFO ${safe(row.sourceFifoDatasetId)}</option>`).join('');if(!candidates.length){await renderFifoFreshness('');q('#csfResult').innerHTML='<div class="warn">Seller Financial Candidate هنوز ساخته نشده است.</div>';return;}const categoryOptions=rows=>(rows||[]).map(row=>`<option value="${safe(row.guid)}">${safe(row.label||row.name||row.guid)}</option>`).join('');const selectedRun=()=>candidates.find(row=>row.runId===q('#csfRun').value);const options=await api('/api/accounting/seller-financial-performance/filters?runId='+encodeURIComponent(q('#csfRun').value));q('#csfSeller').innerHTML='<option value="">همه فروشندگان</option>'+[...(options.sellers||[])].map(row=>`<option value="${safe(row.identity)}">${safe(row.name||row.identity)}</option>`).join('');q('#csfCategory').innerHTML='<option value="">همه دسته‌ها</option>'+categoryOptions(options.categories);q('#csfApply').onclick=()=>{pageNumber=1;refresh();};q('#csfRun').onchange=async()=>{generation++;pageNumber=1;const selected=q('#csfRun').value,options=await api('/api/accounting/seller-financial-performance/filters?runId='+encodeURIComponent(selected));q('#csfSeller').innerHTML='<option value="">همه فروشندگان</option>'+[...(options.sellers||[])].map(row=>`<option value="${safe(row.identity)}">${safe(row.name||row.identity)}</option>`).join('');q('#csfCategory').innerHTML='<option value="">همه دسته‌ها</option>'+categoryOptions(options.categories);await renderFifoFreshness(selectedRun()?.sourceFifoDatasetId||'');await refresh();};await renderFifoFreshness(selectedRun()?.sourceFifoDatasetId||'');await refresh();}catch(error){q('#csfResult').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}}
  const clearCandidateDrill=()=>{drillGeneration++;const panel=q('#csfDrill');if(panel)panel.innerHTML='';};
  document.addEventListener('click',event=>{if(event.target?.id==='csfApply')clearCandidateDrill();},true);
  document.addEventListener('change',event=>{if(event.target?.id==='csfRun')clearCandidateDrill();},true);
  window.__candidateSellerFinancialPage=render;window.pageSellerProfit=render;
  const inheritedRoute=window.route||route;window.route=route=async function(){if((location.hash.slice(1)||firstAllowedPage())===PAGE)return render();return inheritedRoute.apply(this,arguments);};
})();

/* Phase B financial-governance page definitions. Final registration is kept
   at the end of this asset so legacy wrappers cannot duplicate the menu. */
(()=>{
  const ALLOWED=['admin','accounting','manager'];
  const EDIT=['admin','accounting'];
  const APPROVE=['admin','manager'];
  const NAV={
    'commission-policy-governance':'سیاست‌های پورسانت',
    'commission-category-governance':'تعریف پورسانت گروه کالا',
    'commission-rate-governance':'نرخ‌های پورسانت',
    'manual-cost-resolution':'رفع هزینه خرید نامشخص',
    'accounting-fifo-readiness':'آمادگی حسابداری FIFO',
    'accounting-review-workbench':'کاربرگ بررسی حسابداری',
    'accounting-fat':'پذیرش نهایی حسابداری'
  };
  const q=s=>document.querySelector(s);
  const safe=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  async function api(url,options={}){const response=await fetch(url,{credentials:'include',headers:{'Content-Type':'application/json'},...options});const payload=await response.json().catch(()=>({ok:false,error:'پاسخ نامعتبر'}));if(!response.ok||payload.ok===false){const error=new Error(payload.error||response.statusText);error.code=payload.code||'';error.status=response.status;error.details=payload.details||null;throw error;}return payload;}
  function page(title,body){setPage(title,`<main class="main-content"><div class="warn">سود FIFO تغییرناپذیر است؛ این صفحات فقط سیاست، دسته و نرخ پورسانت را مدیریت می‌کنند و هیچ تأیید خودکاری ندارند.</div>${body}<div id="fgMessage"></div></main>`);}
  function message(text,error=false,code=''){const box=q('#fgMessage');if(box)box.innerHTML=`<div class="${error?'error':'success'}"${code?` data-error-code="${safe(code)}"`:''}>${safe(text)}</div>`;}
  function recordId(row,type){return type==='policy'?row.policyVersionId:type==='mapping'?row.mappingId:type==='rate'?row.rateVersionId:'';}
  function actions(row,type){if(row.historicalFrozen||row.status==='historical_frozen')return '<span class="muted">تاریخی و منجمد</span>';const base=type==='policy'?'/api/accounting/commission-policies':type==='mapping'?'/api/accounting/profit-ledger/categories':'/api/accounting/profit-ledger/rates';const id=recordId(row,type);if(!id)return '—';const buttons=[];const editableDraft=['mapping','rate'].includes(type)?['draft','returned'].includes(row.status):row.status==='draft';if(EDIT.includes(userRole())&&editableDraft){buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="submit" data-revision="${safe(row.revision)}">ارسال برای تأیید</button>`);if(['mapping','rate'].includes(type))buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="cancel" data-revision="${safe(row.revision)}">لغو پیش‌نویس</button>`);}if(APPROVE.includes(userRole())&&row.status==='pending'){buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="approve" data-revision="${safe(row.revision)}">تأیید مستقل</button>`);buttons.push(`<button class="mini fg-transition" data-record-type="${safe(type)}" data-base="${base}" data-id="${safe(id)}" data-action="reject" data-revision="${safe(row.revision)}">رد کردن</button>`);}if(APPROVE.includes(userRole())&&row.status==='approved'&&type==='policy')buttons.push(`<button class="mini fg-transition" data-record-type="policy" data-base="${base}" data-id="${safe(id)}" data-action="retire" data-revision="${safe(row.revision)}">بازنشسته‌کردن</button>`);return buttons.join(' ')||'—';}
  function bindTransitions(refresh){document.querySelectorAll('.fg-transition').forEach(button=>button.onclick=async()=>{try{await api(`${button.dataset.base}/${encodeURIComponent(button.dataset.id)}/${button.dataset.action}`,{method:'POST',body:JSON.stringify({revision:Number(button.dataset.revision),reason:q('#fgActionReason')?.value||''})});await refresh();}catch(error){if(error.code==='CATEGORY_MAPPING_NOT_FOUND'){await refresh();message('رکورد نگاشت پیدا نشد. وضعیت این گروه به‌روزرسانی شد؛ ابتدا پیش‌نویس را ایجاد کنید. کد فنی: CATEGORY_MAPPING_NOT_FOUND',true,error.code);return;}if(error.code==='RATE_NOT_FOUND'){await refresh();message('نسخه نرخ پیدا نشد. وضعیت صفحه به‌روزرسانی شد؛ ابتدا پیش‌نویس نرخ را ایجاد کنید. کد فنی: RATE_NOT_FOUND',true,error.code);return;}message(`${error.code} ${error.message}`,true,error.code);}});}
  function reasonBox(){return '<div class="form-group"><label>یادداشت اقدام (اختیاری)</label><input id="fgActionReason" maxlength="1000"></div>';}
  function statusFa(status){return({unregistered:'ثبت‌نشده',draft:'پیش‌نویس',pending:'در انتظار تأیید',approved:'مصوب',rejected:'ردشده',cancelled:'لغوشده',retired:'بازنشسته',historical_frozen:'تاریخی و منجمد'})[status]||status||'نامشخص';}
  function scopeFa(scope){return({rate_pool:'سبد نرخ',product_category:'دسته رسمی کالا'})[scope]||scope||'نامشخص';}
  function selectablePolicies(result){return(result?.selectable||[]).filter(row=>row.status==='approved'&&row.policyVersionId!=='LEGACY_PRE_POLICY'&&!row.historicalFrozen);}
  function configurePolicySelector({selectId,createId,stateId,policies,refresh}){
    const select=q(`#${selectId}`),create=q(`#${createId}`),box=q(`#${stateId}`);if(!select||!box)return;
    const rows=selectablePolicies(policies);
    select.innerHTML=`<option value="">انتخاب سیاست مصوب</option>${rows.map(row=>`<option value="${safe(row.policyVersionId)}">${safe(row.name)} — دوره ${safe(row.accountingPeriod)}</option>`).join('')}`;
    select.disabled=!rows.length;if(create)create.disabled=true;
    const render=()=>{
      const selected=rows.find(row=>row.policyVersionId===select.value);
      if(!rows.length)box.innerHTML='<div class="warn"><b>هیچ سیاست پورسانت مصوبی وجود ندارد.</b><br>ابتدا یک سیاست پورسانت ایجاد و پس از تأیید مدیر، آن را انتخاب کنید.<div class="actions"><button class="btn fg-open-policy">ایجاد سیاست پورسانت</button><button class="btn fg-refresh-policy">بازخوانی سیاست‌ها</button></div></div>';
      else box.innerHTML=`<div class="${selected?'success':'info'}">${selected?`سیاست منتخب: <b>${safe(selected.name)}</b> — ${safe(selected.effectiveFrom)} تا ${safe(selected.effectiveTo||'باز')}`:`${rows.length.toLocaleString('fa-IR')} سیاست مصوب قابل انتخاب است.`}<div class="actions"><button class="mini fg-refresh-policy">بازخوانی سیاست‌ها</button></div></div>`;
      if(create)create.disabled=!selected;
      box.querySelectorAll('.fg-open-policy').forEach(button=>button.onclick=()=>{location.hash='commission-policy-governance';route();});
      box.querySelectorAll('.fg-refresh-policy').forEach(button=>button.onclick=refresh);
    };
    select.onchange=render;render();
  }
  function policyLoadError(stateId,error,retry){const box=q(`#${stateId}`);if(!box)return;box.innerHTML=`<div class="error">فهرست سیاست‌های مصوب دریافت نشد: ${safe(error.code||'')} ${safe(error.message)}<div class="actions"><button class="btn fg-policy-retry">تلاش دوباره</button></div></div>`;box.querySelector('.fg-policy-retry').onclick=retry;}
  async function policyPage(){
    const editable=EDIT.includes(userRole());page('سیاست‌های پورسانت',`${editable?`<div class="card"><div class="card-header"><h5>سیاست جدید</h5></div><div class="card-body"><div class="row four"><div class="form-group"><label>نام سیاست</label><input id="fgpName"></div><div class="form-group"><label>دوره شمسی</label><input id="fgpPeriod" placeholder="140504" maxlength="6"></div><div class="form-group"><label>از تاریخ</label><input id="fgpFrom" placeholder="14050401" maxlength="8"></div><div class="form-group"><label>تا تاریخ</label><input id="fgpTo" placeholder="14050431" maxlength="8"></div></div><div class="form-group"><label>شرح (اختیاری)</label><textarea id="fgpDescription"></textarea></div><button class="btn" id="fgpCreate">ایجاد پیش‌نویس</button></div></div>`:''}<div class="card"><div class="card-header"><h5>نسخه‌های سیاست</h5></div><div class="card-body">${reasonBox()}<div id="fgpList">در حال بارگذاری…</div></div></div>`);
    try{const result=await api('/api/accounting/commission-policies');const list=q('#fgpList');if(!list)return;list.innerHTML=`<table class="table"><thead><tr><th>سیاست</th><th>دوره و اثر</th><th>وضعیت</th><th>هویت‌ها</th><th>اقدام</th></tr></thead><tbody>${(result.list||[]).map(row=>`<tr><td><b>${safe(row.name)}</b><details><summary>جزئیات حسابرسی</summary><small>${safe(row.policyVersionId)}</small></details></td><td>${safe(row.accountingPeriod)}<br>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td><td>${safe(statusFa(row.status))} / بازبینی ${safe(row.revision)}</td><td>سازنده: ${safe(row.createdBy?.username)}<br>تأییدکننده: ${safe(row.approvedBy?.username||'—')}</td><td>${actions(row,'policy')}</td></tr>`).join('')||'<tr><td colspan="5">سیاستی وجود ندارد.</td></tr>'}</tbody></table>`;bindTransitions(policyPage);if(q('#fgpCreate'))q('#fgpCreate').onclick=async()=>{try{await api('/api/accounting/commission-policies',{method:'POST',body:JSON.stringify({name:q('#fgpName').value,accountingPeriod:q('#fgpPeriod').value,effectiveFrom:q('#fgpFrom').value,effectiveTo:q('#fgpTo').value,description:q('#fgpDescription').value})});await policyPage();}catch(error){message(`${error.code} ${error.message}`,true);}};}catch(error){const list=q('#fgpList');if(!list)return;list.innerHTML=`<div class="error">دریافت سیاست‌ها ناموفق بود: ${safe(error.code||'')} ${safe(error.message)} <button class="btn" id="fgpRetry">تلاش دوباره</button></div>`;q('#fgpRetry').onclick=policyPage;}
  }
  async function mappingPage(){
    const editable=EDIT.includes(userRole());page('تعریف پورسانت گروه کالا',`${editable?`<div class="card"><div class="card-header"><h5>نگاشت دسته رسمی کالا به سبد نرخ</h5></div><div class="card-body"><div class="info">دسته رسمی کالا همان گروه اصلی رسمی شایگان است. نرخ در این فرم تعریف نمی‌شود.</div><div id="fgmPolicyState"><div class="muted">در حال دریافت سیاست‌های مصوب…</div></div><div class="row four"><div class="form-group"><label>سیاست مصوب</label><select id="fgmPolicy" disabled><option>در حال بارگذاری…</option></select></div><div class="form-group"><label>گروه اصلی رسمی</label><select id="fgmGroup"></select></div><div class="form-group"><label>سبد نرخ پورسانت</label><select id="fgmPool"><option>UNRESOLVED</option><option>NOTEBOOK</option><option>COMPONENT</option><option>CUSTOM</option><option>NON_COMMISSIONABLE</option></select></div><div class="form-group"><label>شرح (اختیاری)</label><input id="fgmDescription"></div></div><div class="row four"><div class="form-group"><label>از تاریخ</label><input id="fgmFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ</label><input id="fgmTo" value="14050431"></div><div class="form-group"><label>&nbsp;</label><button class="btn" id="fgmCreate" disabled>ایجاد پیش‌نویس</button><div class="small muted">تا انتخاب یک سیاست مصوب، ثبت غیرفعال است.</div></div></div></div></div>`:''}<div class="card"><div class="card-header"><h5>سوابق دسته و سبد نرخ</h5></div><div class="card-body">${reasonBox()}<div id="fgmList">در حال بارگذاری…</div></div></div>`);
    let policies;
    try{policies=await api('/api/accounting/commission-policies');configurePolicySelector({selectId:'fgmPolicy',createId:'fgmCreate',stateId:'fgmPolicyState',policies,refresh:mappingPage});}
    catch(error){policyLoadError('fgmPolicyState',error,mappingPage);return;}if(!q('#fgmList'))return;
    try{
      // The mapping selector must be independent from existing mappings and FIFO facts.
      // Its source of truth is the official, read-only Shaygan group catalog.
      const [catalog,mappings]=await Promise.all([api('/api/accounting/governance/group-catalog?pageSize=500'),api('/api/accounting/profit-ledger/categories?pageSize=500')]);
      const officialGroups=(catalog.list||[]).filter(row=>row.officialProductCategoryGuid);
      if(q('#fgmGroup')){
        q('#fgmGroup').disabled=!officialGroups.length;
        q('#fgmGroup').innerHTML=officialGroups.length
          ?officialGroups.map(row=>`<option value="${safe(row.officialProductCategoryIdentity)}" data-guid="${safe(row.officialProductCategoryGuid)}" data-number="${safe(row.officialProductCategoryNumber)}" data-name="${safe(row.officialProductCategoryName)}">${safe(row.officialProductCategoryNumber)} — ${safe(row.officialProductCategoryName)}</option>`).join('')
          :'<option value="">کاتالوگ رسمی گروه‌های شایگان هنوز آماده نیست</option>';
      }
      let editingMapping=null;
      const createMapping=async group=>{const policyVersionId=q('#fgmPolicy').value;if(!policyVersionId){message('ابتدا یک سیاست پورسانت مصوب را انتخاب کنید.',true);return;}if(!group?.officialProductCategoryGuid){message('هویت GUID گروه اصلی رسمی موجود نیست.',true);return;}try{const result=await api('/api/accounting/profit-ledger/categories',{method:'POST',body:JSON.stringify({policyVersionId,groupPathIdentity:group.officialProductCategoryIdentity,officialProductCategoryIdentity:group.officialProductCategoryIdentity,officialProductCategoryGuid:group.officialProductCategoryGuid,officialProductCategoryNumber:group.officialProductCategoryNumber,officialProductCategoryName:group.officialProductCategoryName,commissionRatePool:q('#fgmPool').value,effectiveFrom:q('#fgmFrom').value,effectiveTo:q('#fgmTo').value,description:q('#fgmDescription').value})});mappings.list=[result.mapping,...(mappings.list||[])];message(`پیش‌نویس واقعی ایجاد شد: ${result.mapping?.mappingId||'شناسه نامشخص'} / بازبینی ${result.mapping?.revision||'—'}`);renderRows();}catch(error){message(`${error.code} ${error.message}`,true,error.code);}};
      const updateMapping=async()=>{if(!editingMapping)return;try{const result=await api(`/api/accounting/profit-ledger/categories/${encodeURIComponent(editingMapping.mappingId)}`,{method:'PUT',body:JSON.stringify({revision:Number(editingMapping.revision),effectiveFrom:q('#fgmFrom').value,effectiveTo:q('#fgmTo').value,commissionRatePool:q('#fgmPool').value,description:q('#fgmDescription').value})});mappings.list=(mappings.list||[]).map(row=>row.mappingId===result.mapping.mappingId?result.mapping:row);editingMapping=null;q('#fgmCreate').textContent='ایجاد پیش‌نویس';message(`پیش‌نویس به‌روزرسانی شد: ${result.mapping.mappingId} / بازبینی ${result.mapping.revision}`);renderRows();}catch(error){message(`${error.code} ${error.message}`,true,error.code);}};
      let workflowFilter='not_started',workflowSearch='';
      const renderRows=()=>{
        const list=q('#fgmList');if(!list)return;
        const policyVersionId=q('#fgmPolicy')?.value||'',effectiveFrom=q('#fgmFrom')?.value||'',effectiveTo=q('#fgmTo')?.value||'';
        const scoped=(mappings.list||[]).filter(row=>row.policyVersionId===policyVersionId&&row.effectiveFrom===effectiveFrom&&(row.effectiveTo||'')===effectiveTo);
        const activeStatuses=new Set(['draft','returned','pending','approved']);
        const entries=officialGroups.map(group=>{const records=scoped.filter(row=>row.officialProductCategoryGuid===group.officialProductCategoryGuid).sort((a,b)=>new Date(b.updatedAt||b.createdAt||0)-new Date(a.updatedAt||a.createdAt||0));const current=records.find(row=>activeStatuses.has(row.status))||null;return{group,records,current,status:current?(current.status==='returned'?'draft':current.status):'not_started'};});
        const counts={not_started:entries.filter(x=>x.status==='not_started').length,draft:entries.filter(x=>x.status==='draft').length,pending:entries.filter(x=>x.status==='pending').length,approved:entries.filter(x=>x.status==='approved').length,rejected:scoped.filter(x=>x.status==='rejected').length,cancelled:scoped.filter(x=>x.status==='cancelled').length};
        const selectedGroup=officialGroups.find(group=>group.officialProductCategoryIdentity===q('#fgmGroup')?.value);const draftCreated=selectedGroup&&entries.find(x=>x.group.officialProductCategoryGuid===selectedGroup.officialProductCategoryGuid)?.status==='draft';
        const checklist=[['Policy Approved',Boolean(policyVersionId)],['Official Catalog Ready',officialGroups.length>0],['Main Group Selected',Boolean(selectedGroup)],['Rate Pool Selected',Boolean(q('#fgmPool')?.value)],['Draft Created',Boolean(draftCreated)],['Submitted',selectedGroup&&entries.find(x=>x.group.officialProductCategoryGuid===selectedGroup.officialProductCategoryGuid)?.status==='pending'],['Approved',selectedGroup&&entries.find(x=>x.group.officialProductCategoryGuid===selectedGroup.officialProductCategoryGuid)?.status==='approved']];
        const needle=workflowSearch.trim().toLowerCase();let visible=entries.filter(entry=>{if(workflowFilter==='rejected')return entry.records.some(row=>row.status==='rejected');if(workflowFilter==='all')return true;return entry.status===workflowFilter;});if(needle)visible=visible.filter(entry=>`${entry.group.officialProductCategoryName} ${entry.group.officialProductCategoryNumber} ${entry.group.officialProductCategoryGuid}`.toLowerCase().includes(needle));
        const nextAction=entry=>entry.status==='not_started'?'ایجاد پیش‌نویس':entry.status==='draft'?'ارسال برای تأیید':entry.status==='pending'?'در انتظار تأیید Admin/Manager':entry.status==='approved'?'تأیید شده':'نیازمند بررسی';
        const statusLabel=entry=>({not_started:'ثبت‌نشده',draft:'پیش‌نویس',pending:'در انتظار تأیید',approved:'تأییدشده'})[entry.status]||entry.status;
        const catalogState=!officialGroups.length?`<div class="warn"><b>کاتالوگ رسمی گروه‌های شایگان آماده نیست.</b><br>صفر بودن نگاشت‌ها طبیعی است، اما تا ساخت یا Refresh موفق کاتالوگ، انتخاب گروه اصلی ممکن نیست.<div class="actions"><button class="btn fgm-open-catalog">مشاهده کاتالوگ رسمی گروه‌ها</button></div></div>`:'';
        const rows=visible.map(entry=>{const group=entry.group,row=entry.current;const canEditDraft=editable&&['draft','returned'].includes(row?.status);const action=row?`${canEditDraft?`<button class="mini fgm-edit-draft" data-id="${safe(row.mappingId)}">ویرایش</button> `:''}${actions(row,'mapping')}`:(editable&&policyVersionId?`<button class="mini fgm-create-projection" data-guid="${safe(group.officialProductCategoryGuid)}">ایجاد پیش‌نویس</button>`:'—');return `<tr data-row-type="${entry.current?'persisted':'projection'}" data-workflow-status="${safe(entry.status)}"><td><b>${safe(group.officialProductCategoryNumber)} — ${safe(group.officialProductCategoryName)}</b><br><small>GUID: ${safe(group.officialProductCategoryGuid)} | ${safe(group.itemAssignmentCount||'—')} Item</small></td><td>${safe(row?.commissionRatePool||q('#fgmPool')?.value||'UNRESOLVED')}</td><td><span class="status-${safe(entry.status)}">${safe(statusLabel(entry))}</span><br><small>مرحله بعد: ${safe(nextAction(entry))}</small></td><td>${safe(row?.effectiveFrom||effectiveFrom)} تا ${safe(row?.effectiveTo||effectiveTo||'باز')}</td><td>${action}</td></tr>`;}).join('')||'<tr><td colspan="5">برای این فیلتر کاری باقی نمانده است.</td></tr>';
        const approved=entries.filter(entry=>entry.status==='approved');const history=scoped.filter(row=>['rejected','cancelled','historical_frozen'].includes(row.status));
        list.innerHTML=`${catalogState}<div class="card"><div class="card-body"><b>پیشرفت Policy</b><br>Policy: ${safe(policyVersionId||'انتخاب‌نشده')} | دوره: ${safe(effectiveFrom)} تا ${safe(effectiveTo||'باز')}<div class="row four"><div>گروه اصلی: <b>${safe(officialGroups.length)}</b></div><div>ثبت‌نشده: <b>${safe(counts.not_started)}</b></div><div>کار باقی‌مانده: <b>${safe(counts.not_started)}</b></div><div>Draft: <b>${safe(counts.draft)}</b></div><div>Pending: <b>${safe(counts.pending)}</b></div><div>Approved: <b>${safe(counts.approved)}</b></div><div>Rejected: <b>${safe(counts.rejected)}</b></div><div>Cancelled: <b>${safe(counts.cancelled)}</b></div><div>Group Coverage: <b>${officialGroups.length?Math.round((counts.approved/officialGroups.length)*100):0}%</b></div></div><div class="small muted">Tir Sale Value Coverage در این UI بدون scan جدید FIFO محاسبه نمی‌شود.</div></div></div><div class="info">${checklist.map(([label,done])=>`${done?'☑':'☐'} ${safe(label)}`).join(' &nbsp; ')}</div><div class="actions"><input id="fgmWorkflowSearch" placeholder="جستجو: نام، شماره یا GUID" value="${safe(workflowSearch)}"><select id="fgmWorkflowFilter"><option value="all">همه</option><option value="not_started">ثبت‌نشده</option><option value="draft">Draft</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select></div><h5>کار باقی‌مانده و وضعیت جاری</h5><div style="overflow:auto"><table class="table"><thead><tr><th>گروه اصلی رسمی</th><th>Rate Pool</th><th>وضعیت / مرحله بعد</th><th>دوره</th><th>اقدام</th></tr></thead><tbody>${rows}</tbody></table></div><details><summary>ثبت‌های تأییدشده (${safe(approved.length)})</summary><table class="table"><tbody>${approved.map(entry=>`<tr><td>${safe(entry.group.officialProductCategoryName)}</td><td>${safe(entry.current.commissionRatePool)}</td><td>${safe(entry.current.effectiveFrom)} تا ${safe(entry.current.effectiveTo||'باز')}</td><td>${safe(entry.current.approvedBy?.username||'—')} / ${safe(entry.current.approvedAt||'—')}</td></tr>`).join('')||'<tr><td>ثبت تأییدشده‌ای نیست.</td></tr>'}</tbody></table></details><details><summary>تاریخچه و ممیزی / رکورد تاریخی (${safe(history.length)})</summary><table class="table"><tbody>${history.map(row=>`<tr><td>${safe(row.officialProductCategoryName||row.identityValue)}</td><td>${safe(statusFa(row.status))}</td><td>بازبینی ${safe(row.revision)}</td><td><details><summary>Audit</summary><pre>${safe(JSON.stringify(row.auditLog||[],null,2))}</pre></details></td></tr>`).join('')||'<tr><td>تاریخچه‌ای برای این Policy/دوره نیست.</td></tr>'}</tbody></table></details>`;
        const filter=q('#fgmWorkflowFilter');if(filter){filter.value=workflowFilter;filter.onchange=()=>{workflowFilter=filter.value;renderRows();};}q('#fgmWorkflowSearch')?.addEventListener('input',event=>{workflowSearch=event.target.value;renderRows();});list.querySelectorAll('.fgm-open-catalog').forEach(button=>button.onclick=()=>{location.hash='official-group-catalog';route();});list.querySelectorAll('.fgm-create-projection').forEach(button=>button.onclick=()=>createMapping(officialGroups.find(group=>group.officialProductCategoryGuid===button.dataset.guid)));list.querySelectorAll('.fgm-edit-draft').forEach(button=>button.onclick=()=>{editingMapping=(mappings.list||[]).find(row=>row.mappingId===button.dataset.id)||null;if(!editingMapping)return;q('#fgmGroup').value=editingMapping.officialProductCategoryIdentity;q('#fgmPool').value=editingMapping.commissionRatePool;q('#fgmFrom').value=editingMapping.effectiveFrom;q('#fgmTo').value=editingMapping.effectiveTo||'';q('#fgmDescription').value=editingMapping.description||'';q('#fgmCreate').textContent='ذخیره پیش‌نویس';renderRows();});bindTransitions(mappingPage);
      };
      const rerender=()=>renderRows();
      // Catalog and mapping history arrive after the policy selector. Reconcile
      // the selected policy with every dependent control both immediately and
      // for the first native selection event; do not require a re-selection.
      const policySelect=q('#fgmPolicy');const policyRender=policySelect?.onchange;
      const syncMappingDependencies=()=>{policyRender?.();rerender();const selected=selectablePolicies(policies).some(row=>row.policyVersionId===policySelect?.value);const create=q('#fgmCreate');if(create)create.disabled=!selected;};
      if(policySelect){policySelect.onchange=syncMappingDependencies;policySelect.oninput=syncMappingDependencies;}
      ['fgmFrom','fgmTo','fgmPool','fgmGroup'].forEach(id=>{if(q(`#${id}`))q(`#${id}`).onchange=rerender;});syncMappingDependencies();
      if(q('#fgmCreate'))q('#fgmCreate').onclick=()=>{if(editingMapping)return updateMapping();const option=q('#fgmGroup').selectedOptions[0];if(!option){message('گروه اصلی رسمی قابل انتخاب وجود ندارد.',true);return;}return createMapping({officialProductCategoryIdentity:option.value,officialProductCategoryGuid:option.dataset.guid,officialProductCategoryNumber:option.dataset.number,officialProductCategoryName:option.dataset.name});};
    }catch(error){const list=q('#fgmList');if(!list)return;list.innerHTML=`<div class="error">دریافت سوابق نگاشت ناموفق بود: ${safe(error.code||'')} ${safe(error.message)} <button class="btn" id="fgmRetry">تلاش دوباره</button></div>`;q('#fgmRetry').onclick=mappingPage;}
  }
  function percentToRateExact(value){
    const text=String(value??'').trim().replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)));
    if(!/^\d{1,3}(?:\.\d{1,6})?$/.test(text))throw Object.assign(new Error('درصد باید عددی بین ۰ و ۱۰۰ و حداکثر دارای ۶ رقم اعشار باشد.'),{code:'RATE_INVALID'});
    const [whole,fraction='']=text.split('.'),percentScaled=BigInt(whole)*100000000n+BigInt(fraction.padEnd(8,'0'));
    if(percentScaled>10000000000n)throw Object.assign(new Error('درصد نباید بیشتر از ۱۰۰ باشد.'),{code:'RATE_INVALID'});
    const rateScaled=percentScaled/100n;return `${rateScaled/100000000n}.${String(rateScaled%100000000n).padStart(8,'0')}`;
  }
  function rateToPercent(value){const [whole='0',fraction='']=String(value||'0').split('.'),scaled=(BigInt(whole)*100000000n+BigInt(fraction.padEnd(8,'0').slice(0,8)))*100n;return `${scaled/100000000n}.${String(scaled%100000000n).padStart(8,'0')}`.replace(/\.0+$/,'').replace(/(\.\d*?)0+$/,'$1');}
  function rateErrorFa(error){return({RATE_NOT_FOUND:'نرخ منطبق پیدا نشد؛ مقدار ناموجود صفر نیست.',RATE_CONFLICT:'بیش از یک نرخ در یک سطح تقدم پیدا شد.',RATE_OVERLAP:'نرخ مصوب عملیاتی هم‌پوشان وجود دارد.',RATE_POLICY_NOT_APPROVED:'سیاست انتخابی دیگر مصوب و عملیاتی نیست.',RATE_STALE_REVISION:'رکورد هم‌زمان تغییر کرده است؛ صفحه را بازخوانی کنید.',RATE_SELF_APPROVAL_FORBIDDEN:'سازنده نمی‌تواند رکورد خود را تأیید کند.',RATE_TARGET_INVALID:'هدف Rate باید فقط دسته رسمی کالا یا سبد نرخ باشد.',RATE_EFFECTIVE_PERIOD_INVALID:'بازه بسته Rate باید کاملاً داخل Policy باشد.',RATE_APPROVAL_LOCKED:'تأیید هم‌زمان دیگری در جریان است.',RATE_RECORD_CLASS_INVALID:'نوع رکورد فقط توسط Backend تعیین می‌شود.',RATE_CLOSED_PERIOD:'بازه Rate بسته است و تغییر مستقیم مجاز نیست.',RATE_SUPERSESSION_INVALID:'نسخه جایگزین یا دلیل آن معتبر نیست.',RATE_DUPLICATE_OPEN_WORKFLOW:'برای همین Policy/Seller/Target/Period یک workflow باز وجود دارد.'})[error.code]||error.message;}
  async function ratePage(){
    const editable=EDIT.includes(userRole());
    page('نرخ‌های پورسانت',`${editable?`<div class="card"><div class="card-header"><h5 id="fgrFormTitle">نسخه نرخ جدید</h5></div><div class="card-body"><div class="info">ترتیب قطعی: فروشنده/دسته ← فروشنده/سبد نرخ ← پیش‌فرض دسته ← پیش‌فرض سبد نرخ. نرخ ناموجود با صفر درصد متفاوت است.</div><div id="fgrPolicyState"><div class="muted">در حال دریافت سیاست‌های مصوب…</div></div><div class="row four"><div class="form-group"><label>Policy</label><select id="fgrPolicy" disabled></select></div><div class="form-group"><label>اعمال برای</label><select id="fgrApplies"><option value="default">پیش‌فرض</option><option value="seller">فروشنده</option></select></div><div class="form-group" id="fgrSellerBox"><label>فروشنده</label><select id="fgrSeller"></select></div><div class="form-group"><label>نوع هدف</label><select id="fgrScope"><option value="product_category">دسته رسمی کالا</option><option value="rate_pool">سبد نرخ</option></select></div></div><div class="row four"><div class="form-group" id="fgrCategoryBox"><label>Product Category</label><select id="fgrCategory"></select></div><div class="form-group" id="fgrPoolBox"><label>Commission Rate Pool</label><select id="fgrPool"><option>NOTEBOOK</option><option>COMPONENT</option><option>CUSTOM</option><option>NON_COMMISSIONABLE</option></select></div><div class="form-group"><label>نرخ درصد</label><input id="fgrRate" placeholder="14.125"></div><div class="form-group"><label>شرح (اختیاری)</label><input id="fgrDescription"></div></div><div class="row four"><div class="form-group"><label>از تاریخ شمسی</label><input id="fgrFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ شمسی</label><input id="fgrTo" value="14050431"></div><div class="form-group" id="fgrSupersessionBox" style="display:none"><label>دلیل جایگزینی (الزامی)</label><input id="fgrSupersessionReason"></div><div class="form-group"><label>&nbsp;</label><button class="btn" id="fgrCreate" disabled>ایجاد پیش‌نویس</button></div></div></div></div>`:''}<div class="card"><div class="card-header"><h5>حاکمیت نرخ و پوشش</h5></div><div class="card-body">${reasonBox()}<div id="fgrList">در حال بارگذاری…</div></div></div>`);
    let policies;try{policies=await api('/api/accounting/commission-policies');configurePolicySelector({selectId:'fgrPolicy',createId:'fgrCreate',stateId:'fgrPolicyState',policies,refresh:ratePage});}catch(error){policyLoadError('fgrPolicyState',error,ratePage);return;}if(!q('#fgrList'))return;
    try{
      const [rates,catalog,filters,coverage]=await Promise.all([api('/api/accounting/profit-ledger/rates?pageSize=500'),api('/api/accounting/governance/group-catalog?pageSize=500'),api('/api/accounting/seller-financial-performance/filters').catch(()=>({sellers:[]})),api('/api/accounting/seller-financial-performance/governance-coverage').catch(()=>null)]);
      const all=rates.list||[],categories=(catalog.list||[]).filter(row=>row.officialProductCategoryGuid),sellers=filters.sellers||[];
      if(q('#fgrCategory'))q('#fgrCategory').innerHTML=categories.map(row=>`<option value="${safe(row.officialProductCategoryIdentity)}" data-name="${safe(row.officialProductCategoryName)}">${safe(row.officialProductCategoryNumber)} — ${safe(row.officialProductCategoryName)}</option>`).join('');
      if(q('#fgrSeller'))q('#fgrSeller').innerHTML=sellers.map(row=>`<option value="${safe(row.identity)}">${safe(row.name)} — ${safe(row.identity)}</option>`).join('');
      let editing=null,superseding=null,search='';
      const policyId=()=>q('#fgrPolicy')?.value||'';
      const sync=()=>{if(q('#fgrSellerBox'))q('#fgrSellerBox').style.display=q('#fgrApplies').value==='seller'?'block':'none';if(q('#fgrCategoryBox'))q('#fgrCategoryBox').style.display=q('#fgrScope').value==='product_category'?'block':'none';if(q('#fgrPoolBox'))q('#fgrPoolBox').style.display=q('#fgrScope').value==='rate_pool'?'block':'none';if(q('#fgrCreate'))q('#fgrCreate').disabled=!selectablePolicies(policies).some(row=>row.policyVersionId===policyId());render();};
      const formPayload=()=>{const scope=q('#fgrScope').value,category=q('#fgrCategory')?.selectedOptions?.[0];return{policyVersionId:policyId(),sellerIdentity:q('#fgrApplies').value==='default'?'*':q('#fgrSeller').value,rateScope:scope,commissionRatePool:scope==='rate_pool'?q('#fgrPool').value:'',officialProductCategoryIdentity:scope==='product_category'?category?.value||'':'',officialProductCategoryName:scope==='product_category'?category?.dataset.name||'':'',rate:percentToRateExact(q('#fgrRate').value),effectiveFrom:q('#fgrFrom').value,effectiveTo:q('#fgrTo').value,description:q('#fgrDescription').value};};
      const reset=()=>{editing=null;superseding=null;q('#fgrFormTitle').textContent='نسخه نرخ جدید';q('#fgrCreate').textContent='ایجاد پیش‌نویس';q('#fgrSupersessionBox').style.display='none';};
      const save=async()=>{try{let result;if(superseding)result=await api(`/api/accounting/profit-ledger/rates/${encodeURIComponent(superseding.rateVersionId)}/supersede`,{method:'POST',body:JSON.stringify({...formPayload(),reason:q('#fgrSupersessionReason').value})});else if(editing)result=await api(`/api/accounting/profit-ledger/rates/${encodeURIComponent(editing.rateVersionId)}`,{method:'PUT',body:JSON.stringify({...formPayload(),revision:Number(editing.revision)})});else result=await api('/api/accounting/profit-ledger/rates',{method:'POST',body:JSON.stringify(formPayload())});const row=result.rateVersion,index=all.findIndex(item=>item.rateVersionId===row.rateVersionId);if(index>=0)all[index]=row;else all.unshift(row);reset();render();message(`پیش‌نویس نرخ ثبت شد: ${row.rateVersionId}`);}catch(error){message(`${error.code} ${rateErrorFa(error)}`,true,error.code);}};
      const loadForm=(row,replacement=false)=>{editing=replacement?null:row;superseding=replacement?row:null;q('#fgrApplies').value=row.sellerIdentity==='*'?'default':'seller';q('#fgrSeller').value=row.sellerIdentity;q('#fgrScope').value=row.rateScope;q('#fgrPool').value=row.commissionRatePool;q('#fgrCategory').value=row.officialProductCategoryIdentity;q('#fgrRate').value=rateToPercent(row.rate);q('#fgrFrom').value=row.effectiveFrom;q('#fgrTo').value=row.effectiveTo;q('#fgrDescription').value=row.description||row.reason||'';q('#fgrFormTitle').textContent=replacement?'نسخه جایگزین':'ویرایش پیش‌نویس';q('#fgrCreate').textContent=replacement?'ایجاد پیش‌نویس جایگزین':'ذخیره پیش‌نویس';q('#fgrSupersessionBox').style.display=replacement?'block':'none';sync();};
      const rowActions=row=>`${editable&&['draft','returned'].includes(row.status)?`<button class="mini fgr-edit" data-id="${safe(row.rateVersionId)}">ویرایش</button> `:''}${actions(row,'rate')}${editable&&row.status==='approved'&&!['superseded','retired'].includes(row.lifecycleStatus)?` <button class="mini fgr-supersede" data-id="${safe(row.rateVersionId)}">ایجاد نسخه جایگزین</button>`:''}`;
      function render(){const list=q('#fgrList');if(!list)return;const rows=all.filter(row=>!policyId()||row.policyVersionId===policyId()),needle=search.trim().toLowerCase();const open=rows.filter(row=>['draft','returned','pending'].includes(row.status)).filter(row=>!needle||`${row.sellerIdentity} ${row.officialProductCategoryName} ${row.commissionRatePool}`.toLowerCase().includes(needle));const approved=rows.filter(row=>row.status==='approved'&&!['superseded','retired'].includes(row.lifecycleStatus));const history=rows.filter(row=>['rejected','cancelled','historical_frozen'].includes(row.status)||['superseded','retired'].includes(row.lifecycleStatus));const valueCoverage=coverage?.approvedRateSaleValueCoveragePercent??coverage?.approvedRateCoveragePercent??0,lineCoverage=coverage?.approvedRateLineCoveragePercent??0;list.innerHTML=`<div class="row four"><div>Policy: <b>${safe(policyId()||'انتخاب‌نشده')}</b></div><div>Total Rates: <b>${safe(rows.length)}</b></div><div>Not Started / Missing: <b>${safe(coverage?.missingRateLines??'—')}</b></div><div>Draft: <b>${safe(rows.filter(row=>['draft','returned'].includes(row.status)).length)}</b></div><div>Pending: <b>${safe(rows.filter(row=>row.status==='pending').length)}</b></div><div>Approved: <b>${safe(approved.length)}</b></div><div>Missing Sale Value: <b>${safe(coverage?.missingRateSaleValueExact??'—')}</b></div><div>Resolved Sale Value: <b>${safe(valueCoverage)}%</b></div></div><div class="warn">Commission مقدماتی و non-payable است؛ Rate روی Gross Sales اعمال نمی‌شود.</div><div class="info">Sale Value ${safe(valueCoverage)}% | Sale Lines ${safe(lineCoverage)}% | Sellers ${safe(coverage?.approvedRateSellerCoveragePercent??'—')}% | Groups ${safe(coverage?.approvedRateGroupCoveragePercent??'—')}%</div><div class="actions"><input id="fgrSearch" placeholder="جستجو" value="${safe(search)}"></div><h5>نیازمند اقدام / Missing Coverage</h5><div class="small muted">فقط Policy، Mapping و Rate مصوب عملیاتی در پوشش Approved شمرده می‌شوند.</div><h5>Draft / Pending</h5><table class="table"><thead><tr><th>اعمال برای</th><th>هدف</th><th>Rate</th><th>بازه</th><th>وضعیت / اقدام بعدی</th><th>اقدام</th></tr></thead><tbody>${open.map(row=>`<tr><td>${row.sellerIdentity==='*'?'پیش‌فرض':safe(row.sellerIdentity)}</td><td>${safe(row.officialProductCategoryName||row.commissionRatePool)}</td><td>${safe(rateToPercent(row.rate))}%</td><td>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo)}</td><td>${safe(statusFa(row.status))}<br><small>${row.status==='pending'?'تأیید مستقل':'ارسال برای تأیید'}</small></td><td>${rowActions(row)}</td></tr>`).join('')||'<tr><td colspan="6">رکورد باز وجود ندارد.</td></tr>'}</tbody></table><details><summary>Approved (${safe(approved.length)})</summary>${approved.map(row=>`<div>${safe(row.sellerIdentity)} — ${safe(row.officialProductCategoryName||row.commissionRatePool)} — ${safe(rateToPercent(row.rate))}% — ${rowActions(row)}</div>`).join('')||'موردی نیست.'}</details><details><summary>History / Audit (${safe(history.length)})</summary>${history.map(row=>`<div><b>${safe(row.rateVersionId)}</b> — ${safe(row.lifecycleStatus||row.status)}<details><summary>Audit</summary><pre>${safe(JSON.stringify(row.auditLog||[],null,2))}</pre></details></div>`).join('')||'موردی نیست.'}</details>`;q('#fgrSearch')?.addEventListener('input',event=>{search=event.target.value;render();});list.querySelectorAll('.fgr-edit').forEach(button=>button.onclick=()=>loadForm(all.find(row=>row.rateVersionId===button.dataset.id),false));list.querySelectorAll('.fgr-supersede').forEach(button=>button.onclick=()=>loadForm(all.find(row=>row.rateVersionId===button.dataset.id),true));bindTransitions(ratePage);}
      const policySelect=q('#fgrPolicy'),policyRender=policySelect?.onchange;const syncPolicy=()=>{policyRender?.();sync();};if(policySelect){policySelect.onchange=syncPolicy;policySelect.oninput=syncPolicy;}['fgrApplies','fgrScope'].forEach(id=>{if(q(`#${id}`))q(`#${id}`).onchange=sync;});if(q('#fgrCreate'))q('#fgrCreate').onclick=save;sync();
    }catch(error){q('#fgrList').innerHTML=`<div class="error">${safe(error.code)} ${safe(rateErrorFa(error))}<button class="btn" id="fgrRetry">تلاش دوباره</button></div>`;q('#fgrRetry').onclick=ratePage;}
  }
  const renderers={'commission-policy-governance':policyPage,'commission-category-governance':mappingPage,'commission-rate-governance':ratePage};
  window.__phaseBFinancialRenderers=renderers;
  window.__phaseBFinancialNav=NAV;
})();
...
(()=>{
  const PAGES={'commission-category-governance':'نگاشت دسته و سبد نرخ','commission-rate-governance':'حاکمیت نرخ پورسانت','saved-profit-opening-governance':'مانده افتتاحیه سود سیوشده','commission-export-readiness':'سلامت داده‌های مالی'};
  ...
  function shell(page,actions=''){setPage(PAGES[page],`<main class="main-content"><div id="accountingGovernanceFifoFreshness"></div><div class="warn"><b>HUMAN GOVERNANCE ONLY</b> — no automatic approval, no FIFO rerun, no payable commission</div>${nav(page)}${actions}<div id="agBody"><div class="muted">در حال بارگذاری…</div></div></main>`);document.querySelectorAll('.ag-nav').forEach(b=>b.onclick=()=>{location.hash=b.dataset.page;route();});if(page==='commission-export-readiness')window.renderCanonicalFifoFreshness('#accountingGovernanceFifoFreshness');}
...
(()=>{
  ...
  window.pageAccountingFifoReadiness=async function(){
    setPage('Accounting FIFO Readiness',`<main class="main-content">
      <div id="accountingReadinessFifoFreshness"></div>
    ...
    window.renderCanonicalFifoFreshness('#accountingReadinessFifoFreshness');
    await refresh();
  };
  const inheritedMenu=window.renderMenu||renderMenu;
  window.renderMenu=renderMenu=function(){
...
(()=>{
  ...
  let controlDatasetId='';
  let fifoDatasetOptions=[];
  let fifoFreshness=null;
  let fifoBuildContext=null;
  ...
  function freshnessHtml(value={}){
    const stale=value.sourceFreshnessStatus==='STALE';
    return `<div class="${stale?'warn':'success'}"><div class="row three"><div><small>آخرین به‌روزرسانی FIFO</small><br><b>${safe(value.lastFifoUpdatedAt?toJalaliDisplay(value.lastFifoUpdatedAt):'—')}</b></div><div><small>محاسبات FIFO تا تاریخ</small><br><b>${safe(value.calculationCutoff||'—')}</b></div><div><small>وضعیت به‌روزرسانی</small><br><b>${safe(value.fifoStatus||'نامشخص')}</b></div></div>${(value.staleReasons||[]).length?`<small>${safe((value.staleReasons||[]).join('، '))}</small>`:''}</div>`;
  }
  async function loadFreshness(){
    fifoFreshness=await json('/api/accounting/fifo-shadow/freshness');
    const box=q('#fifoFreshness');if(box)box.innerHTML=freshnessHtml(fifoFreshness);
    const cutoff=q('#fifoCutoff');if(cutoff&&!cutoff.value)cutoff.value=fifoFreshness.calculationCutoff||'';
    const details=q('#fifoTechnicalFreshness');if(details)details.textContent=JSON.stringify(fifoFreshness,null,2);
    return fifoFreshness;
  }
  ...
  async function loadDatasets(){
    ...
    controlDatasetId=response.activeDatasetId||controlDatasetId;
  ...
  async function loadReport(){
    ...
    try{
      const [report,dimensions]=await Promise.all([
        json('/api/accounting/fifo-shadow/report?'+query({datasetId:selectedDatasetId})),
        json('/api/accounting/fifo-shadow/audit-dimensions?'+query({datasetId:selectedDatasetId}))
      ]);
      ...
      box.innerHTML=`
        ...
        <div class="card"><div class="card-header"><h5>Allocation Drill-down / Control Delta</h5></div><div class="card-body">
          ...
          <div class="row four"><div class="form-group"><label>فروشنده (canonicalSellerId)</label><select id="fifoSellerId"><option value="">همه</option>${sellerOptions}</select></div><div class="form-group"><label>جست‌وجوی نام فروشنده</label><input id="fifoSellerSearch"></div><div class="form-group"><label>دسته رسمی (Main Group GUID)</label><select id="fifoCategoryGuid"><option value="">همه</option>${categoryOptions}</select></div><div class="form-group"><label>جست‌وجوی نام دسته</label><input id="fifoCategorySearch"></div></div>
          ...
          <div class="row four"><div class="form-group"><label>Provenance</label><select id="fifoStatus"><option value="">همه</option><option>PROVEN</option><option>PARTIAL</option><option>UNKNOWN</option></select></div><div class="form-group"><label>Delta</label><select id="fifoDelta"><option value="">همه</option><option value="UNKNOWN_TO_PROVEN">UNKNOWN → PROVEN</option><option value="PROVEN_TO_UNKNOWN">PROVEN → UNKNOWN</option><option value="PROVEN_TO_PROVEN_SAME_COST">PROVEN → PROVEN SAME COST</option><option value="PROVEN_TO_PROVEN_CHANGED_COST">PROVEN → PROVEN CHANGED COST</option><option value="UNKNOWN_TO_UNKNOWN">UNKNOWN → UNKNOWN</option><option value="PARTIAL_TO_PROVEN">PARTIAL → PROVEN</option><option value="PROVEN_TO_PARTIAL">PROVEN → PARTIAL</option><option value="UNKNOWN_TO_PARTIAL">UNKNOWN → PARTIAL</option><option value="PARTIAL_TO_UNKNOWN">PARTIAL → UNKNOWN</option><option value="PARTIAL_TO_PARTIAL">PARTIAL → PARTIAL</option></select></div><div class="form-group"><label>مبلغ فروش از / تا</label><div class="row"><input id="fifoSaleMin"><input id="fifoSaleMax"></div></div><div class="form-group"><label>هزینه FIFO از / تا</label><div class="row"><input id="fifoCostMin"><input id="fifoCostMax"></div></div></div>
      ...
      q('#fifoMode').onchange=()=>{if(q('#fifoMode').value!=='delta')q('#fifoDelta').value='';};
      q('#fifoDelta').onchange=()=>{if(q('#fifoDelta').value)q('#fifoMode').value='delta';};
  ...
  async function startBuild(resumeDatasetId=''){
    ...
    try{
      let body={resumeDatasetId};
      if(!resumeDatasetId){
        const calculationCutoff=q('#fifoCutoff').value.trim();
        fifoBuildContext=await json('/api/accounting/fifo-shadow/build-context?'+query({calculationCutoff}));
        if(!confirm(`به‌روزرسانی FIFO تا تاریخ ${fifoBuildContext.calculationCutoff}\n\nSale: ${fifoBuildContext.saleSnapshotId}\nPurchase: ${fifoBuildContext.purchaseDatasetId}\nOpening: ${fifoBuildContext.openingDatasetId}\n\nنتیجه فقط Candidate غیرفعال است و فعال‌سازی جداگانه انجام می‌شود. ادامه می‌دهید؟`)){message.innerHTML='';return;}
        body={dateTo:fifoBuildContext.calculationCutoff,expectedBuildContextFingerprint:fifoBuildContext.buildContextFingerprint};
      }
      ...
      message.innerHTML='<div class="success"><b>FIFO جدید آماده بررسی است.</b><br>Candidate غیرفعال است؛ Delta و Human Validation را بررسی کنید.</div>';
      controlDatasetId=fifoFreshness?.activeFifoDatasetId||controlDatasetId;
      selectedDatasetId=job?.result?.datasetId||'';
      await loadDatasets();
      await loadFreshness();
      await loadReport();
  ...
  window.pageFifoShadowValidation=async function(){
    setPage('به‌روزرسانی و اعتبارسنجی FIFO',`<main class="main-content">
      <div id="fifoFreshness"><div class="info">در حال بررسی تازگی FIFO…</div></div>
      <div class="warn"><b>BUILD ≠ HUMAN VALIDATION ≠ ACTIVATION</b><br>SHADOW MODE — NOT ACCOUNTING APPROVED<br>به‌روزرسانی فقط یک Candidate تغییرناپذیر می‌سازد و هیچ Commission یا Payroll تولید نمی‌کند.</div>
      ${['admin','accounting'].includes(userRole())?`<div class="card"><div class="card-header"><h5>به‌روزرسانی FIFO</h5></div><div class="card-body"><div class="info">FIFO تمام رویدادهای مالی لازم تا این تاریخ را از ابتدا و بر اساس منابع مالی معتبر بازپخش می‌کند.</div><div class="row three"><div class="form-group"><label>به‌روزرسانی FIFO تا تاریخ</label><input id="fifoCutoff" placeholder="14051229" maxlength="8"></div><div class="form-group"><label>&nbsp;</label><button class="btn green" id="fifoStart">به‌روزرسانی FIFO</button></div><div class="form-group"><label>&nbsp;</label><button class="mini" id="fifoPurchaseFullRefresh">بازخوانی کامل تاریخی خرید</button></div></div><div class="small muted">بازخوانی خرید فقط یک Purchase Candidate جدید با mode=full می‌سازد؛ فعال‌سازی Purchase و FIFO جداگانه است.</div><details><summary>جزئیات فنی و ممیزی</summary><pre id="fifoTechnicalFreshness"></pre></details><div id="fifoJob"></div></div></div>`:'<div id="fifoJob"></div>'}
    ...
    if(q('#fifoPurchaseFullRefresh'))q('#fifoPurchaseFullRefresh').onclick=async()=>{const message=q('#fifoJob');try{const cutoff=q('#fifoCutoff').value.trim();if(!confirm(`بازخوانی کامل تاریخی Purchase تا ${cutoff} یک Candidate جدید می‌سازد. هیچ Dataset فعالی تغییر نمی‌کند. ادامه؟`))return;const response=await json('/api/purchase-layer-datasets/start',{method:'POST',body:JSON.stringify({dateFrom:'12000101',dateTo:cutoff,mode:'full',reset:true})});await waitForJob(response.jobId);message.innerHTML='<div class="success">Purchase Candidate کامل ساخته شد. پس از Delta، بررسی و فعال‌سازی governed، تازگی FIFO دوباره محاسبه می‌شود.</div>';await loadFreshness();}catch(error){message.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}};
    await loadFreshness();
    await loadDatasets();
    await loadReport();
  };
  ...
  window.renderMenu=renderMenu=function(){
    ...
    const menu=q('#menu');if(!menu||menu.querySelector(`[data-page="${PAGE}"]`))return;
    const button=document.createElement('button');button.className='navbtn';button.dataset.page=PAGE;button.textContent='به‌روزرسانی FIFO';
    button.onclick=event=>{event.preventDefault();location.hash=PAGE;route();};
    const before=menu.querySelector('[data-page="seller-profit"]')||menu.querySelector('[data-page="reports"]');
    if(before)before.parentNode.insertBefore(button,before);else menu.appendChild(button);
  };
...
(function(){
  ...
  window.openTypedTurnoverDocument=async function(identity,trigger=document.activeElement){
    const invNo=String(identity?.invNo||'').trim(),invType=invoiceTypes.normalizeInvTyp(identity?.invType);const isCurrent=typeof identity?.isCurrent==='function'?identity.isCurrent:()=>true;
    ...
    modal.innerHTML=`<div class="mk-modal-backdrop"></div><div class="mk-modal-card wide"><div class="mk-modal-head"><h3 id="turnoverDocumentTitle">${esc(docTypeLabel(invType))} شماره ${esc(invNo)}</h3><button class="mini" id="closeTurnoverDocument">بستن</button></div><div id="turnoverDocumentBody" class="mk-modal-body"><div class="info">در حال خواندن سند...</div></div></div>`;
    ...
    if(!/^\d+$/.test(invNo)||Number(invNo)<=0||!invoiceTypes.turnoverTypes.includes(invType)){$('#turnoverDocumentBody').innerHTML='<div class="error">شماره یا نوع سند گردش حساب معتبر نیست.</div>';return;}
    try{
      if(!isCurrent()){uiModalController.close({restoreFocus:false});return;}
      const r=await api(`/api/invoices/${encodeURIComponent(invNo)}?invType=${invType}&source=turnover`);if(!isCurrent()){uiModalController.close({restoreFocus:false});return;}if(!r.ok){$('#turnoverDocumentBody').innerHTML='<div class="error">خواندن سند با خطا مواجه شد. دوباره تلاش کنید.</div>';return;}const inv=(r.list||[]).find(x=>invoiceNoOf(x)===invNo&&invoiceTypeOf(x)===invType);if(!inv){$('#turnoverDocumentBody').innerHTML=`<div class="warn">${esc(docTypeLabel(invType))} شماره ${esc(invNo)} در شایگان پیدا نشد یا قابل خواندن نیست.</div>`;return;}
      const lines=invoiceBody(inv),canKardex=[2,6].includes(invType),gross=lines.reduce((sum,x)=>sum+Number((x.Amount??x.TotalPrice??(Number(x.Quan??x.Quantity??0)*Number(x.Price??x.UnitPrice??0)))||0),0),discount=Number(inv.DiscAmount??inv.DiscountAmount??0),finalTotal=Number(inv.TotalAmount??inv.SourceTotalAmount??(gross-discount));let annotations={list:[]};try{annotations=await api(`/api/invoices/${encodeURIComponent(invNo)}/annotations?invType=${invType}`);}catch{}
      if(!isCurrent()){uiModalController.close({restoreFocus:false});return;}
      $('#turnoverDocumentBody').innerHTML=`<div class="info">منبع رسمی این نمایش WebService شایگان است. اطلاعات ثبت‌شده در این نما فقط خواندنی است.</div><div class="grid3 inv-head"><div><b>شماره:</b> ${esc(inv.InvNo||invNo)}</div><div><b>تاریخ:</b> ${esc(toJalaliDisplay(inv.InvDate||inv.CreatedDate||''))}</div><div><b>طرف حساب:</b> ${esc(inv.AccountName||inv.AccountNumber||'')}</div><div><b>توضیحات:</b> ${esc(inv.InvDescription||'')}</div><div><b>Lead ID:</b> ${esc(inv.LeadId||inv.leadId||'')}</div><div><b>نوع:</b> ${esc(docTypeLabel(invType))} (InvTyp=${invType})</div></div><div class="table-scroll"><table class="table"><thead><tr><th>#</th><th>کد کالا</th><th>نام کالا</th><th>انبار</th><th>تعداد</th><th>قیمت</th><th>مبلغ</th>${canKardex?'<th>کاردکس</th>':''}</tr></thead><tbody>${lines.map((x,i)=>{const code=x.ItemNumber||x.ItemCode||'',qty=Number(x.Quan??x.Quantity??0),price=Number(x.Price??x.UnitPrice??0),amount=Number(x.Amount??x.TotalPrice??qty*price);return `<tr><td>${i+1}</td><td>${esc(code)}</td><td>${esc(x.ItemDescription||x.ItemDesc||x.ItemName||'')}</td><td>${esc(x.STNumber||x.StockNumber||x.StockName||'')}</td><td>${fmt(qty)}</td><td>${fmt(price)}</td><td>${fmt(amount)}</td>${canKardex?`<td><button class="mini invoice-item-kardex" data-code="${esc(code)}">مشاهده کاردکس</button></td>`:''}</tr>`}).join('')}</tbody></table></div><div class="summary-box"><div>جمع ردیف‌ها: <b>${fmt(gross)} ریال</b></div><div>تخفیف: <b>${fmt(discount)} ریال</b></div><div>جمع کل: <b>${fmt(finalTotal)} ریال</b></div></div><div class="actions"><a class="btn" target="_blank" href="/print/invoice/${encodeURIComponent(invNo)}?invType=${invType}">چاپ / نمایش نسخه رسمی</a></div><div class="card mt"><div class="card-header"><h5>یادداشت‌ها و لاگ CRM</h5></div><div class="card-body">${(annotations.list||[]).length?`<table class="table"><thead><tr><th>زمان</th><th>کاربر</th><th>نوع</th><th>شرح</th></tr></thead><tbody>${annotations.list.map(x=>`<tr><td>${esc(toJalaliDisplay(x.at||''))}</td><td>${esc(x.byName||x.by||'')}</td><td>${esc(x.type||'')}</td><td>${esc(x.note||x.reason||'')}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">یادداشتی ثبت نشده است.</div>'}</div></div><div id="invoiceKardexPanel"></div>`;
      modal.querySelectorAll('.invoice-item-kardex').forEach(b=>b.onclick=async()=>{const panel=$('#invoiceKardexPanel'),code=b.dataset.code;panel.innerHTML='<div class="info">در حال خواندن کاردکس...</div>';try{const kr=await api(`/api/invoices/${encodeURIComponent(invNo)}/items/${encodeURIComponent(code)}/kardex?invType=${invType}`);if(!kr.ok){panel.innerHTML=`<div class="error">${esc(kr.error||'خطا')}</div>`;return;}panel.innerHTML=`<h4>کاردکس ${esc(code)}</h4>${(kr.rows||[]).length?`<table class="table"><thead><tr><th>تاریخ</th><th>نوع</th><th>سند</th><th>ورود</th><th>خروج</th><th>مانده</th><th>خرید/تمام‌شده</th><th>فروش</th></tr></thead><tbody>${kr.rows.map(x=>`<tr><td>${esc(toJalaliDisplay(x.date||''))}</td><td>${esc(docTypeLabel(x.invoiceType))}</td><td>${esc(x.invoiceNumber||'')}</td><td>${fmt(x.inQty)}</td><td>${fmt(x.outQty)}</td><td>${fmt(x.remainQty)}</td><td>${fmt(x.costPrice)}</td><td>${fmt(x.salePrice)}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">ردیفی برای این کالا وجود ندارد.</div>'}`;}catch(e){panel.innerHTML='<div class="error">خواندن کاردکس با خطا مواجه شد.</div>';}});
    }catch(e){if(isCurrent())$('#turnoverDocumentBody').innerHTML='<div class="error">خواندن سند با خطا مواجه شد. دوباره تلاش کنید.</div>';else uiModalController.close({restoreFocus:false});}
  };
...
/* Hash routing is registered once by the later dynamic router. */
(async()=>{
...
(function(){
  ...
  window.pageSaleSnapshot=async function(){
    setPage('Sale Snapshot', `<main class="main-content"><div class="card"><div class="card-header"><h5>Sale Snapshot - زیرساخت فروش برای سود، عملکرد فروشنده و پیش‌بینی بازار</h5></div><div class="card-body"><div class="info">هر اجرا ابتدا در Candidate مستقل ساخته می‌شود. فقط Snapshot کامل، بدون صفحه ناموفق و با Validation موفق به‌صورت اتمیک فعال می‌شود؛ اجرای ناقص گزارش فعال قبلی را تغییر نمی‌دهد. فروش واقعی فقط InvoiceType=2 است. نگاشت شایگان: 2=فروش، 3=خرید، 6=برگشت از فروش، 7=برگشت از خرید.</div><div class="warn"><b>عملیات این صفحه ساخت Candidate جدید است، نه فعال‌سازی یک Candidate موجود.</b><br>برای گزارش رسمی، بازسازی کامل الزامی است. بازسازی افزایشی تا تأیید مجدد قرارداد آن، از این UI آغاز نمی‌شود.</div><div class="row four"><div class="form-group"><label>از تاریخ فروش</label><input id="saleSnapFrom" value="14050101"></div><div class="form-group"><label>تا تاریخ</label><input id="saleSnapTo" value=""></div><div class="form-group"><label>حداکثر صفحات فروش</label><input id="saleSnapPages" value="1000" inputmode="numeric"></div><div class="form-group"><label>RowCount ثابت</label><input id="saleSnapDetails" value="20" inputmode="numeric" disabled><div class="small muted">WebService حداکثر ۲۰ ردیف در هر صفحه می‌دهد؛ هر صفحه تا ۳ بار retry می‌شود.</div></div></div><div class="form-group"><label><input type="checkbox" id="saleSnapReset" checked disabled> بازسازی کامل در Candidate مستقل (الزامی)</label></div><div class="actions"><button class="btn green" id="saleSnapInit">آماده‌سازی</button><button class="btn red" id="saleSnapStart">ساخت Candidate با بازسازی کامل</button><button class="btn" id="saleSnapRefresh">Refresh</button></div><div id="saleSnapMsg" class="mt"></div></div></div><div class="card"><div class="card-header"><h5>Snapshotهای فروش</h5></div><div class="card-body"><div id="saleSnapList"></div></div></div><div class="card"><div class="card-header"><h5>Diagnostic فروش</h5></div><div class="card-body"><div id="saleSnapStatus"></div></div></div></main>`);
...
(function(){
  ...
  const manualCostPreviewFingerprints=new Map();
  async function json(url,options={}){
    const response=await fetch(url,{credentials:'include',headers:{'Content-Type':'application/json'},...options});
    const payload=await response.json().catch(()=>({ok:false,error:'پاسخ JSON معتبر نیست'}));
    if(!response.ok||payload.ok===false){const error=new Error(payload.error||response.statusText);error.code=payload.code||'';throw error;}
  ...
  function reasonLabel(reason){
    return ({
      ...
      source_history_incomplete:'تاریخچه منبع ناقص',
      unknown_cost:'هزینه نامشخص'
    })[reason]||reason||'—';
  }
  function manualEvidenceLabel(row){
    if(row.evidenceClass==='COMMERCIAL_ANNOUNCED_COST'||row.sourceType==='commercial_announced_cost')return 'قیمت اعلام بازرگانی';
    if((row.resolutionScope||'item')==='item')return 'هزینه دستی قدیمی — نیازمند بازبینی';
    return 'شواهد هزینه تاریخی';
  }
  async function loadQueue(){
    ...
    try{
      ...
      q('#mcDatasetMeta').textContent=`Sale Snapshot: ${report.activeSnapshotId||'ندارد'} | Purchase Dataset: ${report.activePurchaseLayerDatasetId||'ندارد'} | Opening Dataset: ${report.openingDatasetId||'ندارد'} | Opening Approval: ${report.openingApprovalStatus||'ندارد'} | سود و FIFO: غیرفعال`;
      box.innerHTML=`<div class="info">آیتم‌ها: ${number(report.total)} | صفحه ${number(report.page)} | Sale Snapshot: <code>${safe(report.activeSnapshotId||'')}</code> | Purchase Dataset: <code>${safe(report.activePurchaseLayerDatasetId||'')}</code> | Opening Dataset: <code>${safe(report.openingDatasetId||'')}</code> | Opening Approval: ${safe(report.openingApprovalStatus||'')}</div>
      ...
      document.querySelectorAll('.mc-resolve').forEach(button=>button.onclick=async()=>{
        ...
        const params=new URLSearchParams({itemCode:row.itemCode||'',itemGuid:row.itemGuid||'',applicableDate:row.firstSaleDate||'',reviewDateTo:row.lastSaleDate||row.firstSaleDate||'',affectedQuantityExact:String(row.saleQuantity||0),purchaseDatasetId:report.activePurchaseLayerDatasetId||''});
        ...
        try{
          ...
          const labels={EXACT_OFFICIAL_PURCHASE_LAYER:'فاکتور خرید رسمی',OPENING_ACCOUNTING_COST:'قیمت میانگین موجودی ابتدای دوره',HISTORICAL_PURCHASE_AVERAGE:'میانگین موزون خریدهای معتبر',SOURCE_HISTORY_INCOMPLETE:'تاریخچه خرید هنوز کامل و بازبینی نشده',PENDING_PURCHASE_PRICE:'قیمت خرید موقت — قرنطینه',PURCHASE_RETURN_CONFLICT:'برگشت خرید حل‌نشده',NO_VALID_COST_BASIS:'بدون مأخذ پیشنهادی',CONFLICT_REQUIRES_REVIEW:'تعارض مأخذ — نیازمند بررسی'};
          const canDecide=['admin','accounting','purchase'].includes(userRole());
          const approvedOpening=suggestion.sourceClass==='OPENING_ACCOUNTING_COST'&&Boolean(suggestion.openingEvidenceDatasetId)&&suggestion.approvalStatus==='approved';
          const canFinalize=false&&canDecide&&suggestion.reviewOnly!==true&&suggestion.financialAuthority!==false&&!approvedOpening&&!['EXACT_OFFICIAL_PURCHASE_LAYER','SOURCE_HISTORY_INCOMPLETE','PENDING_PURCHASE_PRICE','PURCHASE_RETURN_CONFLICT','CONFLICT_REQUIRES_REVIEW'].includes(suggestion.sourceClass);
          const roundedSuggested=suggestion.suggestedCostExact?Math.round(Number(suggestion.suggestedCostExact)):null;
          const openingWarehouses=(suggestion.warehouseEvidence||[]).map(value=>`<tr><td>${safe(value.warehouseNumber)}</td><td>${safe(value.openingQuantityExact)}</td><td>${safe(value.openingTotalValueExact)}</td><td>${safe(value.openingUnitCostExact)}</td><td>${safe(value.evidenceQuality)}</td></tr>`).join('');
          const lineage=suggestion.reviewLineage||{};
          detail.innerHTML=`<div class="card"><div class="card-header"><h5>بررسی و تعیین هزینه — ${safe(row.itemCode)}</h5></div><div class="card-body"><div class="info">Sale Snapshot: <code>${safe(lineage.saleSnapshotId||report.activeSnapshotId||'')}</code> | Purchase Dataset: <code>${safe(lineage.purchaseDatasetId||report.activePurchaseLayerDatasetId||'')}</code> | Opening Dataset: <code>${safe(lineage.openingDatasetId||suggestion.openingEvidenceDatasetId||'')}</code> | Approval: ${safe(lineage.openingApprovalStatus||suggestion.approvalStatus||'')}</div>${suggestion.reviewOnly===true?'<div class="warn"><b>NOT YET APPROVED FOR FIFO</b> — فقط پیش‌نمایش بررسی انسانی؛ وضعیت مالی فعلی همچنان UNKNOWN است.</div>':''}${approvedOpening?'<div class="success"><b>APPROVED OPENING ACCOUNTING COST</b> — authority مستقل برای FIFO آینده؛ هیچ Manual Cost ساخته نمی‌شود.</div>':''}<div class="info"><small>قیمت پیشنهادی سیستم</small><br><b>${suggestion.available?number(roundedSuggested)+' ریال':'قیمت پیشنهادی قابل محاسبه نیست؛ مأخذ معتبر کافی وجود ندارد.'}</b>${suggestion.available&&String(suggestion.suggestedCostExact)!==String(roundedSuggested)?`<br><small>جزئیات دقیق: ${safe(suggestion.suggestedCostExact)} ریال</small>`:''}</div><div class="row four"><div><small>مأخذ پیشنهاد</small><br>${safe(labels[suggestion.sourceClass]||suggestion.sourceClass)}</div><div><small>قیمت واحد پیشنهادی</small><br>${safe(suggestion.suggestedCostExact||'—')}</div><div><small>مقدار قابل پوشش پیشنهادی</small><br>${safe(suggestion.eligibleTargetQuantityExact||'0.000000')}</div><div><small>کل مقدار نیازمند تعیین هزینه</small><br>${safe(suggestion.totalRequiredQuantityExact||row.saleQuantity)}</div><div><small>باقی‌مانده پس از شواهد پیشنهادی</small><br>${safe(suggestion.remainingUnknownQuantityExact||'0.000000')}</div><div><small>ارزش فروش در معرض</small><br>${number(row.saleAmount)} ریال</div><div><small>تاریخ مبنا</small><br>${safe(suggestion.effectiveOpeningDate||suggestion.dateTo||row.firstSaleDate)}</div><div><small>کیفیت مأخذ</small><br>${safe(suggestion.evidenceQuality||'UNPROVEN')}</div></div>${suggestion.sourceClass==='SOURCE_HISTORY_INCOMPLETE'?'<div class="warn">تکمیل تاریخچه رسمی این کالا اثبات نشده است. ابتدا Recovery محدود باید از مسیر Purchase Engine رسمی بازبینی شود؛ ورود هزینه دستی فعلاً مسدود است.</div>':''}${suggestion.sourceClass==='OPENING_ACCOUNTING_COST'?`<div class="info">مأخذ: ارزش حسابداری موجودی ابتدای دوره شایگان (نه فاکتور خرید)<br>Opening Qty: ${safe(suggestion.openingQuantityExact)} | Potentially Covered: ${safe(suggestion.eligibleTargetQuantityExact)} | Remaining Unresolved: ${safe(suggestion.remainingUnknownQuantityExact)} | Opening Unit Cost: ${safe(suggestion.openingUnitCostExact)} | Opening Total Value: ${safe(suggestion.openingTotalValueExact)} | Effective Date: ${safe(suggestion.effectiveOpeningDate)}<br>Evidence ID: <code>${safe(suggestion.evidenceId)}</code> | Dataset: <code>${safe(suggestion.openingEvidenceDatasetId)}</code><br>Fingerprint: <code>${safe(suggestion.sourceFingerprint)}</code> | Approval: ${safe(suggestion.approvalStatus||'not-approved')}</div><details><summary>جزئیات ۱۹ انبار (${safe(suggestion.queriedWarehouseCount)})</summary><table class="table"><thead><tr><th>انبار</th><th>مقدار</th><th>ارزش</th><th>هزینه واحد</th><th>وضعیت منبع</th></tr></thead><tbody>${openingWarehouses}</tbody></table></details>`:''}${suggestion.sourceClass==='HISTORICAL_PURCHASE_AVERAGE'?`<div class="warn">این پیشنهاد مأخذ item-level گسترده است و فقط تا سقف Quantity مستند قابل اعمال است.</div><div class="info">خرید معتبر: ${number(suggestion.purchaseCount)} | Quantity basis: ${safe(suggestion.quantityBasisExact)} | بازه: ${safe(suggestion.dateFrom)} تا ${safe(suggestion.dateTo)} | Min/Max/Latest: ${safe(suggestion.minPurchaseCostExact)} / ${safe(suggestion.maxPurchaseCostExact)} / ${safe(suggestion.latestPurchaseCostExact)}</div><details><summary>فاکتورها و تأمین‌کنندگان</summary><pre>${safe(JSON.stringify(suggestion.evidence||[],null,2))}</pre></details>`:''}${suggestion.sourceClass==='CONFLICT_REQUIRES_REVIEW'?`<div class="error">تعارض مبلغ مأخذها باید پیش از تصمیم انسانی رفع شود.</div><div class="info">مأخذ رسمی شایگان: تاریخ ${safe(suggestion.openingEvidence?.effectiveOpeningDate)} | مانده ${safe(suggestion.openingEvidence?.openingQuantityExact)} | هزینه ${safe(suggestion.openingEvidence?.openingUnitCostExact)} | ارزش ${safe(suggestion.openingEvidence?.openingTotalValueExact)}<br>Evidence: ${safe(suggestion.openingEvidence?.evidenceQuality)} | Fingerprint: <code>${safe(suggestion.openingEvidence?.sourceFingerprint)}</code></div><details><summary>Opening Inventory Evidence متعارض</summary><pre>${safe(JSON.stringify(suggestion.conflicts||[],null,2))}</pre></details>`:''}${canFinalize?`<div class="row four"><input id="mcAssistFrom" value="${safe(row.firstSaleDate||'')}" placeholder="شروع اثر"><input id="mcAssistTo" value="${safe(row.lastSaleDate||'')}" placeholder="پایان اثر"><input id="mcAssistCost" inputmode="decimal" value="${safe(suggestion.suggestedCostExact||'')}" placeholder="مبلغ نهایی"><input id="mcAssistReason" placeholder="دلیل Override/Manual"></div><button class="btn green" id="mcAssistAccept" ${suggestion.available?'':'disabled'}>تأیید مبلغ پیشنهادی</button> <button class="btn" id="mcAssistOverride">اصلاح/ورود مبلغ دستی</button>`:approvedOpening?'': '<div class="warn">این مأخذ قابل finalize نیست؛ مسیر اصلاح منبع یا رفع تعارض را انجام دهید.</div>'}${canDecide&&suggestion.reviewOnly!==true&&!approvedOpening?'<div><input id="mcAssistReason" placeholder="دلیل تعویق / نیازمند بررسی"><button class="mini" id="mcAssistDefer">تعویق / نیازمند بررسی</button></div>':''}</div></div>`;
          if(suggestion.eligibilityPreview?.rows?.length){const previewRows=suggestion.eligibilityPreview.rows.map(value=>`<tr><td>${safe(value.saleInvoiceNo)} / ${safe(value.saleRow)}</td><td>${safe(value.saleDate)}</td><td>${safe(value.unknownQuantityExact)}</td><td>${safe(value.openingEligibleQuantityExact)}</td><td>${safe(value.remainingUnknownQuantityExact)}</td><td>${safe(value.classification)}</td></tr>`).join('');detail.insertAdjacentHTML('beforeend',`<details open><summary>پیش‌نمایش خط‌به‌خط Opening (${safe(suggestion.eligibilityPreview.rows.length)})</summary><table class="table"><thead><tr><th>فاکتور / ردیف</th><th>تاریخ</th><th>نیازمند هزینه</th><th>قابل پوشش</th><th>Unknown</th><th>وضعیت</th></tr></thead><tbody>${previewRows}</tbody></table></details>`);}
          if(suggestion.openingReviewEvidence?.status==='NO_OPENING_STOCK')detail.insertAdjacentHTML('beforeend',`<div class="info"><b>Opening Evidence: NO_OPENING_STOCK</b><br>Dataset: <code>${safe(suggestion.openingReviewEvidence.datasetId)}</code> | Approval: ${safe(suggestion.openingReviewEvidence.approvalStatus)} | 19-Warehouse extraction: ${safe(suggestion.openingReviewEvidence.queriedWarehouseCount)} | Opening Qty: ${safe(suggestion.openingReviewEvidence.openingQuantityExact||'0.000000')}<br>این نتیجه فقط نبود موجودی ابتدای دوره را اثبات می‌کند؛ هزینه صفر ایجاد نمی‌کند.</div>`);
          const submit=async decision=>{const payload={decision,itemCode:row.itemCode,itemGuid:row.itemGuid,applicableDate:row.firstSaleDate,reviewDateTo:row.lastSaleDate,affectedQuantityExact:String(row.saleQuantity||0),affectedLineCount:Number(row.saleLineCount||0),saleValueExposure:Number(row.saleAmount||0),purchaseDatasetId:report.activePurchaseLayerDatasetId||'',effectiveFrom:q('#mcAssistFrom')?.value||row.firstSaleDate,effectiveTo:q('#mcAssistTo')?.value||row.lastSaleDate,finalCost:q('#mcAssistCost')?.value||'',reason:q('#mcAssistReason')?.value||''};await json('/api/manual-cost-resolutions/assisted/decisions',{method:'POST',body:JSON.stringify(payload)});detail.innerHTML='<div class="success">تصمیم governed ثبت شد؛ فقط FIFO Candidate بعدی stale است و Dataset فعال تغییر نکرد.</div>';await refreshAll();};
        ...
        }catch(error){if(requestId===suggestionRequest)detail.innerHTML=`<div class="error">${safe(error.message)}</div>`;}
      });
    }catch(error){box.innerHTML=`<div class="error">${safe(error.message)}</div>`;}
  }
  function actionButtons(row){
    if(row.status==='draft'&&['admin','accounting'].includes(userRole()))return `<button class="mini mc-edit" data-id="${safe(row.resolutionId)}">ویرایش</button> <button class="mini mc-action" data-action="submit" data-id="${safe(row.resolutionId)}" data-revision="${safe(row.revision)}">ارسال تأیید</button>`;
    if(row.status==='pending'&&['admin','manager'].includes(userRole()))return `<button class="mini mc-action" data-action="approve" data-id="${safe(row.resolutionId)}" data-revision="${safe(row.revision)}">تأیید</button> <button class="mini mc-action" data-action="reject" data-id="${safe(row.resolutionId)}" data-revision="${safe(row.revision)}">رد</button>`;
    if(row.status==='approved'&&['admin','manager'].includes(userRole()))return `<button class="mini mc-action" data-action="expire" data-id="${safe(row.resolutionId)}" data-revision="${safe(row.revision)}">منقضی‌کردن</button>`;
    return '—';
  }
  async function loadResolutions(){
    ...
    try{
      ...
      box.innerHTML=`<table class="table"><thead><tr><th>Resolution</th><th>کالا</th><th>هزینه</th><th>بازه</th><th>منبع</th><th>وضعیت</th><th>ایجادکننده / تأییدکننده</th><th>Audit</th><th></th></tr></thead><tbody>${(report.list||[]).map(row=>`<tr>
        ...
        <td>${safe(row.manualCostExact||row.manualCost)} ${safe(row.currency)}</td><td>${safe(row.effectiveFrom)} تا ${safe(row.effectiveTo||'باز')}</td>
        <td><b>${safe(manualEvidenceLabel(row))}</b>${row.supersedesResolutionId?`<br><small>جایگزین: ${safe(row.supersedesResolutionId)}</small>`:''}<details><summary>جزئیات فنی</summary><small>${safe(row.sourceType)} / ${safe(row.resolutionScope||'item')} ${safe(row.purchaseLineIdentity||'')}</small></details></td><td>${safe(row.status)}</td>
        ...
        <td>${actionButtons(row)} <button class="mini mc-preview" data-id="${safe(row.resolutionId)}">Impact Preview</button></td></tr>`).join('')||'<tr><td colspan="9">Resolution ثبت نشده است.</td></tr>'}</tbody></table>`;
      document.querySelectorAll('.mc-edit').forEach(button=>button.onclick=async()=>{
        ...
        q('#mcScope').value=row.resolutionScope||'item';q('#mcPurchaseDataset').value=row.purchaseDatasetId||'';q('#mcPurchaseLine').value=row.purchaseLineIdentity||'';q('#mcTargetQuantity').value=row.targetQuantityExact||'';
        q('#mcFrom').value=row.effectiveFrom||'';q('#mcTo').value=row.effectiveTo||'';q('#mcSource').value=row.sourceType||'manual';
        q('#mcReason').value=row.reason||'';q('#mcNotes').value=row.notes||'';q('#mcAttachment').value=row.attachment?.reference||'';q('#mcSupersedes').value=row.supersedesResolutionId||'';
        if(q('#mcCommercialReference'))q('#mcCommercialReference').value=row.commercialReference||'';
        q('#mcItemCode').focus();
      });
      document.querySelectorAll('.mc-action').forEach(button=>button.onclick=async()=>{
        ...
        try{
          ...
          if(button.dataset.action==='approve'){
            const preview=await json(`/api/manual-cost-resolutions/${encodeURIComponent(button.dataset.id)}/impact-preview`);
            previewFingerprint=preview.previewFingerprint||'';manualCostPreviewFingerprints.set(button.dataset.id,previewFingerprint);
            if(!confirm(`قیمت اعلام بازرگانی: ${preview.evidenceType||'—'}\nردیف‌های فروش تحت اثر: ${preview.affected?.saleLines||0}\nمقدار قابل پوشش: ${preview.quantity?.coveredExact||0}\nActive FIFO تغییر نمی‌کند. Approval را ثبت می‌کنید؟`))return;
          }
          await json(`/api/manual-cost-resolutions/${encodeURIComponent(button.dataset.id)}/${button.dataset.action}`,{method:'POST',body:JSON.stringify({reason,revision:Number(button.dataset.revision||0),previewFingerprint})});
          await refreshAll();
        }catch(error){q('#mcFormMsg').innerHTML=`<div class="error">${safe(error.message)}</div>`;}
      });
      document.querySelectorAll('.mc-preview').forEach(button=>button.onclick=async()=>{try{const report=await json(`/api/manual-cost-resolutions/${encodeURIComponent(button.dataset.id)}/impact-preview`);const evidence=report.evidence||{};const affectedRows=(report.affectedLines||[]).map(row=>`<tr><td>${safe(row.saleInvoiceNo)}</td><td><small>${safe(row.saleLineId)}</small></td><td>${safe(row.saleDate)}</td><td>${safe(row.sellerName||row.sellerAccountNumber)}</td><td>${safe(row.storeName)}</td><td>${safe(row.potentiallyCoveredQuantityExact)}</td><td>${safe(row.currentProvenance)}</td><td>${safe(row.currentFifoCostExact)} ریال</td><td>${safe(row.proposedCostExact)} ریال</td><td>${safe(row.expectedNextFifoProvenance)}</td></tr>`).join('');q('#mcFormMsg').innerHTML=`<div class="info"><b>پیش‌نمایش اثر — ${safe(report.evidenceType)}</b><div class="row four"><div>کالا: <b>${safe(evidence.itemCode)}</b><br><small>${safe(evidence.itemGuid)}</small></div><div>قیمت: <b>${safe(evidence.manualCostExact)} ریال</b></div><div>مقدار مصوب: <b>${safe(evidence.targetQuantityExact)}</b></div><div>بازه اثر: <b>${safe(evidence.effectiveFrom)} تا ${safe(evidence.effectiveTo)}</b></div><div>فاکتورهای تحت تأثیر: <b>${number(report.affected?.invoices)}</b></div><div>ردیف‌های فروش: <b>${number(report.affected?.saleLines)}</b></div><div>فروشندگان: <b>${number(report.affected?.sellers)}</b></div><div>کل مقدار نیازمند هزینه: <b>${safe(report.quantity?.requiredExact)}</b></div><div>مقدار قابل پوشش: <b>${safe(report.quantity?.coveredExact)}</b></div><div>مقدار باقی‌مانده Unknown: <b>${safe(report.quantity?.remainingUnknownExact)}</b></div><div>Sale Value Exposure: <b>${safe(report.saleValueExposureExact)} ریال</b></div><div>Projected resolved cost: <b>${safe(report.projectedResolvedCostExact)} ریال</b></div></div><div class="table-wrap"><table class="table"><thead><tr><th>فاکتور</th><th>Sale Line</th><th>تاریخ</th><th>فروشنده</th><th>فروشگاه</th><th>مقدار قابل پوشش</th><th>وضعیت فعلی</th><th>هزینه فعلی FIFO</th><th>هزینه پیشنهادی</th><th>وضعیت پس از Build</th></tr></thead><tbody>${affectedRows||'<tr><td colspan="10">هیچ ردیف فروش واجد شرایطی وجود ندارد.</td></tr>'}</tbody></table></div><div class="warn">FIFO فعال بدون تغییر می‌ماند؛ اعمال این Evidence نیازمند FIFO Candidate جدید است.</div><details><summary>جزئیات فنی</summary><pre>${safe(JSON.stringify(report,null,2))}</pre></details></div>`;}catch(error){q('#mcFormMsg').innerHTML=`<div class="error">${safe(error.message)}</div>`;}});
    }catch(error){box.innerHTML=`<div class="error">${safe(error.message)}</div>`;}
  }
  ...
  async function loadOpeningCandidates(){
    ...
    try{
      const report=await json('/api/accounting/opening-accounting-evidence/candidates?limit=20');
      box.innerHTML=`<div class="warn">شواهد پیش از Approval فقط برای Review و Impact Preview قابل مشاهده‌اند و هرگز مستقیماً FIFO را تغییر نمی‌دهند. Submit و Approval دو رخداد حسابرسی مستقل هستند؛ self-approval فقط با تأیید صریح Management ثبت می‌شود.</div><table class="table"><thead><tr><th>Dataset</th><th>تاریخ مبنا</th><th>استخراج</th><th>Approval</th><th>کالا</th><th>Source Calls</th><th>Fingerprint</th><th></th></tr></thead><tbody>${(report.list||[]).map(row=>`<tr><td><code>${safe(row.datasetId)}</code></td><td>${safe(row.openingDate)}</td><td>${safe(row.status)}</td><td><b>${safe(openingStatus(row.approvalStatus))}</b><br><small>Revision ${safe(row.revision||1)}</small></td><td>${number(row.completedItemCount||0)} / ${number(row.itemCount||0)}<br><small>Incomplete: ${number(row.incompleteItemCount||0)}</small></td><td>${number(row.sourceCallCount||0)}</td><td><code>${safe((row.datasetFingerprint||'').slice(0,16))}</code></td><td><button class="mini mc-opening-detail" data-id="${safe(row.datasetId)}">جزئیات</button> ${['incomplete','failed'].includes(row.status)&&['admin','accounting'].includes(userRole())?`<button class="mini mc-opening-action" data-action="resume" data-id="${safe(row.datasetId)}">Resume</button>`:''} ${row.approvalStatus==='validated'&&['admin','accounting','purchase'].includes(userRole())?`<button class="mini mc-opening-action" data-action="submit" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">ارسال برای تأیید</button>`:''} ${row.approvalStatus==='pending'&&['admin','accounting','manager','purchase'].includes(userRole())?`<button class="mini mc-opening-action" data-action="approve" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">تأیید حاکمیتی</button> <button class="mini mc-opening-action" data-action="reject" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">رد</button> <button class="mini mc-opening-action" data-action="defer" data-id="${safe(row.datasetId)}" data-revision="${safe(row.revision||1)}">تعویق</button>`:''}</td></tr>`).join('')||'<tr><td colspan="8">Dataset موجود نیست.</td></tr>'}</tbody></table>`;
      document.querySelectorAll('.mc-opening-detail').forEach(button=>button.onclick=async()=>{try{const detail=await json(`/api/accounting/opening-accounting-evidence/candidates/${encodeURIComponent(button.dataset.id)}?limit=5000`),summary=detail.financialApprovalSummary||{};q('#mcOpeningDetail').innerHTML=`<div class="info"><b>Opening Accounting Evidence</b> | Dataset <code>${safe(detail.dataset.datasetId)}</code> | ${safe(openingStatus(detail.dataset.approvalStatus))} | Actor role ${safe(userRole())}</div><div class="row four"><div>Items: <b>${number(summary.itemCount||detail.rows.length)}</b></div><div>Total Opening Qty: <b>${safe(summary.totalOpeningQuantityExact||'—')}</b></div><div>Total Opening Value: <b>${safe(summary.totalOpeningAccountingValueExact||'—')} ریال</b></div><div>Affected Sale Exposure: <b>${safe(summary.affectedSaleExposureExact||detail.impactPreview?.saleExposureExact||'0')} ریال</b></div><div>Projected Line Coverage: <b>${safe(summary.projectedCoverage?.linePercent||'—')}%</b></div><div>Projected Qty Coverage: <b>${safe(summary.projectedCoverage?.quantityPercent||'—')}%</b></div><div>Projected Sale Coverage: <b>${safe(summary.projectedCoverage?.saleValuePercent||'—')}%</b></div><div>Approval: <b>${safe(openingStatus(detail.dataset.approvalStatus))}</b></div></div><div class="row four"><div>VALIDATED: <b>${number(detail.dataset.statusCounts?.VALIDATED_CANDIDATE||0)}</b></div><div>NO STOCK: <b>${number(detail.dataset.statusCounts?.NO_OPENING_STOCK||0)}</b></div><div>INCOMPLETE: <b>${number(detail.dataset.statusCounts?.INCOMPLETE_SOURCE||0)}</b></div><div>Fingerprint: <code>${safe(detail.dataset.datasetFingerprint||'')}</code></div></div><details open><summary>شواهد کالا</summary><table class="table"><thead><tr><th>کالا</th><th>وضعیت</th><th>Opening Qty</th><th>Unit Cost</th><th>Total</th><th>انبار</th></tr></thead><tbody>${(detail.rows||[]).map(row=>`<tr><td>${safe(row.itemCode)}<br><small>${safe(row.itemDescription)}</small></td><td>${safe(row.status)}</td><td>${safe(row.openingQuantityExact||'0')}</td><td>${safe(row.openingUnitCostExact||'0')}</td><td>${safe(row.openingTotalValueExact||'0')}</td><td>${number(row.queriedWarehouseCount||0)}</td></tr>`).join('')}</tbody></table></details><details><summary>Sale-line Eligibility / Impact Preview</summary><table class="table"><thead><tr><th>Invoice / Line</th><th>Date</th><th>Item</th><th>Unknown</th><th>Opening Eligible</th><th>Remaining</th><th>خرید قبلی / بعدی</th><th>Classification</th></tr></thead><tbody>${(detail.eligibility||[]).map(row=>`<tr><td>${safe(row.saleInvoiceNo)} / ${safe(row.saleRow)}</td><td>${safe(row.saleDate)}</td><td>${safe(row.itemCode)}</td><td>${safe(row.unknownQuantityExact)}</td><td>${safe(row.openingEligibleQuantityExact)}</td><td>${safe(row.remainingUnknownQuantityExact)}</td><td>${number(row.earlierOfficialPurchaseCount||0)} / ${number(row.laterPurchaseCount||0)}</td><td>${safe(row.classification)}</td></tr>`).join('')}</tbody></table></details><details><summary>Governance Audit</summary><pre>${safe(JSON.stringify(detail.approvalHistory||[],null,2))}</pre></details>`;}catch(error){q('#mcOpeningDetail').innerHTML=`<div class="error">${safe(error.message)}</div>`;}});
  ...
  window.pageManualCostResolution=async function(){
    setPage('رفع هزینه خرید نامشخص',`<main class="main-content">
      ...
      <div class="card"><div class="card-header"><h5 id="mcResolutionId">Resolution جدید</h5></div><div class="card-body">
        ...
        <div class="row four"><div class="form-group"><label>ItemCode</label><input id="mcItemCode"></div><div class="form-group"><label>ItemGuid پایدار</label><input id="mcItemGuid"></div><div class="form-group"><label>قیمت اعلام بازرگانی (ریال)</label><input id="mcCost" inputmode="decimal"></div><div class="form-group"><label>مقدار هدف</label><input id="mcTargetQuantity" inputmode="decimal"></div></div>
        <input id="mcScope" type="hidden" value="commercial_announced_quantity"><input id="mcSource" type="hidden" value="commercial_announced_cost"><input id="mcPurchaseDataset" type="hidden"><input id="mcPurchaseLine" type="hidden">
        <div class="row four"><div class="form-group"><label>شروع اثر</label><input id="mcFrom" placeholder="14050101"></div><div class="form-group"><label>پایان اثر (الزامی)</label><input id="mcTo" placeholder="14050131"></div><div class="form-group"><label>مرجع اعلام بازرگانی</label><input id="mcCommercialReference" placeholder="شماره نامه/تیکت/اعلام"></div><div class="form-group"><label>علت (الزامی)</label><input id="mcReason"></div></div>
        <div class="form-group"><label>مرجع پیوست</label><input id="mcAttachment" placeholder="شماره سند یا URL داخلی"></div>
        <div class="form-group"><label>Supersedes Resolution ID (فقط اصلاح governed؛ تاریخ شروع و پایان الزامی)</label><input id="mcSupersedes" placeholder="MCOST-..."></div>
      ...
      <div class="card"><div class="card-header"><h5>Opening Accounting Evidence — Review و Approval مستقل</h5></div><div class="card-body"><div id="mcOpeningCandidates"></div><div id="mcOpeningDetail" class="mt"></div></div></div>
    ...
    q('#mcClear').onclick=()=>{selectedResolutionId='';selectedResolutionRevision=0;q('#mcResolutionId').textContent='Resolution جدید';['#mcItemCode','#mcItemGuid','#mcCost','#mcPurchaseDataset','#mcPurchaseLine','#mcTargetQuantity','#mcFrom','#mcTo','#mcCommercialReference','#mcAttachment','#mcReason','#mcNotes','#mcSupersedes'].forEach(id=>{q(id).value='';});q('#mcScope').value='commercial_announced_quantity';q('#mcSource').value='commercial_announced_cost';};
    q('#mcSave').onclick=async()=>{
      const body={revision:selectedResolutionRevision,itemCode:q('#mcItemCode').value,itemGuid:q('#mcItemGuid').value,manualCost:q('#mcCost').value,resolutionScope:'commercial_announced_quantity',targetQuantityExact:q('#mcTargetQuantity').value,effectiveFrom:q('#mcFrom').value,effectiveTo:q('#mcTo').value,sourceType:'commercial_announced_cost',commercialReference:q('#mcCommercialReference').value,currency:'IRR',reason:q('#mcReason').value,notes:q('#mcNotes').value,supersedesResolutionId:q('#mcSupersedes').value,attachment:q('#mcAttachment').value?{reference:q('#mcAttachment').value}:null};
      try{
...
/* Phase B final registry — intentionally last. */
(()=>{
  const allowed=['admin','accounting','manager'];
  const nav=window.__phaseBFinancialNav||{};
  const renderers=window.__phaseBFinancialRenderers||{};
  const inheritedMenu=window.renderMenu||renderMenu;
  window.renderMenu=renderMenu=function(){inheritedMenu.apply(this,arguments);const menu=document.querySelector('#menu');if(!menu)return;for(const id of Object.keys(nav))menu.querySelectorAll(`[data-page="${id}"]`).forEach(node=>node.remove());if(!allowed.includes(userRole()))return;for(const[id,label]of Object.entries(nav)){const button=document.createElement('button');button.className='navbtn';button.dataset.page=id;button.textContent=label;button.onclick=event=>{event.preventDefault();location.hash=id;route();};menu.appendChild(button);}};
  const inheritedRoute=window.route||route;
  window.route=route=async function(){const current=location.hash.slice(1)||firstAllowedPage();if(renderers[current]&&allowed.includes(userRole()))return renderers[current]();return inheritedRoute.apply(this,arguments);};
  try{renderMenu();}catch{}
})();

/* Phase C final registry — upgrades the existing #seller-profit route only.
   All data is read from the materialized server-side projection. */
(()=>{
  const PAGE='seller-profit';
  const ALLOWED=['admin','accounting','manager','purchase'];
  const legacyPage=window.pageSellerProfit||(()=>setPage('عملکرد فروشندگان','<main class="main-content"><div class="warn">Read Model غیرفعال است.</div></main>'));
  const q=selector=>document.querySelector(selector);
  const safe=value=>esc(value==null?'':String(value));
  const toman=value=>value==null||value===''?'ناموجود':(Number(value)/10).toLocaleString('fa-IR',{maximumFractionDigits:2});
  const rialFromToman=value=>{const normalized=String(value||'').trim().replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d)).replace(/[,٬]/g,'');if(!/^-?\d+(?:\.\d+)?$/.test(normalized))return '';const negative=normalized.startsWith('-');const raw=negative?normalized.slice(1):normalized;const[whole,fraction='']=raw.split('.');const coefficient=BigInt((whole||'0')+fraction)*10n;const digits=coefficient.toString().padStart(fraction.length+1,'0');const result=fraction.length?`${digits.slice(0,-fraction.length)}.${digits.slice(-fraction.length)}`:digits;return `${negative?'-':''}${result}`;};
  const number=value=>Number(value||0).toLocaleString('fa-IR');
  const selected=id=>[...(q(id)?.selectedOptions||[])].map(option=>option.value).filter(Boolean).join(',');
  const SELECTED_RUN_KEY='mkcrm-seller-financial-selected-run';
  let page=1;
  let statusCache=null;
  let buildContextCache=null;
  let selectedRunId='';
  let selectedRunMeta=null;
  let activeRenderGeneration=0;
  let freshnessGeneration=0;
  const renderAlive=()=>window.__sellerFinancialRenderGeneration===activeRenderGeneration&&location.hash.slice(1)===PAGE;
  const isCurrent=()=>renderAlive()&&Boolean(q('#sfRun'));
  const selectedReadRun=()=>q('#sfRun')?.value||selectedRunId||'';
  function rememberSelectedRun(runId){selectedRunId=String(runId||'').trim();try{if(selectedRunId)sessionStorage.setItem(SELECTED_RUN_KEY,selectedRunId);else sessionStorage.removeItem(SELECTED_RUN_KEY);}catch{}}
  function rememberedSelectedRun(){try{return String(sessionStorage.getItem(SELECTED_RUN_KEY)||'').trim();}catch{return '';}}
  function runScopedUrl(url){const runId=selectedReadRun();if(!runId)return url;return `${url}${url.includes('?')?'&':'?'}runId=${encodeURIComponent(runId)}`;}
  async function api(url,options={}){const response=await fetch(url,{credentials:'include',headers:{Accept:'application/json','Content-Type':'application/json'},cache:'no-store',...options});const payload=await response.json().catch(()=>({ok:false,error:'پاسخ نامعتبر سرور'}));if(!response.ok||payload.ok===false){const error=new Error(payload.error||response.statusText);error.code=payload.code||'';error.status=response.status;throw error;}return payload;}
  function params(){const p=new URLSearchParams({runId:selectedReadRun(),page:String(page),pageSize:q('#sfPageSize')?.value||'50',dateFrom:q('#sfFrom')?.value||'',dateTo:q('#sfTo')?.value||'',sellerIdentity:selected('#sfSeller'),categoryGuid:selected('#sfCategory'),ratePool:selected('#sfPool'),store:selected('#sfStore'),costStatus:selected('#sfCost'),provenanceStatus:selected('#sfProvenance'),invoiceNumber:q('#sfInvoice')?.value||'',itemSearch:q('#sfItem')?.value||'',policyAvailability:q('#sfPolicyAvailability')?.value||'',categoryAvailability:q('#sfCategoryAvailability')?.value||'',rateAvailability:q('#sfRateAvailability')?.value||'',hasApprovedAdjustment:q('#sfHasAdjustment')?.value||'',hasSavedProfitCredit:q('#sfHasCredit')?.value||'',hasSavedProfitSubsidy:q('#sfHasSubsidy')?.value||'',sortBy:q('#sfSort')?.value||'date',sortDirection:q('#sfDirection')?.value||'desc'});for(const[id,key]of [['#sfInvoiceMin','invoiceAmountMin'],['#sfInvoiceMax','invoiceAmountMax'],['#sfLineMin','lineAmountMin'],['#sfLineMax','lineAmountMax'],['#sfProfitMin','fifoProfitMin'],['#sfProfitMax','fifoProfitMax'],['#sfCommissionableMin','commissionableProfitMin'],['#sfCommissionableMax','commissionableProfitMax'],['#sfDraftMin','draftCommissionMin'],['#sfDraftMax','draftCommissionMax']]){const value=rialFromToman(q(id)?.value);if(value)p.set(key,value);}for(const[key,value]of [...p.entries()])if(!value)p.delete(key);return p;}
  function optionRows(rows,valueKey='identity',nameKey='name'){return (rows||[]).map(row=>typeof row==='string'?`<option value="${safe(row)}">${safe(row)}</option>`:`<option value="${safe(row[valueKey])}">${safe(row[nameKey]||row[valueKey])}</option>`).join('');}
  function shell(){setPage('عملکرد فروشنده',`<main class="main-content">
    <div id="sfFifoFreshness"><div class="info">در حال بررسی تازگی FIFO و Seller Financial…</div></div>
    <div class="warn"><b>Accounting Read Model — PRELIMINARY / NON-PAYABLE</b><br>سود واقعی FIFO تغییرناپذیر است. مبلغ ناموجود هرگز صفر تلقی نمی‌شود و این صفحه برای فروشنده قابل مشاهده نیست.</div>
    <div class="actions"><button class="btn" id="sfApply">اعمال فیلتر</button><select id="sfView"><option value="lines">نمای ردیف فروش</option><option value="invoices">نمای فاکتور</option></select><button class="btn" id="sfSavePreset">ذخیره Preset</button><select id="sfPreset"><option value="">Preset ذخیره‌شده</option></select><button class="mini" id="sfLoadPreset">بارگذاری</button><button class="mini" id="sfFreshness">بررسی سریع تازگی</button>${['admin','accounting'].includes(userRole())?'<button class="mini" id="sfDeepVerify">اعتبارسنجی عمیق در پس‌زمینه</button><button class="btn green" id="sfCandidateBuild" disabled>ساخت Read Model کاندیدا</button>':''}</div>
    <div id="sfStatus" class="small muted"></div>
    <div class="card"><div class="card-header"><h5>Lineage فقط‌خواندنی ساخت کاندیدا</h5></div><div class="card-body"><div id="sfAuthorityStatus"></div><div id="sfBuildLineage"><div class="muted">در حال خواندن Active FIFO…</div></div></div></div>
    <div class="card"><div class="card-header"><h5>Read Model فقط‌خواندنی</h5></div><div class="card-body"><div class="row two"><div class="form-group"><label>Candidate / Active Run</label><select id="sfRun"><option value="">در حال خواندن Runها…</option></select></div><div id="sfSelectedRunStatus" class="info">SELECTED READ MODEL: none</div></div></div></div>
    <div class="card"><div class="card-header"><h5>فیلترهای server-side</h5></div><div class="card-body">
      <div class="row four"><div class="form-group"><label>از تاریخ شمسی</label><input id="sfFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ شمسی</label><input id="sfTo" value="14050431"></div><div class="form-group"><label>فروشنده (چندانتخابی)</label><select id="sfSeller" multiple></select></div><div class="form-group"><label>Product Category رسمی</label><select id="sfCategory" multiple></select></div></div>
      <div class="row four"><div class="form-group"><label>Commission Rate Pool</label><select id="sfPool" multiple></select></div><div class="form-group"><label>فروشگاه</label><select id="sfStore" multiple></select></div><div class="form-group"><label>وضعیت هزینه</label><select id="sfCost" multiple><option value="complete">complete</option><option value="partial">partial</option><option value="unknown">unknown</option></select></div><div class="form-group"><label>کد کالا</label><input id="sfItem"></div></div>
      <div class="row four"><div class="form-group"><label>Provenance سود</label><select id="sfProvenance" multiple><option value="PROVEN">PROVEN</option><option value="PARTIAL">PARTIAL</option><option value="UNKNOWN">UNKNOWN</option></select></div></div>
      <details><summary>فیلترهای پیشرفته</summary><div class="row four"><div class="form-group"><label>شماره فاکتور</label><input id="sfInvoice"></div><div class="form-group"><label>مبلغ فاکتور از (تومان)</label><input id="sfInvoiceMin" inputmode="decimal"></div><div class="form-group"><label>مبلغ فاکتور تا (تومان)</label><input id="sfInvoiceMax" inputmode="decimal"></div><div class="form-group"><label>مبلغ ردیف از/تا (تومان)</label><input id="sfLineMin" placeholder="از"><input id="sfLineMax" placeholder="تا"></div></div><div class="row four"><div class="form-group"><label>سود واقعی FIFO از/تا (تومان)</label><input id="sfProfitMin" placeholder="از"><input id="sfProfitMax" placeholder="تا"></div><div class="form-group"><label>سود مبنای پورسانت از/تا</label><input id="sfCommissionableMin" placeholder="از"><input id="sfCommissionableMax" placeholder="تا"></div><div class="form-group"><label>پورسانت آزمایشی از/تا</label><input id="sfDraftMin" placeholder="از"><input id="sfDraftMax" placeholder="تا"></div><div class="form-group"><label>Policy / Category / Rate</label><select id="sfPolicyAvailability"><option value="">Policy: همه</option><option value="available">موجود</option><option value="unavailable">ناموجود</option></select><select id="sfCategoryAvailability"><option value="">Category: همه</option><option value="available">موجود</option><option value="unavailable">ناموجود</option></select><select id="sfRateAvailability"><option value="">Rate: همه</option><option value="available">موجود</option><option value="unavailable">ناموجود</option></select></div></div><div class="row four"><div class="form-group"><label>تعدیل تأییدشده</label><select id="sfHasAdjustment"><option value="">همه</option><option value="true">دارد</option><option value="false">ندارد</option></select></div><div class="form-group"><label>سود ذخیره‌شده</label><select id="sfHasCredit"><option value="">همه</option><option value="true">دارد</option><option value="false">ندارد</option></select></div><div class="form-group"><label>سود مصرف‌شده</label><select id="sfHasSubsidy"><option value="">همه</option><option value="true">دارد</option><option value="false">ندارد</option></select></div><div class="form-group"><label>مرتب‌سازی / صفحه</label><select id="sfSort"><option value="date">تاریخ</option><option value="invoice">فاکتور</option><option value="sale">مبلغ فروش</option><option value="profit">سود FIFO</option></select><select id="sfDirection"><option value="desc">نزولی</option><option value="asc">صعودی</option></select><select id="sfPageSize"><option>25</option><option selected>50</option><option>100</option><option>250</option></select></div></div></details>
    </div></div>
    <div id="sfCards" style="display:flex;gap:10px;flex-wrap:wrap"></div>
    <div class="card"><div class="card-header"><h5 id="sfResultTitle">ردیف‌های مالی</h5></div><div class="card-body"><div id="sfLines"><div class="muted">در حال بارگذاری…</div></div><div class="actions"><button class="mini" id="sfPrev">قبلی</button><span id="sfPaging"></span><button class="mini" id="sfNext">بعدی</button></div></div></div>
    <div class="card"><div class="card-header"><h5>Drill-down حسابرسی</h5></div><div class="card-body"><div id="sfDrill"><div class="muted">یک ردیف را انتخاب کنید.</div></div></div></div>
    <div class="card"><div class="card-header"><h5>Run history و منبع فعال</h5></div><div class="card-body"><div id="sfRuns"></div></div></div>
  </main>`);}
  function card(label,value,note=''){return `<div class="info" style="min-width:160px"><b>${safe(label)}</b><br>${safe(value)}<br><small>${safe(note)}</small></div>`;}
  function exactDisplay(value,{unit='',empty='ناموجود'}={}){
    if(value==null||value==='')return safe(empty);
    const raw=String(value).trim();const match=raw.match(/^(-?)(\d+)(\.\d+)?$/);
    if(!match)return safe(`${raw}${unit?` ${unit}`:''}`);
    const grouped=match[2].replace(/\B(?=(\d{3})+(?!\d))/g,'٬');
    const rendered=`${match[1]}${grouped}${match[3]||''}${unit?` ${unit}`:''}`;
    return `<span dir="ltr" style="unicode-bidi:isolate;white-space:nowrap;${match[1]?'color:#b91c1c;font-weight:700;':''}">${safe(rendered)}</span>`;
  }
  function auditSourceLabel(type){return ({OFFICIAL_PURCHASE_LAYER:'لایه رسمی خرید',MULTI_PURCHASE_LAYER:'چند لایه رسمی خرید',APPROVED_OPENING_ACCOUNTING_COST:'هزینه حسابداری موجودی افتتاحیه',OPENING_INVENTORY_EVIDENCE:'شواهد موجودی افتتاحیه',MANUAL_COST_PURCHASE_LAYER:'هزینه دستی مصوبِ لایه خرید',MANUAL_COST_OPENING_BASIS:'هزینه دستی مصوبِ مبنای افتتاحیه',MANUAL_COST_HISTORICAL_EVIDENCE:'شواهد تاریخی مصوب',MANUAL_COST_ITEM_LEGACY:'هزینه دستی قدیمی مصوب',UNKNOWN:'منبع هزینه اثبات‌نشده'})[type]||type||'ناموجود';}
  function auditStatus(status){const value=status||'UNKNOWN';const css=value==='PROVEN'?'success':value==='PARTIAL'?'warn':'error';return `<span class="${css}" style="display:inline-block;margin:0;padding:4px 8px"><b>${safe(value)}</b></span>`;}
  function auditField(label,value,{mono=false,html=false}={}){return `<div style="padding:8px;border:1px solid #e6e8eb;border-radius:6px;background:#fff"><div class="muted small">${safe(label)}</div><div${mono?' dir="ltr" style="font-family:monospace;overflow-wrap:anywhere;text-align:right"':''}>${html?value:safe(value==null||value===''?'—':value)}</div></div>`;}
  function auditGrid(fields){return `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:8px">${fields.join('')}</div>`;}
  function costSourceRows(sources){return (sources||[]).map(source=>{
    const reference=source.purchaseInvoiceNumber?`خرید ${source.purchaseInvoiceNumber}`:source.openingDatasetId?`Opening ${source.openingDatasetId}`:source.manualCostResolutionId?`Manual Cost ${source.manualCostResolutionId}`:'—';
    const date=source.purchaseInvoiceDate||source.openingBaseDate||'—';
    const evidence=source.purchaseLineIdentity||source.openingEvidenceId||source.manualCostResolutionId||source.allocationId||'—';
    return `<tr><td>${safe(auditSourceLabel(source.sourceType))}</td><td>${safe(reference)}</td><td>${safe(date)}</td><td>${safe(source.supplierName||source.supplierIdentity||'—')}</td><td>${exactDisplay(source.allocatedQty,{empty:'—'})}</td><td>${exactDisplay(source.unitCostExact,{unit:'ریال'})}</td><td>${exactDisplay(source.allocatedCostExact,{unit:'ریال'})}</td><td dir="ltr" style="font-family:monospace;overflow-wrap:anywhere">${safe(evidence)}</td></tr>`;
  }).join('');}
  function preserveSelections(id,html){const old=new Set([...q(id).selectedOptions].map(x=>x.value));q(id).innerHTML=html;[...q(id).options].forEach(option=>option.selected=old.has(option.value));}
  function showSelectedRun(){const box=q('#sfSelectedRunStatus');if(!box)return;const row=selectedRunMeta;if(!row){box.innerHTML='<b>SELECTED READ MODEL:</b> none';return;}const status=row.active?'ACTIVE':row.candidateOnly?'CANDIDATE / INACTIVE / NON-PAYABLE':String(row.activationStatus||row.status||'').toUpperCase();box.innerHTML=`<b>SELECTED READ MODEL:</b> ${safe(row.runId)}<br><b>Status:</b> ${safe(status)}`;}
  async function renderCanonicalSellerFreshness(){
    const box=q('#sfFifoFreshness');if(!box)return;
    const request=++freshnessGeneration;
    const consumer=selectedRunMeta||buildContextCache?.latestCandidate||null;
    const consumerFifoDatasetId=String(consumer?.sourceFifoDatasetId||'').trim();
    try{
      const value=await api('/api/accounting/fifo-shadow/freshness?consumerFifoDatasetId='+encodeURIComponent(consumerFifoDatasetId));
      if(!isCurrent()||request!==freshnessGeneration)return;
      const sellerStale=value.sellerFinancialStale===true;
      const status=sellerStale?'عملکرد فروشنده نیاز به به‌روزرسانی دارد':(value.fifoStatus||'نامشخص');
      box.innerHTML=`<div class="${sellerStale?'warn':'success'}"><div class="row three"><div><small>آخرین به‌روزرسانی FIFO</small><br><b>${safe(value.lastFifoUpdatedAt?toJalaliDisplay(value.lastFifoUpdatedAt):'—')}</b></div><div><small>محاسبات FIFO تا تاریخ</small><br><b>${safe(value.calculationCutoff||'—')}</b></div><div><small>وضعیت عملکرد فروشنده</small><br><b>${safe(status)}</b></div></div>${sellerStale?`<div class="warn" style="margin-top:10px"><b>هشدار مدیریتی:</b> Read Model انتخابی${consumer?.runId?` <code>${safe(consumer.runId)}</code>`:''} بر اساس FIFO <code>${safe(consumerFifoDatasetId||'نامشخص')}</code> است، اما Active FIFO فعلی <code>${safe(value.activeFifoDatasetId||'نامشخص')}</code> است.<br><b>اقدام بعدی:</b> از دکمه «ساخت Read Model کاندیدا» استفاده کنید. این اقدام فقط یک Candidate غیرفعال و غیرقابل‌پرداخت می‌سازد.</div>`:''}</div>`;
    }catch(error){if(isCurrent()&&request===freshnessGeneration)box.innerHTML=`<div class="warn">وضعیت تازگی FIFO و Seller Financial قابل دریافت نیست: ${safe(error.code||error.message)}</div>`;}
  }
  async function loadRunSelector(){const r=await api('/api/accounting/seller-financial-performance/runs?pageSize=100');if(!isCurrent())return r;const eligible=(r.list||[]).filter(row=>row.status==='completed');const stored=rememberedSelectedRun(),validStored=eligible.find(row=>row.runId===stored),active=eligible.find(row=>row.runId===r.activeRunId),latestCandidate=eligible.find(row=>row.candidateOnly===true&&row.activationStatus==='validated-candidate'),chosen=validStored||active||latestCandidate||eligible[0]||null;const control=q('#sfRun');control.innerHTML=eligible.map(row=>`<option value="${safe(row.runId)}">${safe(row.runId)} — FIFO ${safe(row.sourceFifoDatasetId)}</option>`).join('')||'<option value="">Read Model تکمیل‌شده‌ای وجود ندارد</option>';rememberSelectedRun(chosen?.runId||'');if(selectedRunId)control.value=selectedRunId;selectedRunMeta=eligible.find(row=>row.runId===selectedRunId)||null;showSelectedRun();control.onchange=async()=>{rememberSelectedRun(control.value);selectedRunMeta=eligible.find(row=>row.runId===selectedRunId)||null;showSelectedRun();page=1;await renderCanonicalSellerFreshness();await loadOptions();await refresh();};return r;}
  async function loadOptions(){const runId=selectedReadRun(),r=await api(runScopedUrl('/api/accounting/seller-financial-performance/filters'));if(!isCurrent()||runId!==selectedReadRun())return;preserveSelections('#sfSeller',optionRows(r.sellers));preserveSelections('#sfCategory',optionRows(r.categories,'guid','label'));preserveSelections('#sfPool',optionRows(r.ratePools));preserveSelections('#sfStore',optionRows(r.stores));}
  async function loadTotals(){const request=params(),requestKey=request.toString(),r=await api('/api/accounting/seller-financial-performance/totals?'+request);if(!isCurrent()||requestKey!==params().toString())return;q('#sfCards').innerHTML=[card('فاکتور',number(r.invoiceCount)),card('ردیف',number(r.lineCount),`${number(r.unavailableLineCount)} پورسانت ناموجود`),card('فروش ناخالص',toman(r.saleValueExact),'تومان'),card('سود FIFO اثبات‌شده',toman(r.provenFifoProfitExact),`${number(r.provenProfitCoveragePercent)}٪ پوشش ردیف؛ سود کل نیست`),card('سود پورسانتی',toman(r.commissionableProfitExact),'تومان'),card('پورسانت مقدماتی',toman(r.preliminaryCommissionExact),'تومان؛ غیرقابل پرداخت'),card('هزینه نامشخص',number(r.unknownCostLineCount),'ردیف')].join('');}
  function rowHtml(row){const unavailable=row.commissionStatus!=='available';return `<tr><td>${safe(row.saleDate)}<br>${safe(row.saleInvoiceNumber)}</td><td>${safe(row.sellerName||row.sellerIdentity)}<br><small>${safe(row.storeName)}</small></td><td>${safe(row.itemCode)}<br><small>${safe(row.itemDescription)}</small></td><td>${safe(row.officialProductCategoryName)}<br><small>${safe(row.commissionRatePool)}</small></td><td>${toman(row.grossSaleAmountExact)}<br><small>تخفیف ${toman(row.lineDiscountExact)} / ${safe(row.discountStatus)} / فروش خالص ${toman(row.netSaleAmountExact)}</small></td><td>${toman(row.fifoCostExact)}</td><td><b>${toman(row.actualFifoProfitExact)}</b><br><small>${safe(row.costCoverageStatus)}</small></td><td>${toman(row.approvedAdjustmentAmountExact)}<br><small>${safe(row.adjustmentEligibility)}: ${safe((row.adjustmentBlockers||[]).join('، '))}</small></td><td>${toman(row.commissionableProfitExact)}</td><td>${safe(row.commissionRateExact??'ناموجود')}<br><small>${safe(row.rateResolutionPrecedence)}</small></td><td class="${unavailable?'error':''}">${toman(row.draftCommissionExact)}<br><small>${safe((row.blockers||[]).join('، '))}</small></td><td><button class="mini sf-drill" data-line="${safe(row.saleLineIdentity)}">جزئیات</button></td></tr>`;}
  async function loadLines(){const box=q('#sfLines');if(!box)return;const request=params(),requestKey=request.toString();q('#sfResultTitle').textContent='ردیف‌های مالی';box.innerHTML='<div class="muted">در حال خواندن صفحه از Read Model…</div>';try{const r=await api('/api/accounting/seller-financial-performance/lines?'+request);if(!isCurrent()||requestKey!==params().toString())return;box.innerHTML=`<div style="overflow:auto"><table class="table"><thead><tr><th>تاریخ/فاکتور</th><th>فروشنده/فروشگاه</th><th>کالا</th><th>دسته/Pool</th><th>فروش ناخالص / تخفیف / فروش خالص (تومان)</th><th>بهای FIFO</th><th>سود واقعی FIFO</th><th>تعدیل تأییدشده / ذخیره</th><th>سود مبنای پورسانت</th><th>نرخ پورسانت / نسخه</th><th>پورسانت آزمایشی</th><th></th></tr></thead><tbody>${(r.list||[]).map(rowHtml).join('')||'<tr><td colspan="12">نتیجه‌ای وجود ندارد.</td></tr>'}</tbody></table></div>`;q('#sfPaging').textContent=`صفحه ${number(r.page)} — ${number(r.total)} ردیف`;q('#sfPrev').disabled=r.page<=1;q('#sfNext').disabled=r.page*r.pageSize>=r.total;box.querySelectorAll('.sf-drill').forEach(button=>button.onclick=()=>drill(button.dataset.line));}catch(error){if(!isCurrent())return;box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)} <button class="mini" id="sfRetryLines">تلاش مجدد</button></div>`;q('#sfRetryLines')&&(q('#sfRetryLines').onclick=loadLines);}}
  async function loadInvoices(){const box=q('#sfLines');if(!box)return;const request=params(),requestKey=request.toString();q('#sfResultTitle').textContent='نمای فاکتور';box.innerHTML='<div class="muted">در حال تجمیع server-side فاکتورها…</div>';try{const r=await api('/api/accounting/seller-financial-performance/invoices?'+request);if(!isCurrent()||requestKey!==params().toString())return;box.innerHTML=`<div style="overflow:auto"><table class="table"><thead><tr><th>تاریخ/فاکتور</th><th>فروشنده/فروشگاه</th><th>دسته‌ها/Pool</th><th>ردیف</th><th>فروش ناخالص</th><th>فروش خالص</th><th>سود واقعی FIFO</th><th>سود مبنای پورسانت</th><th>پورسانت آزمایشی</th><th></th></tr></thead><tbody>${(r.list||[]).map(row=>`<tr><td>${safe(row.saleDate)}<br>${safe(row.saleInvoiceNumber)}</td><td>${safe(row.sellerName||row.sellerIdentity)}<br>${safe(row.storeName)}</td><td>${safe((row.categories||[]).join('، '))}<br><small>${safe((row.ratePools||[]).join('، '))}</small></td><td>${number(row.lineCount)}<br><small>${number(row.unavailableLineCount)} ناموجود</small></td><td>${toman(row.grossSaleAmountExact)}</td><td>${toman(row.netSaleAmountExact)}</td><td>${toman(row.actualFifoProfitExact)}</td><td>${toman(row.commissionableProfitExact)}</td><td>${toman(row.preliminaryCommissionExact)}</td><td><button class="mini sf-invoice-lines" data-invoice="${safe(row.saleInvoiceIdentity)}">بازکردن ردیف‌ها</button></td></tr>`).join('')||'<tr><td colspan="10">نتیجه‌ای وجود ندارد.</td></tr>'}</tbody></table></div>`;q('#sfPaging').textContent=`صفحه ${number(r.page)} — ${number(r.total)} فاکتور`;q('#sfPrev').disabled=r.page<=1;q('#sfNext').disabled=r.page*r.pageSize>=r.total;box.querySelectorAll('.sf-invoice-lines').forEach(button=>button.onclick=()=>invoiceLines(button.dataset.invoice));}catch(error){if(!isCurrent())return;box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)} <button class="mini" id="sfRetryLines">تلاش مجدد</button></div>`;q('#sfRetryLines')&&(q('#sfRetryLines').onclick=loadInvoices);}}
  async function invoiceLines(invoice){const box=q('#sfDrill'),runId=selectedReadRun();if(!box)return;box.innerHTML='<div class="muted">در حال بارگذاری ردیف‌های فاکتور…</div>';try{const r=await api(runScopedUrl(`/api/accounting/seller-financial-performance/invoices/${encodeURIComponent(invoice)}/lines?pageSize=500`));if(!isCurrent()||runId!==selectedReadRun())return;box.innerHTML=`<div class="actions"><button class="mini" id="sfCloseDrill">بستن (Esc)</button></div><div class="info">Invoice ${safe(invoice)} — ${number(r.total)} line</div><div style="overflow:auto"><table class="table"><tbody>${(r.list||[]).map(rowHtml).join('')}</tbody></table></div>`;q('#sfCloseDrill').onclick=closeDrill;box.querySelectorAll('.sf-drill').forEach(button=>button.onclick=()=>drill(button.dataset.line));}catch(error){if(isCurrent())box.innerHTML=`<div class="error">${safe(error.message)}</div>`;}}
  function closeDrill(){const box=q('#sfDrill');if(box)box.innerHTML='<div class="muted">یک ردیف یا فاکتور را انتخاب کنید.</div>';}
  async function drill(line){
    const box=q('#sfDrill'),runId=selectedReadRun();if(!box)return;
    box.innerHTML='<div class="muted">در حال بارگذاری زنجیره حسابرسی…</div>';
    try{
      const r=await api(runScopedUrl(`/api/accounting/seller-financial-performance/lines/${encodeURIComponent(line)}/drilldown`));
      if(!isCurrent()||runId!==selectedReadRun())return;
      const audit=r.humanAudit;
      if(!audit?.readOnly){box.innerHTML='<div class="error">نمای حسابرسی انسانی از سرور دریافت نشد؛ برای جلوگیری از بازسازی مالی در مرورگر، نمایش متوقف شد.</div>';return;}
      const tx=audit.transaction||{},financial=audit.financial||{},coverage=audit.allocationCoverage||{},recon=audit.reconciliation||{},reversal=audit.returnReversal||{};
      const reasons=(financial.canonicalReasons||[]).length?`<div class="error"><b>${safe(financial.unavailableMessage||'دلیل شواهد ناقص')}</b><br>${(financial.canonicalReasons||[]).map(reason=>`<code dir="ltr">${safe(reason)}</code>`).join('، ')}</div>`:'';
      const sourceTable=`<div style="overflow:auto"><table class="table"><thead><tr><th>نوع منبع هزینه</th><th>سند خرید / Opening</th><th>تاریخ</th><th>تأمین‌کننده</th><th>مقدار تخصیص</th><th>هزینه واحد</th><th>هزینه تخصیص</th><th>شناسه شواهد</th></tr></thead><tbody>${costSourceRows(audit.costSources)||'<tr><td colspan="8">هیچ منبع هزینه‌ای ثبت نشده است.</td></tr>'}</tbody></table></div>`;
      const returnSection=reversal.isReturn?`<div class="card"><div class="card-header"><h5>برگشت / معکوس‌سازی فروش</h5></div><div class="card-body">${auditGrid([
        auditField('تراکنش اصلی',reversal.originalInvoiceNumber?`فاکتور ${reversal.originalInvoiceNumber}`:'نامشخص'),
        auditField('شناسه ردیف اصلی',reversal.originalSaleLineIdentity||'نامشخص',{mono:true}),
        auditField('تراکنش برگشتی',reversal.reversalInvoiceNumber?`فاکتور ${reversal.reversalInvoiceNumber}`:'نامشخص'),
        auditField('مقدار معکوس‌شده',exactDisplay(reversal.reversedQuantityExact,{empty:'ناموجود'}),{html:true}),
        auditField('اثر هزینه معکوس‌شده',exactDisplay(reversal.reversedCostExact,{unit:'ریال'}),{html:true}),
        auditField('اثر سود معکوس‌شده',exactDisplay(reversal.reversedProfitExact,{unit:'ریال'}),{html:true})
      ])}<div class="mt"><b>پیوند منبع:</b> ${(reversal.sources||[]).map(source=>`${safe(source.linkageSource||'نامشخص')} / ${safe(source.linkageQuality||'نامشخص')} / ${safe(source.reversedAllocationId||'—')}`).join('<br>')||safe(reversal.lineLinkage?.linkageSource||'نامشخص')}</div></div></div>`:'';
      const differenceCell=(value,within)=>`${exactDisplay(value,{unit:'ریال'})}${within===true?' <span class="success" style="display:inline-block;padding:2px 5px;margin:0">منطبق</span>':within===false?' <span class="error" style="display:inline-block;padding:2px 5px;margin:0">اختلاف</span>':''}`;
      const rawEvidence={runId:r.runId,fifoDatasetId:r.line.fifoDatasetId,saleSnapshotId:r.line.saleSnapshotId,lineFingerprint:r.line.lineFingerprint,provenanceFingerprint:r.line.provenanceFingerprint,policyVersionId:r.line.policyVersionId||null,categoryMappingId:r.line.categoryMappingId||null,rateVersionId:r.line.rateVersionId||null,...r.source};
      box.innerHTML=`
        <div class="actions"><button class="mini" id="sfCloseDrill">بستن (Esc)</button></div>
        <div class="info"><b>حسابرسی مالی فقط‌خواندنی</b> — Run <span dir="ltr" style="font-family:monospace">${safe(r.runId)}</span><br>تمام مبالغ این نما از DTO سرور و شواهد canonical موجود خوانده شده‌اند؛ محاسبه FIFO در مرورگر انجام نمی‌شود.</div>
        <div class="card"><div class="card-header"><h5>مشخصات تراکنش</h5></div><div class="card-body">${auditGrid([
          auditField('شماره فاکتور',tx.invoiceNumber),auditField('شناسه ردیف فروش',tx.saleLineIdentity,{mono:true}),auditField('کد کالا',tx.itemCode,{mono:true}),auditField('شرح کالا',tx.itemDescription),
          auditField('فروشنده',`${tx.sellerName||'—'}${tx.canonicalSellerId?` — ${tx.canonicalSellerId}`:''}`),auditField('فروشگاه',tx.storeName),auditField('تاریخ فروش',tx.saleDate),auditField('نوع تراکنش',tx.typeLabel),auditField('مقدار',exactDisplay(tx.quantityExact,{empty:'ناموجود'}),{html:true})
        ])}</div></div>
        <div class="card"><div class="card-header"><h5>نتیجه مالی</h5></div><div class="card-body">${auditGrid([
          auditField('اثر فروش',exactDisplay(financial.saleImpactExact,{unit:'ریال'}),{html:true}),auditField('بهای FIFO',exactDisplay(financial.fifoCostExact,{unit:'ریال'}),{html:true}),auditField('سود FIFO',exactDisplay(financial.fifoProfitExact,{unit:'ریال'}),{html:true}),auditField('حاشیه FIFO (نسبت)',financial.fifoMarginExact||'ناموجود'),auditField('وضعیت Provenance',auditStatus(financial.provenanceStatus),{html:true}),auditField('نوع منبع هزینه',auditSourceLabel(financial.costSourceType))
        ])}${financial.provenanceStatus==='UNKNOWN'&&!reasons?'<div class="error"><b>هزینه قابل اثبات نیست</b></div>':reasons}</div></div>
        <div class="card"><div class="card-header"><h5>پوشش تخصیص</h5></div><div class="card-body">${auditGrid([
          auditField('مقدار موردنیاز',exactDisplay(coverage.requiredQuantityExact,{empty:'ناموجود'}),{html:true}),auditField('مقدار اثبات‌شده',exactDisplay(coverage.provenQuantityExact,{empty:'۰'}),{html:true}),auditField('مقدار حل‌نشده',exactDisplay(coverage.unresolvedQuantityExact,{empty:'۰'}),{html:true}),auditField('هزینه بخش اثبات‌شده',exactDisplay(coverage.knownAllocationCostExact,{unit:'ریال'}),{html:true}),auditField('حفظ مقدار',coverage.quantityConserved?'بله':'خیر'),auditField('حفظ مبلغ',coverage.moneyConserved?'بله':'خیر')
        ])}</div></div>
        <div class="card"><div class="card-header"><h5>منابع هزینه</h5></div><div class="card-body">${sourceTable}</div></div>
        ${returnSection}
        <div class="card"><div class="card-header"><h5>تطبیق Seller Financial با FIFO</h5></div><div class="card-body"><div class="muted small">واحد: ریال — تلورانس گردکردن: ${safe(recon.roundingToleranceExact||'0.01')}</div><div style="overflow:auto"><table class="table"><thead><tr><th>مولفه</th><th>Seller Financial</th><th>FIFO</th><th>اختلاف</th></tr></thead><tbody>
          <tr><td>فروش</td><td>${exactDisplay(recon.sellerFinancialSaleExact,{unit:'ریال'})}</td><td>${exactDisplay(recon.fifoSaleImpactExact,{unit:'ریال'})}</td><td>${differenceCell(recon.saleDifferenceExact,recon.saleWithinTolerance)}</td></tr>
          <tr><td>هزینه</td><td>${exactDisplay(recon.sellerFinancialFifoCostExact,{unit:'ریال'})}</td><td>${exactDisplay(recon.fifoAllocationCostExact,{unit:'ریال'})}</td><td>${differenceCell(recon.costDifferenceExact,recon.costWithinTolerance)}</td></tr>
          <tr><td>سود</td><td>${exactDisplay(recon.sellerFinancialProfitExact,{unit:'ریال'})}</td><td>${exactDisplay(recon.expectedFifoProfitExact,{unit:'ریال'})}</td><td>${differenceCell(recon.profitDifferenceExact,recon.profitWithinTolerance)}</td></tr>
        </tbody></table></div></div></div>
        <details><summary><b>نمایش اطلاعات فنی خام</b></summary><div class="info">Seller Financial Run: ${safe(r.runId)}<br>FIFO Dataset: ${safe(r.line.fifoDatasetId)}<br>Sale Snapshot: ${safe(r.line.saleSnapshotId)}<br>Fingerprint: ${safe(r.line.lineFingerprint)}</div><pre dir="ltr" style="text-align:left;max-height:520px;overflow:auto;white-space:pre-wrap">${safe(JSON.stringify(rawEvidence,null,2))}</pre></details>`;
      q('#sfCloseDrill').onclick=closeDrill;
    }catch(error){if(isCurrent())box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}
  }
  async function loadRuns(){const r=await api('/api/accounting/seller-financial-performance/runs?pageSize=10');if(!isCurrent())return;q('#sfRuns').innerHTML=`<div class="info">Active Run: ${safe(r.activeRunId)}</div><div style="overflow:auto"><table class="table"><thead><tr><th>Run</th><th>وضعیت</th><th>منابع</th><th>ردیف/خلاصه</th><th>Retry/Resume</th><th>زمان</th></tr></thead><tbody>${(r.list||[]).map(row=>`<tr><td>${safe(row.runId)}${row.runId===r.activeRunId?' <b>ACTIVE</b>':''}</td><td>${safe(row.status)} / ${safe(row.activationStatus)}</td><td><small>FIFO ${safe(row.sourceFifoDatasetId)}<br>Sale ${safe(row.sourceSaleSnapshotId)}<br>Catalog ${safe(row.sourceCatalogRunId)}</small></td><td>${number(row.lineCount)} / ${number(row.summaryCount)}</td><td>${number(row.retryCount)} / ${number(row.resumeCount)}</td><td>${safe(row.completedAt||row.createdAt)}</td></tr>`).join('')}</tbody></table></div>`;}
  async function loadBuildContext(){const box=q('#sfBuildLineage'),authority=q('#sfAuthorityStatus'),button=q('#sfCandidateBuild');try{const context=await api('/api/accounting/seller-financial-performance/build-context');if(!isCurrent())return;buildContextCache=context;const f=context.activeFifo||{};authority.innerHTML=`<div class="info"><b>CURRENT ACTIVE SELLER FINANCIAL:</b> ${safe(context.currentActiveSellerFinancialId||'none')}<br><b>LATEST CANDIDATE:</b> ${safe(context.latestCandidate?.runId||'none')}</div>`;box.innerHTML=`<table class="table"><tbody><tr><th>Active FIFO</th><td>${safe(f.datasetId)}</td><th>Authority revision</th><td>${safe(f.authorityRevision)}</td></tr><tr><th>Sale Snapshot</th><td>${safe(f.saleSnapshotId)}</td><th>Purchase Dataset</th><td>${safe(f.purchaseDatasetId)}</td></tr><tr><th>Opening Dataset</th><td>${safe(f.openingDatasetId)}</td><th>نتیجه Build</th><td>Inactive / Candidate Only / Non-Payable</td></tr><tr><th>FIFO Source fingerprint</th><td colspan="3"><small>${safe(f.sourceFingerprint)}</small></td></tr><tr><th>FIFO Allocation fingerprint</th><td colspan="3"><small>${safe(f.allocationFingerprint)}</small></td></tr><tr><th>FIFO Candidate fingerprint</th><td colspan="3"><small>${safe(f.candidateFingerprint)}</small></td></tr><tr><th>Commission</th><td>No</td><th>Activation</th><td>Separate governed action</td></tr></tbody></table>`;if(button)button.disabled=!context.canBuild;}catch(error){if(!isCurrent())return;buildContextCache=null;authority.innerHTML='<div class="warn"><b>CURRENT ACTIVE SELLER FINANCIAL:</b> نامشخص</div>';box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}<br>ساخت کاندیدا تا رفع Active FIFO binding مسدود است.</div>`;if(button)button.disabled=true;}}
  async function refresh(){await Promise.all([loadTotals(),q('#sfView')?.value==='invoices'?loadInvoices():loadLines(),loadRuns()]);}
  function presetPayload(){const fields=['sfView','sfFrom','sfTo','sfInvoice','sfItem','sfInvoiceMin','sfInvoiceMax','sfLineMin','sfLineMax','sfProfitMin','sfProfitMax','sfCommissionableMin','sfCommissionableMax','sfDraftMin','sfDraftMax','sfPolicyAvailability','sfCategoryAvailability','sfRateAvailability','sfHasAdjustment','sfHasCredit','sfHasSubsidy','sfSort','sfDirection','sfPageSize'];const payload=Object.fromEntries(fields.map(id=>[id,q('#'+id)?.value||'']));for(const id of ['sfSeller','sfCategory','sfPool','sfStore','sfCost','sfProvenance'])payload[id]=[...q('#'+id).selectedOptions].map(x=>x.value);return payload;}
  function presets(){try{return JSON.parse(localStorage.getItem('mkcrm-seller-financial-presets')||'{}');}catch{return {};}}
  function renderPresets(){const all=presets();q('#sfPreset').innerHTML='<option value="">Preset ذخیره‌شده</option>'+Object.keys(all).sort().map(name=>`<option value="${safe(name)}">${safe(name)}</option>`).join('');}
  function applyPreset(value){for(const[id,stored]of Object.entries(value||{})){const control=q('#'+id);if(!control)continue;if(Array.isArray(stored))[...control.options].forEach(option=>option.selected=stored.includes(option.value));else control.value=stored;}}
  async function buildCandidate(){const context=buildContextCache,f=context?.activeFifo;if(!context?.canBuild||!f?.datasetId){q('#sfStatus').innerHTML='<div class="error">Active FIFO binding معتبر نیست؛ صفحه را Refresh کنید.</div>';return;}const confirmation=`ساخت Read Model کاندیدا\n\nActive FIFO:\n${f.datasetId}\n\nSale:\n${f.saleSnapshotId}\n\nPurchase:\n${f.purchaseDatasetId}\n\nOpening:\n${f.openingDatasetId}\n\nResult:\nInactive\nCandidate Only\nNon-Payable\n\nCommission:\nNo\n\nActivation:\nSeparate governed action\n\nآیا این lineage دقیقاً تأیید می‌شود؟`;if(!confirm(confirmation))return;q('#sfCandidateBuild').disabled=true;try{const r=await api('/api/accounting/seller-financial-performance/candidate-build',{method:'POST',body:JSON.stringify({fifoDatasetId:f.datasetId,expectedActiveFifoDatasetId:f.datasetId,expectedFifoAuthorityRevision:f.authorityRevision,expectedFifoSourceFingerprint:f.sourceFingerprint,expectedFifoAllocationFingerprint:f.allocationFingerprint,expectedFifoCandidateFingerprint:f.candidateFingerprint,batchSize:500,maxAttempts:3})});q('#sfStatus').innerHTML=`<div class="info">Job ${safe(r.jobId)} برای Candidate غیرفعال و غیرقابل پرداخت ثبت شد. هیچ Activation یا Commission انجام نشد.</div>`;}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;await loadBuildContext();}}
  async function pageRenderer(){
    activeRenderGeneration=(window.__sellerFinancialRenderGeneration||0)+1;window.__sellerFinancialRenderGeneration=activeRenderGeneration;
    if(!ALLOWED.includes(userRole())){setPage('عدم دسترسی','<main class="main-content"><div class="error">گزارش مالی فروشندگان برای نقش فروشنده قابل مشاهده نیست.</div></main>');return;}
    try{statusCache=await api('/api/accounting/seller-financial-performance/status');}catch(error){if(!renderAlive())return;if(error.code==='SELLER_FINANCIAL_READ_MODEL_DISABLED')return legacyPage();setPage('عملکرد مالی فروشندگان',`<main class="main-content"><div class="error">${safe(error.code)} ${safe(error.message)}</div></main>`);return;}
    if(!renderAlive())return;
    if(!statusCache.enabled)return legacyPage();shell();const recency=statusCache.activeRun?.sourceRecency||{};q('#sfStatus').textContent=`Run فعال: ${statusCache.activeRunId||'موجود نیست'} | داده مالی تا: ${recency.financialDataThrough||'نامشخص'} | داده فروش تا: ${recency.salesDataThrough||'نامشخص'} | ${statusCache.buildLocked?'در حال ساخت':'آماده'} | NON-PAYABLE`;renderPresets();
    q('#sfApply').onclick=()=>{page=1;refresh();};q('#sfView').onchange=()=>{page=1;refresh();};q('#sfPrev').onclick=()=>{page=Math.max(1,page-1);refresh();};q('#sfNext').onclick=()=>{page++;refresh();};q('#sfCandidateBuild')&&(q('#sfCandidateBuild').onclick=buildCandidate);
    q('#sfFreshness').onclick=async()=>{try{const r=await api('/api/accounting/seller-financial-performance/freshness');q('#sfStatus').innerHTML=`<div class="${r.stale?'warn':'success'}">${r.stale?'Read Model قدیمی است؛ اعتبارسنجی عمیق و سپس rebuild لازم است.':'شناسه‌ها و metadata منابع فعال یکسان است.'} (${number(r.durationMs)} ms)<br>داده مالی تا: ${safe(r.financialDataThrough||'نامشخص')} | داده فروش تا: ${safe(r.salesDataThrough||'نامشخص')}</div>`;}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.message)}</div>`;}};
    q('#sfDeepVerify')&&(q('#sfDeepVerify').onclick=async()=>{try{const r=await api('/api/accounting/seller-financial-performance/verify',{method:'POST',body:'{}'});q('#sfStatus').innerHTML=`<div class="info">اعتبارسنجی عمیق فقط‌خواندنی آغاز شد: ${safe(r.verificationId)} / Job ${safe(r.jobId)}</div>`;}catch(error){q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}});
    q('#sfSavePreset').onclick=()=>{const name=prompt('نام Preset:')||'';if(!name)return;const all=presets();all[name.slice(0,80)]=presetPayload();localStorage.setItem('mkcrm-seller-financial-presets',JSON.stringify(all));renderPresets();};q('#sfLoadPreset').onclick=()=>{const value=presets()[q('#sfPreset').value];if(value){applyPreset(value);page=1;refresh();}};
    if(window.__sellerFinancialEscapeHandler)document.removeEventListener('keydown',window.__sellerFinancialEscapeHandler);window.__sellerFinancialEscapeHandler=event=>{if(event.key==='Escape')closeDrill();};document.addEventListener('keydown',window.__sellerFinancialEscapeHandler);
    try{await Promise.all([loadBuildContext(),loadRunSelector()]);if(!isCurrent())return;await renderCanonicalSellerFreshness();if(selectedReadRun()){await loadOptions();await refresh();}else q('#sfStatus').innerHTML='<div class="warn">Read Model تکمیل‌شده‌ای برای مشاهده وجود ندارد.</div>';}catch(error){if(isCurrent())q('#sfStatus').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}
  }
  window.pageSellerProfit=pageRenderer;
  const inheritedMenu=window.renderMenu||renderMenu;
  window.renderMenu=renderMenu=function(){inheritedMenu.apply(this,arguments);const menu=q('#menu');if(!menu)return;if(!ALLOWED.includes(userRole())){menu.querySelectorAll(`[data-page="${PAGE}"]`).forEach(node=>node.remove());return;}const node=menu.querySelector(`[data-page="${PAGE}"]`);if(node)node.textContent='عملکرد مالی فروشندگان';};
  const inheritedRoute=window.route||route;
  window.route=route=async function(){const current=location.hash.slice(1)||firstAllowedPage();if(current===PAGE)return pageRenderer();window.__sellerFinancialRenderGeneration=(window.__sellerFinancialRenderGeneration||0)+1;return inheritedRoute.apply(this,arguments);};
  try{renderMenu();}catch{}
})();
 /* Financial Navigation and Policy Selection cleanup.
   Historical pages and hashes remain implemented, but the primary accounting
   workflow exposes one Persian entry per business capability. */
(()=>{
  const PRIMARY=[
    {section:'تنظیمات پورسانت',id:'commission-policy-governance',label:'سیاست‌های پورسانت',roles:['admin','accounting','manager']},
    {section:'تنظیمات پورسانت',id:'commission-category-governance',label:'تعریف پورسانت گروه کالا',roles:['admin','accounting','manager']},
    {section:'تنظیمات پورسانت',id:'commission-rate-governance',label:'نرخ‌های پورسانت',roles:['admin','accounting','manager']},
    {section:'گزارش و عملیات',id:'seller-profit',label:'عملکرد فروشنده',roles:['admin','accounting','manager','purchase']},
    {section:'گزارش و عملیات',id:'profit-adjustment-review',label:'تعدیلات سود مبنای پورسانت',roles:['admin','accounting','manager']},
    {section:'گزارش و عملیات',id:'saved-profit-ledger',label:'سودهای سیوشده',roles:['admin','accounting','manager']},
    {section:'گزارش و عملیات',id:'draft-commission-report',label:'گزارش پورسانت آزمایشی',roles:['admin','accounting','manager']},
    {section:'گزارش و عملیات',id:'accounting-excel-audit',label:'خروجی و ورود Excel',roles:['admin','accounting','manager']},
    {section:'گزارش و عملیات',id:'financial-data-health',label:'سلامت داده‌های مالی',roles:['admin','accounting','manager']}
  ];
  const ADVANCED=[
    {id:'fifo-profit-facts',label:'جزئیات سود FIFO',roles:['admin','manager']},
    {id:'accounting-review-workbench',label:'کاربرگ بررسی حسابداری',roles:['admin','manager']},
    {id:'accounting-fat',label:'پذیرش نهایی حسابداری',roles:['admin','manager']},
    {id:'tir-1405-reconstruction',label:'بازسازی تیر ۱۴۰۵',roles:['admin','manager']},
    {id:'supplier-incentive-ledger',label:'مشوق‌های تأمین‌کننده',roles:['admin']}
  ];
  const REDIRECTS={
    'commission-rate-versions':'commission-rate-governance',
    'accounting-fifo-readiness':'financial-data-health',
  ...
  window.__financialNavigationContract={primary:PRIMARY,advanced:ADVANCED,redirects:REDIRECTS,hiddenDirect:HIDDEN_DIRECT};
  function addButton(parent,row){const button=document.createElement('button');button.className='navbtn financial-navbtn';button.dataset.page=row.id;button.dataset.financialNavigation='true';button.textContent=row.label;button.onclick=event=>{event.preventDefault();location.hash=row.id;route();};parent.appendChild(button);}
  function addSection(menu,title,rows){if(!rows.length)return;const heading=document.createElement('div');heading.className='menu-group';heading.dataset.financialNavigation='true';heading.textContent=title;menu.appendChild(heading);rows.forEach(row=>addButton(menu,row));}
  const inheritedMenu=window.renderMenu||renderMenu;
  window.renderMenu=renderMenu=function(){
    inheritedMenu.apply(this,arguments);
    const menu=document.querySelector('#menu');if(!menu)return;
    menu.querySelectorAll('[data-financial-navigation="true"]').forEach(node=>node.remove());
    for(const id of ALL_IDS)menu.querySelectorAll(`[data-page="${id}"]`).forEach(node=>node.remove());
    const role=userRole();
    for(const section of ['تنظیمات پورسانت','گزارش و عملیات'])addSection(menu,section,PRIMARY.filter(row=>row.section===section&&row.roles.includes(role)));
    const advanced=ADVANCED.filter(row=>row.roles.includes(role));
    if(advanced.length){const details=document.createElement('details');details.className='financial-advanced-menu';details.dataset.financialNavigation='true';details.open=false;const summary=document.createElement('summary');summary.className='menu-group';summary.textContent='ابزارهای پیشرفته';details.appendChild(summary);advanced.forEach(row=>addButton(details,row));menu.appendChild(details);}
  };
  const inheritedRoute=window.route||route;
  window.route=route=async function(){
    const requested=location.hash.slice(1)||firstAllowedPage();
    const current=REDIRECTS[requested]||requested;
    const visible=[...PRIMARY,...ADVANCED].find(row=>row.id===current);
    const hiddenRoles=HIDDEN_DIRECT[current];
    if((visible&&!visible.roles.includes(userRole()))||(hiddenRoles&&!hiddenRoles.includes(userRole()))){location.hash=firstAllowedPage();return inheritedRoute.apply(this,arguments);}
    if(REDIRECTS[requested]){if(typeof history!=='undefined'&&history.replaceState)history.replaceState(null,'',`#${current}`);else location.hash=current;}
    if(current==='financial-data-health'){
      const renderer=window.__accountingGovernanceRenderers?.['commission-export-readiness']||window.pageAccountingFifoReadiness;
...
/* Financial operations navigation — final, single registry.
   This intentionally replaces only financial menu entries.  It does not alter
   inventory sync, sale issuance, FIFO facts, or any server authorization. */
(()=>{
  ...
  const MAIN=[
    ...
    {id:'saved-profit-ledger',label:'سودهای سیوشده'},
    {id:'supplier-aging',label:'خواب کالا و تأمین‌کننده'}
  ];
  const ADVANCED=[
    {id:'sale-snapshot',label:'عملیات Sale Snapshot'},
    {id:'manual-cost-resolution',label:'Manual Cost Resolution'},
    {id:'fifo-shadow-validation',label:'به‌روزرسانی FIFO'},
    {id:'fifo-audit',label:'ممیزی FIFO'},
    {id:'official-group-catalog',label:'کاتالوگ رسمی گروه‌های شایگان'},
    {id:'financial-data-health',label:'آمادگی و سلامت داده‌های مالی'},
    {id:'saved-profit-opening-governance',label:'مانده آغازین سودهای سیوشده'},
    {id:'accounting-review-workbench',label:'کاربرگ بررسی حسابداری'},
    {id:'accounting-fat',label:'پذیرش نهایی حسابداری'}
  ];
  const ALL=[...MAIN,...ADVANCED];
  const IDS=[...new Set([...ALL.map(row=>row.id),'commission-policy-governance','commission-rate-versions','accounting-fifo-readiness','commission-export-readiness','fifo-profit-facts','fifo-shadow-validation','draft-commission-report','accounting-excel-audit','tir-1405-reconstruction','supplier-incentive-ledger'])];
  const REDIRECTS={'commission-rate-versions':'commission-rate-governance','accounting-fifo-readiness':'financial-data-health','commission-export-readiness':'financial-data-health','fifo-profit-facts':'fifo-audit'};
  for(const role of ['accounting','manager']){
    ROLE_PAGES[role]=[...new Set([...(ROLE_PAGES[role]||[]),'official-group-catalog','fifo-audit','sale-snapshot','financial-data-health'])];
  }
  const safe=value=>esc(value==null?'':String(value));
  const api=async(url,options={})=>{const response=await fetch(url,{credentials:'include',headers:{Accept:'application/json','Content-Type':'application/json'},cache:'no-store',...options});const body=await response.json().catch(()=>({ok:false,error:'پاسخ نامعتبر'}));if(!response.ok||body.ok===false){const error=new Error(body.error||response.statusText);error.code=body.code||'';error.status=response.status;throw error;}return body;};
  const page=(title,body)=>setPage(title,`<main class="main-content">${body}</main>`);
  const navTo=id=>{location.hash=id;route();};

  async function officialCatalogPage(){
    if(!FINANCIAL_ROLES.includes(userRole()))return denied();
    page('کاتالوگ رسمی گروه‌های شایگان',`<div class="warn">این کاتالوگ فقط منبع مرجع انتخاب Product Category است. Refresh فقط ItemGroup/GetList و Item/Get رسمی را می‌خواند؛ هیچ Mapping یا تأییدی خودکار ایجاد نمی‌شود.</div><div class="actions">${['admin','accounting'].includes(userRole())?'<button class="btn" id="ogcRefresh">Refresh رسمی کاتالوگ (Read-only)</button>':''}<button class="mini" id="ogcReload">بازخوانی</button><button class="mini" id="ogcOpenMapping">تعریف پورسانت گروه کالا</button></div><div id="ogcBody" class="mt"><div class="muted">در حال خواندن وضعیت کاتالوگ…</div></div>`);
    const render=async()=>{const box=document.querySelector('#ogcBody');try{const data=await api('/api/accounting/governance/group-catalog?pageSize=500');const run=data.latestRun||{};const counts=data.counts||{};const rows=data.list||[];const summary=`<div class="info"><b>${data.catalogAvailable?'کاتالوگ قابل استفاده است':'کاتالوگ هنوز آماده نیست'}</b><br>آخرین Run: ${safe(run.catalogRunId||'—')} | زمان: ${safe(run.completedAt||run.fetchedAt||'—')} | وضعیت: ${safe(run.status||'—')}<br>Main Group: ${safe(counts.mainGroups||0)} | Child Group: ${safe(counts.childGroups||0)} | Item Assignment: ${safe(counts.itemAssignments||0)} | Orphan: ${safe(counts.orphans||0)} | Ambiguity: ${safe(counts.ambiguous||0)}</div>`;const table=rows.length?`<div style="overflow:auto"><table class="table"><thead><tr><th>GUID رسمی</th><th>شماره</th><th>نام گروه اصلی</th><th>زیرگروه</th><th>Item Assignment</th><th>وضعیت سلسله‌مراتب</th></tr></thead><tbody>${rows.map(row=>`<tr><td><small>${safe(row.officialProductCategoryGuid)}</small></td><td>${safe(row.officialProductCategoryNumber)}</td><td><b>${safe(row.officialProductCategoryName)}</b></td><td>${safe(row.childGroupCount)}</td><td>${safe(row.itemAssignmentCount)}</td><td>${safe(row.hierarchyStatus||row.validationStatus)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="warn">Catalog خالی است. صفر بودن Mappingها علت این وضعیت نیست. Accounting یا Admin باید ابتدا Refresh رسمی کاتالوگ را اجرا و نتیجه را بررسی کند.</div>';box.innerHTML=summary+table;}catch(error){box.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)} <button class="mini" id="ogcRetry">تلاش دوباره</button></div>`;box.querySelector('#ogcRetry')?.addEventListener('click',render);}};
    document.querySelector('#ogcReload').onclick=render;document.querySelector('#ogcOpenMapping').onclick=()=>navTo('commission-category-governance');
    document.querySelector('#ogcRefresh')?.addEventListener('click',async()=>{const button=document.querySelector('#ogcRefresh');button.disabled=true;try{await api('/api/accounting/governance/group-catalog/refresh',{method:'POST',body:JSON.stringify({pageSize:100,maxGroupPages:100,maxPages:1000})});await render();}catch(error){document.querySelector('#ogcBody').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}finally{button.disabled=false;}});
    await render();
  }
  async function fifoAuditPage(){
    if(!FINANCIAL_ROLES.includes(userRole()))return denied();
    page('ممیزی FIFO',`<div id="fifoAuditFreshness"></div><div class="warn"><b>READ-ONLY FIFO AUDIT — NOT PAYROLL APPROVED</b><br>واحد همه مبالغ این صفحه ریال است. سود Partial/Unknown هرگز به‌عنوان سود کامل نمایش داده نمی‌شود.</div><div id="faReliability" class="muted">در حال خواندن کنترل‌های اتکا…</div><div class="card"><div class="card-body"><div class="row four"><div class="form-group"><label>از تاریخ شمسی</label><input id="faFrom" value="14050401"></div><div class="form-group"><label>تا تاریخ شمسی</label><input id="faTo" value="14050431"></div><div class="form-group"><label>شماره فاکتور فروش</label><input id="faInvoice"></div><div class="form-group"><label>کد کالا</label><input id="faItem"></div></div><div class="row four"><div class="form-group"><label>وضعیت هزینه</label><select id="faCost"><option value="">همه</option><option value="complete">Complete</option><option value="partial">Partial</option><option value="unknown">Unknown</option></select></div><div class="form-group"><label>فروشنده</label><input id="faSeller"></div><div class="form-group"><label>دسته رسمی کالا</label><input id="faCategory"></div><div class="form-group"><label>Rate Pool</label><input id="faPool"></div></div><div class="row four"><div class="form-group"><label>فروش ردیف حداقل/حداکثر (ریال)</label><input id="faSaleMin" placeholder="حداقل"><input id="faSaleMax" placeholder="حداکثر"></div><div class="form-group"><label>سود FIFO حداقل/حداکثر (ریال)</label><input id="faProfitMin" placeholder="حداقل"><input id="faProfitMax" placeholder="حداکثر"></div><div class="form-group"><label>Margin حداقل/حداکثر (0..1)</label><input id="faMarginMin" placeholder="حداقل"><input id="faMarginMax" placeholder="حداکثر"></div><div class="form-group"><label>فاکتور خرید / تأمین‌کننده</label><input id="faPurchaseInvoice" placeholder="شماره خرید"><input id="faSupplier" placeholder="نام یا حساب تأمین‌کننده"></div></div><button class="btn" id="faSearch">جست‌وجوی Read-only</button></div></div><div id="faResult"><div class="muted">فیلتر را اعمال کنید.</div></div><div id="faDrill" class="mt"></div>`);
    window.renderCanonicalFifoFreshness('#fifoAuditFreshness');
    const result=document.querySelector('#faResult'),drill=document.querySelector('#faDrill');
    const load=async()=>{result.innerHTML='<div class="muted">در حال خواندن FIFO facts…</div>';const query=new URLSearchParams({page:'1',pageSize:'100',dateFrom:document.querySelector('#faFrom').value,dateTo:document.querySelector('#faTo').value,invoiceNumber:document.querySelector('#faInvoice').value,itemSearch:document.querySelector('#faItem').value,sellerIdentity:document.querySelector('#faSeller').value,category:document.querySelector('#faCategory').value,ratePool:document.querySelector('#faPool').value,costStatus:document.querySelector('#faCost').value,lineAmountMin:document.querySelector('#faSaleMin').value,lineAmountMax:document.querySelector('#faSaleMax').value,fifoProfitMin:document.querySelector('#faProfitMin').value,fifoProfitMax:document.querySelector('#faProfitMax').value,marginMin:document.querySelector('#faMarginMin').value,marginMax:document.querySelector('#faMarginMax').value,purchaseInvoiceNumber:document.querySelector('#faPurchaseInvoice').value,supplier:document.querySelector('#faSupplier').value});for(const [key,value]of [...query.entries()])if(!value)query.delete(key);try{const data=await api('/api/accounting/seller-financial-performance/lines?'+query);result.innerHTML=`<div class="info">${safe(data.total||0)} ردیف؛ مبالغ ریال؛ هزینه نامشخص صفر نیست.</div><div style="overflow:auto"><table class="table"><thead><tr><th>فروش</th><th>فروشنده</th><th>کالا / دسته</th><th>Quantity</th><th>Sale (IRR)</th><th>FIFO Cost (IRR)</th><th>Profit / Margin</th><th>Coverage</th><th></th></tr></thead><tbody>${(data.list||[]).map(row=>`<tr><td>${safe(row.saleDate)}<br>${safe(row.saleInvoiceNumber)}</td><td>${safe(row.sellerName||row.sellerIdentity)}</td><td>${safe(row.itemCode)}<br><small>${safe(row.officialProductCategoryName)} / ${safe(row.commissionRatePool)}</small></td><td>${safe(row.quantityExact)}</td><td>${safe(row.grossSaleAmountExact)}</td><td>${safe(row.fifoCostExact??'UNKNOWN')}</td><td>${safe(row.actualFifoProfitExact??'UNAVAILABLE')}<br><small>${safe(row.fifoMarginExact??'UNAVAILABLE')}</small></td><td>${safe(row.costCoverageStatus)}</td><td><button class="mini faDrill" data-line="${safe(row.saleLineIdentity)}">Drill-down</button></td></tr>`).join('')||'<tr><td colspan="9">نتیجه‌ای وجود ندارد.</td></tr>'}</tbody></table></div>`;result.querySelectorAll('.faDrill').forEach(button=>button.onclick=()=>loadDrill(button.dataset.line));}catch(error){result.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}};
    const loadDrill=async line=>{drill.innerHTML='<div class="muted">در حال خواندن زنجیره Allocation…</div>';try{const data=await api(`/api/accounting/seller-financial-performance/lines/${encodeURIComponent(line)}/drilldown`);const sources=(data.source.costProvenance||data.source.allocations||[]);drill.innerHTML=`<div class="card"><div class="card-header"><h5>FIFO Drill-down — ${safe(data.line.saleLineIdentity)}</h5></div><div class="card-body"><div class="info">FIFO Dataset: ${safe(data.line.fifoDatasetId)} | Sale Snapshot: ${safe(data.line.saleSnapshotId)} | مأخذ سود: ${safe(data.line.profitProvenanceStatus||'UNKNOWN')}</div><h5>مأخذ بهای تمام‌شده</h5><pre>${safe(JSON.stringify(sources,null,2))}</pre><h5>Manual Cost Evidence</h5><pre>${safe(JSON.stringify(data.source.manualCostEvidence||[],null,2))}</pre><h5>Cost exception / unknown reason</h5><pre>${safe(JSON.stringify({costCoverageStatus:data.line.costCoverageStatus,profitProvenanceStatus:data.line.profitProvenanceStatus,blockers:data.line.blockers||[],unknownCostReason:data.line.unknownCostReason||null},null,2))}</pre></div></div>`;}catch(error){drill.innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;}};
    api('/api/accounting/fifo-reliability').then(data=>{document.querySelector('#faReliability').innerHTML=`<div class="${data.reliable?'success':'warn'}"><b>${safe(data.reliabilityDecision)}</b> | Line ${safe(data.coverage?.line?.percent)}% | Quantity ${safe(data.coverage?.quantity?.percent)}% | Sale Value ${safe(data.coverage?.saleValue?.percent)}% | stale=${safe(data.source?.stale)}</div>`;}).catch(error=>{document.querySelector('#faReliability').innerHTML=`<div class="error">${safe(error.code)} ${safe(error.message)}</div>`;});
    document.querySelector('#faSearch').onclick=load;
  }
