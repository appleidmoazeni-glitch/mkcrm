...
(()=>{
  ...
  async function mappingPage(){
    ...
    catch(error){policyLoadError('fgmPolicyState',error,mappingPage);return;}
    try{
    ...
    }catch(error){q('#fgmList').innerHTML=`<div class="error">دریافت سوابق نگاشت ناموفق بود: ${safe(error.code||'')} ${safe(error.message)} <button class="btn" id="fgmRetry">تلاش دوباره</button></div>`;q('#fgmRetry').onclick=mappingPage;}
  }
  async function ratePage(){
    ...
    }catch(error){q('#fgrList').innerHTML=`<div class="error">دریافت نسخه‌های نرخ ناموفق بود: ${safe(error.code||'')} ${safe(error.message)} <button class="btn" id="fgrRetry">تلاش دوباره</button></div>`;q('#fgrRetry').onclick=ratePage;}
  }
...
(()=>{
  ...
  window.route=route=async function(){
    ...
    if(REDIRECTS[requested])location.hash=current;
