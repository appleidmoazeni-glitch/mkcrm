...
async function buildReadModel(db,input={},requestedBy={}) {
  ...
  try{
    ...
    const requestedMode=clean(input.mode||'full',30).toLowerCase();const currentBeforeBuild=candidateOnly?null:await activeRun(db);const duplicateCandidate=candidateOnly?await db.collection(RUNS).findOne({status:'completed',candidateOnly:true,sourceFifoDatasetId:bundle.fifoDatasetId,sourceFingerprint:bundle.sourceFingerprint}):null;if(duplicateCandidate)return {ok:true,runId:duplicateCandidate.runId,lineCount:Number(duplicateCandidate.lineCount||0),summaryCount:Number(duplicateCandidate.summaryCount||0),retryCount:Number(duplicateCandidate.retryCount||0),resumeCount:Number(duplicateCandidate.resumeCount||0),sourceFingerprint:bundle.sourceFingerprint,resultFingerprint:duplicateCandidate.resultFingerprint,active:false,candidateOnly:true,duplicate:true,buildMode:'candidate-no-source-change',nonPayable:true};if(requestedMode==='incremental'&&currentBeforeBuild?.run?.sourceFingerprint===bundle.sourceFingerprint)return {ok:true,runId:currentBeforeBuild.runId,lineCount:Number(currentBeforeBuild.run.lineCount||0),summaryCount:Number(currentBeforeBuild.run.summaryCount||0),retryCount:0,resumeCount:Number(currentBeforeBuild.run.resumeCount||0),sourceFingerprint:bundle.sourceFingerprint,resultFingerprint:currentBeforeBuild.run.resultFingerprint,active:true,duplicate:true,buildMode:'incremental-no-source-change',nonPayable:true};
...
async function categoryTotals(db,filters={},requestedBy={}) {
  ...
  const rows=await db.collection(LINES).aggregate([{$match:query},{$group:{
    _id:{sellerIdentity:'$sellerIdentity',officialProductCategoryName:'$officialProductCategoryName'},sellerIdentity:{$first:'$sellerIdentity'},sellerName:{$first:'$sellerName'},officialProductCategoryName:{$first:'$officialProductCategoryName'},lineCount:{$sum:1},
  ...
  }},{$sort:{financialNetSalesNumeric:-1,sellerIdentity:1,officialProductCategoryName:1}}]).toArray();
