...
async function lineDrilldown(db,saleLineIdentity,requestedBy={}) {
  ...
  return {ok:true,runId:active.runId,line:decorateAdjustmentEligibility(line,fresh.stale),readModelStale:fresh.stale,source:{allocations,adjustments,savedEntries,discountFact},immutableFifo:true,auditable:true,nonPayable:true};
}
...
async function governanceCoverage(db,requestedBy={}) {requireRole(requestedBy,READ_ROLES);const active=await requireActive(db);const collection=db.collection(LINES);const aggregate=async query=>(await collection.aggregate([{$match:{runId:active.runId,...query}},{$group:{_id:null,lineCount:{$sum:1},saleValueNumeric:{$sum:'$grossSaleAmountNumeric'}}}]).toArray())[0]||{};const [all,mapped,rated,approvedPolicies,approvedMappings,approvedRates]=await Promise.all([aggregate({}),aggregate({categoryAvailability:'available'}),aggregate({rateAvailability:'available'}),count(db.collection(policies.POLICIES),{status:'approved'}),count(db.collection(ledger.CATEGORY_MAPPINGS),{status:'approved'}),count(db.collection(ledger.RATE_VERSIONS),{status:'approved'})]);const allValue=Number(decimalText(all.saleValueNumeric)||0);const percent=value=>Number((Number(decimalText(value)||0)*100/Math.max(1,allValue)).toFixed(4));return {ok:true,runId:active.runId,approvedSelectablePolicies:approvedPolicies,approvedMappings,approvedRates,totalLines:Number(all.lineCount||0),totalSaleValueExact:decimalText(all.saleValueNumeric)||'0.00',approvedMappedLines:Number(mapped.lineCount||0),approvedMappedLineCoveragePercent:Number((Number(mapped.lineCount||0)*100/Math.max(1,Number(all.lineCount||0))).toFixed(4)),approvedMappedSaleValueExact:decimalText(mapped.saleValueNumeric)||'0.00',approvedMappedSaleValueCoveragePercent:percent(mapped.saleValueNumeric),approvedRateLines:Number(rated.lineCount||0),approvedRateCoveragePercent:percent(rated.saleValueNumeric),approvedOnly:true,automaticApproval:false};}
