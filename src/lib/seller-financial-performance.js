...
const HUMAN_VALIDATIONS = 'sellerFinancialPerformanceHumanValidations';
const ACTIVATION_AUDITS = 'sellerFinancialPerformanceActivationAudits';
const SCOPE_KEY = 'seller-financial-performance-v1';
const SCHEMA_VERSION = 4;
const ALGORITHM_VERSION = 'seller-financial-performance-v4-canonical-financial-dimensions';
const MODULE_VERSION = 'seller-financial-performance-1.3.0';
const UNRESOLVED_CATEGORY_GUID = '__UNRESOLVED__';
const READ_ROLES = Object.freeze(['admin', 'accounting', 'manager', 'purchase']);
const BUILD_ROLES = Object.freeze(['admin', 'accounting']);
const VALIDATE_ROLES = Object.freeze(['admin', 'manager']);
const ACTIVATE_ROLES = Object.freeze(['admin', 'manager']);
const COLLECTIONS = Object.freeze([RUNS, LINES, SUMMARIES, STATE, LOCKS, VERIFICATIONS, HUMAN_VALIDATIONS, ACTIVATION_AUDITS]);
...
async function ensureIndexes(db) {
  ...
  await db.collection(LINES).createIndex({runId:1,canonicalSellerId:1,canonicalCategoryGuid:1,saleDate:1});
  await db.collection(LINES).createIndex({runId:1,canonicalCategoryGuid:1,saleDate:1});
  ...
  await db.collection(HUMAN_VALIDATIONS).createIndex({runId:1,candidateFingerprint:1,result:1,createdAt:-1});
  await db.collection(ACTIVATION_AUDITS).createIndex({activationId:1},{unique:true});
  await db.collection(ACTIVATION_AUDITS).createIndex({newActiveSellerFinancialId:1,createdAt:-1});
...
function fifoFingerprints(dataset = {}) {
  return {
...
async function resolveActiveFifoLineage(db) {
  const state=await db.collection(fifo.STATE).findOne({scopeKey:fifo.SCOPE_KEY});
  const datasetId=clean(state?.activeDatasetId,100);
  if(!datasetId)fail('SELLER_FINANCIAL_ACTIVE_FIFO_MISSING','Active FIFO authority is not configured; Candidate build is blocked.',409);
  const dataset=await db.collection(fifo.DATASETS).findOne({datasetId});
  if(!dataset||dataset.status!=='completed')fail('SELLER_FINANCIAL_ACTIVE_FIFO_INVALID','Active FIFO authority does not resolve to a completed immutable Dataset.',409);
  const fingerprints=fifoFingerprints(dataset);
  if(Object.values(fingerprints).some(value=>!SHA256_HEX.test(value)))fail('SELLER_FINANCIAL_ACTIVE_FIFO_FINGERPRINT_INVALID','Active FIFO fingerprints are incomplete or invalid.',409,{datasetId});
  return {
    datasetId,
    ...
    activationStatus:clean(dataset.activationStatus,50),
    immutable:dataset.immutable!==false
  };
}

async function assertCanonicalBuildBinding(db,input={}) {
  if(input.candidateOnly!==true)fail('SELLER_FINANCIAL_CANDIDATE_ONLY_REQUIRED','Canonical Seller Financial build must be Candidate-only.',409);
  const expectedId=clean(input.expectedActiveFifoDatasetId,100);
  const requestedId=clean(input.fifoDatasetId,100);
  if(!expectedId||!requestedId)fail('SELLER_FINANCIAL_ACTIVE_FIFO_BINDING_REQUIRED','Exact Active FIFO binding from the current page is required.',400);
  if(!Object.prototype.hasOwnProperty.call(input,'expectedFifoAuthorityRevision'))fail('SELLER_FINANCIAL_FIFO_AUTHORITY_REVISION_REQUIRED','Active FIFO authority revision is required.',400);
  const active=await resolveActiveFifoLineage(db);
  if(expectedId!==active.datasetId||requestedId!==active.datasetId)fail('SELLER_FINANCIAL_ACTIVE_FIFO_STALE','Active FIFO changed or the requested FIFO is not authoritative; refresh the page.',409,{expectedActiveFifoDatasetId:expectedId,requestedFifoDatasetId:requestedId,currentActiveFifoDatasetId:active.datasetId});
  if(Number(input.expectedFifoAuthorityRevision)!==active.authorityRevision)fail('SELLER_FINANCIAL_ACTIVE_FIFO_STALE','Active FIFO authority revision changed; refresh the page.',409,{expectedFifoAuthorityRevision:Number(input.expectedFifoAuthorityRevision),currentFifoAuthorityRevision:active.authorityRevision});
  const expected={
    source:clean(input.expectedFifoSourceFingerprint,64),
    allocation:clean(input.expectedFifoAllocationFingerprint,64),
    candidate:clean(input.expectedFifoCandidateFingerprint,64)
  };
  if(Object.values(expected).some(value=>!SHA256_HEX.test(value)))fail('SELLER_FINANCIAL_FIFO_FINGERPRINT_BINDING_REQUIRED','Exact Active FIFO fingerprints are required.',400);
  if(expected.source!==active.sourceFingerprint||expected.allocation!==active.allocationFingerprint||expected.candidate!==active.candidateFingerprint)fail('SELLER_FINANCIAL_ACTIVE_FIFO_STALE','Active FIFO lineage fingerprints changed; refresh the page.',409);
  return active;
}

async function buildContext(db,requestedBy={}) {
  requireRole(requestedBy,READ_ROLES);await ensureIndexes(db);
  const activeFifo=await resolveActiveFifoLineage(db);
  const [active,latestCandidate]=await Promise.all([
    activeRun(db),
    db.collection(RUNS).findOne({status:'completed',candidateOnly:true},{sort:{completedAt:-1,createdAt:-1}})
  ]);
  const current=actor(requestedBy);
  return {
    ok:true,
    activeFifo,
    currentActiveSellerFinancialId:active?.runId||'',
    latestCandidate:latestCandidate?{runId:latestCandidate.runId,sourceFifoDatasetId:latestCandidate.sourceFifoDatasetId,candidateFingerprint:latestCandidate.candidateFingerprint||latestCandidate.resultFingerprint||'',status:latestCandidate.status,activationStatus:latestCandidate.activationStatus,active:Boolean(latestCandidate.active),candidateOnly:Boolean(latestCandidate.candidateOnly),nonPayable:latestCandidate.nonPayable!==false}:null,
    canBuild:BUILD_ROLES.includes(current.role),
    buildRoles:BUILD_ROLES,
    resultContract:{active:false,candidateOnly:true,nonPayable:true,activationStatus:'validated-candidate',commissionCreated:false,activationSeparate:true},
    legacyRebuild:{classification:'LEGACY_UNSAFE_FOR_CANONICAL_BUILD',availableFromCanonicalUi:false}
  };
}
...
async function sourceBundle(db,input={}) {
  const requestedDatasetId=clean(input.fifoDatasetId,100);
  const fifoActive=requestedDatasetId
    ? {datasetId:requestedDatasetId,dataset:await db.collection(fifo.DATASETS).findOne({datasetId:requestedDatasetId})}
    : await fifo.activeDataset(db);
  if(!fifoActive?.datasetId||fifoActive.dataset?.status!=='completed')fail('SELLER_FINANCIAL_FIFO_SOURCE_MISSING','FIFO Dataset کامل پیدا نشد.',409);
  const candidateOnly=input.candidateOnly===true;
  if(candidateOnly&&fifoActive.dataset.activationStatus!=='validated-candidate')fail('SELLER_FINANCIAL_FIFO_CANDIDATE_INVALID','فقط validated-candidate برای Read Model کاندیدا مجاز است.',409);
  if(!candidateOnly&&requestedDatasetId&&fifoActive.dataset.activationStatus!=='validated-shadow')fail('SELLER_FINANCIAL_FIFO_SOURCE_NOT_ACTIVE','منبع غیرکاندیدا باید validated-shadow باشد.',409);
  ...
  const sourceProjection={
    ...
    fifoSourceFingerprint:fifoActive.dataset.sourceFingerprint||'',fifoAllocationFingerprint:fifoActive.dataset.allocationFingerprint||'',candidateFingerprint:fifoActive.dataset.candidateFingerprint||'',candidateOnly,
  ...
  return {fifoActive,fifoDatasetId,saleSnapshotId,candidateOnly,facts,catalogMaps,approvedPolicies,governedMappings,governedRates,discountFacts,adjustments,savedEntries,headers,userMappings,users,allocations,sourceFingerprint:sha(stable(sourceProjection)),sourceProjection};
}
...
async function fastSourceMetadata(db,input={}) {
  const requestedDatasetId=clean(input.fifoDatasetId,100);
  const fifoActive=requestedDatasetId?{datasetId:requestedDatasetId,dataset:await db.collection(fifo.DATASETS).findOne({datasetId:requestedDatasetId})}:await fifo.activeDataset(db);
  ...
  const metadata={schemaVersion:1,fifoDatasetId,saleSnapshotId,candidateOnly:input.candidateOnly===true,fifoSourceFingerprint:fifoActive.dataset.sourceFingerprint||'',fifoAllocationFingerprint:fifoActive.dataset.allocationFingerprint||'',candidateFingerprint:fifoActive.dataset.candidateFingerprint||'',catalogRunId:catalogRun?.catalogRunId||'',catalogFetchedAt:catalogRun?.fetchedAt||catalogRun?.createdAt||null,policy,mappings,rates,bindings,discounts,adjustments,savedEntries,headers,sellerMappings,users};
...
function buildProjectedLines(bundle,runId,calculatedAt) {
  ...
  for(const fact of bundle.facts){
    ...
    const core={
      ...
      sellerIdentity,canonicalSellerId:sellerIdentity,sellerName:clean(sellerActor.displayName||header.sellerName||fact.sellerName,200),sellerDisplayName:clean(sellerActor.displayName||header.sellerName||fact.sellerName,200),sellerUsername:sellerActor.username||'',storeIdentity,storeName,
      ...
      officialMainGroupGuid:classification.officialProductCategoryGuid||'',officialProductCategoryIdentity:classification.officialProductCategoryIdentity||'',officialProductCategoryGuid:classification.officialProductCategoryGuid||'',canonicalCategoryGuid:classification.officialProductCategoryGuid||'',officialProductCategoryNumber:classification.officialProductCategoryNumber||'',officialProductCategoryName:classification.officialProductCategoryName||'UNRESOLVED',productCategoryResolutionStatus:classification.officialProductCategoryGuid?'resolved':'unresolved',
      ...
      fifoCostExact:fact.fifoCostExact==null?null:exact(fact.fifoCostExact),actualFifoProfitExact:fact.actualFifoProfitExact==null?null:exact(fact.actualFifoProfitExact),fifoMarginExact:marginExact,costCoverageStatus:fact.costCoverageStatus||'unknown',profitProvenanceStatus,costSourceType:clean(fact.costSourceType||'UNKNOWN',50),provenanceFingerprint:clean(fact.provenanceFingerprint,64),profitAvailability,
      returnLinkage:fact.returnLinkage||null,originSaleInvoiceNumber:fact.returnLinkage?.originSaleInvoiceNumber||null,originSaleLineIdentity:clean(fact.returnLinkage?.originSaleLineIdentity,500),returnLinkageClass:clean(fact.returnLinkage?.linkageClass,100),restoredAllocationId:clean(fact.returnLinkage?.restoredAllocationId,100),
...
function createSummary(runId,dimension,dimensionKey,labels,rows,calculatedAt) {
  ...
  const provenProfit=rows.filter(row=>row.profitProvenanceStatus==='PROVEN'&&row.actualFifoProfitExact!=null);
  const blockerCounts={};for(const row of rows)for(const blocker of row.blockers||[])blockerCounts[blocker]=(blockerCounts[blocker]||0)+1;
    ...
    lineCount:rows.length,invoiceCount:new Set(rows.map(row=>row.saleInvoiceIdentity)).size,saleQuantityExact:add(rows.map(row=>row.quantityExact),6),saleValueExact:sumLineEntries(rows,'grossSaleAmountExact'),grossSalesExact:sumLineEntries(rows.filter(row=>row.saleInvoiceType===2),'grossSaleAmountExact'),financialNetSalesExact:sumLineEntries(rows,'grossSaleAmountExact'),lineDiscountExact:sumLineEntries(rows.filter(row=>row.lineDiscountExact!=null),'lineDiscountExact'),allocatedInvoiceDiscountExact:rows.every(row=>row.allocatedInvoiceDiscountExact!=null)?sumLineEntries(rows,'allocatedInvoiceDiscountExact'):null,netSaleValueExact:rows.some(row=>row.netSaleAmountExact!=null)?sumLineEntries(rows.filter(row=>row.netSaleAmountExact!=null),'netSaleAmountExact'):null,
    ...
    knownProfitLineCount:knownProfit.length,provenProfitLineCount:provenProfit.length,availableCommissionLineCount:knownCommission.length,unavailableLineCount:rows.filter(row=>row.blockers?.length).length,unavailableCommissionLineCount:rows.length-knownCommission.length,partialCostLineCount:rows.filter(row=>row.costCoverageStatus==='partial').length,unknownCostLineCount:rows.filter(row=>row.costCoverageStatus==='unknown').length,profitCoveragePercent:Number((knownProfit.length*100/Math.max(1,rows.length)).toFixed(4)),provenProfitCoveragePercent:Number((provenProfit.length*100/Math.max(1,rows.length)).toFixed(4)),provenSaleValueCoveragePercent:Number((Number(sumLineEntries(provenProfit,'grossSaleAmountExact'))*100/Math.max(1,Number(sumLineEntries(rows,'grossSaleAmountExact')))).toFixed(4)),commissionCoveragePercent:Number((knownCommission.length*100/Math.max(1,rows.length)).toFixed(4)),blockerCounts,calculatedAt,nonPayable:true,nonPayroll:true};
  for(const field of ['saleValueExact','grossSalesExact','financialNetSalesExact','lineDiscountExact','allocatedInvoiceDiscountExact','netSaleValueExact','saleReturnValueExact','knownFifoCostExact','knownFifoProfitExact','provenFifoProfitExact','provenProfitSaleValueExact','unknownOrPartialSaleValueExact','unknownCostSaleValueExact','unavailableProfitSaleValueExact','unavailableCommissionSaleValueExact','approvedAdjustmentAmountExact','savedProfitCreditExact','savedProfitSubsidyExact','commissionableProfitExact','preliminaryCommissionExact'])summary[field.replace(/Exact$/,'Numeric')]=numeric(summary[field]);
...
function buildSummaries(lines,runId,calculatedAt) {
  const dimensions=[
    ['seller-month',row=>`${row.canonicalSellerId}|${row.accountingMonth}`,row=>({sellerIdentity:row.sellerIdentity,canonicalSellerId:row.canonicalSellerId,sellerName:row.sellerName,accountingMonth:row.accountingMonth})],
    ['seller-category-month',row=>`${row.canonicalSellerId}|${row.canonicalCategoryGuid||UNRESOLVED_CATEGORY_GUID}|${row.accountingMonth}`,row=>({sellerIdentity:row.sellerIdentity,canonicalSellerId:row.canonicalSellerId,sellerName:row.sellerName,officialProductCategoryIdentity:row.officialProductCategoryIdentity,officialProductCategoryGuid:row.officialProductCategoryGuid,canonicalCategoryGuid:row.canonicalCategoryGuid,officialProductCategoryNumber:row.officialProductCategoryNumber,officialProductCategoryName:row.officialProductCategoryName,productCategoryResolutionStatus:row.productCategoryResolutionStatus,accountingMonth:row.accountingMonth})],
    ['seller-rate-pool-month',row=>`${row.canonicalSellerId}|${row.commissionRatePool}|${row.accountingMonth}`,row=>({sellerIdentity:row.sellerIdentity,canonicalSellerId:row.canonicalSellerId,sellerName:row.sellerName,commissionRatePool:row.commissionRatePool,accountingMonth:row.accountingMonth})],
    ['seller-store-month',row=>`${row.canonicalSellerId}|${row.storeIdentity}|${row.accountingMonth}`,row=>({sellerIdentity:row.sellerIdentity,canonicalSellerId:row.canonicalSellerId,sellerName:row.sellerName,storeIdentity:row.storeIdentity,storeName:row.storeName,accountingMonth:row.accountingMonth})],
    ['invoice',row=>row.saleInvoiceIdentity,row=>({saleInvoiceIdentity:row.saleInvoiceIdentity,saleInvoiceType:row.saleInvoiceType,saleInvoiceNumber:row.saleInvoiceNumber,saleDate:row.saleDate,sellerIdentity:row.sellerIdentity,canonicalSellerId:row.canonicalSellerId,sellerName:row.sellerName})],
...
async function buildReadModel(db,input={},requestedBy={}) {
  const current=requireRole(requestedBy,BUILD_ROLES);await ensureIndexes(db);const candidateOnly=input.candidateOnly===true;const requestedFifoDatasetId=clean(input.fifoDatasetId,100);if(candidateOnly&&!requestedFifoDatasetId)fail('SELLER_FINANCIAL_FIFO_CANDIDATE_REQUIRED','fifoDatasetId برای Candidate Read Model الزامی است.',400);if(candidateOnly)await assertCanonicalBuildBinding(db,input);const runId=clean(input.runId,100)||newId('SFPR');let lock=null;const memorySamples=[memorySample('before-rebuild')];
  try{
    lock=await acquireLock(db,runId);progress(input.jobControl,'Reading Immutable Sources',0,1,'Reading approved source datasets');const bundle=await sourceBundle(db,{fifoDatasetId:requestedFifoDatasetId,candidateOnly});memorySamples.push(memorySample('after-source-read'));
    if(candidateOnly)await assertCanonicalBuildBinding(db,input);
    const [fastMetadata,recency]=await Promise.all([fastSourceMetadata(db,{fifoDatasetId:bundle.fifoDatasetId,candidateOnly}),sourceRecency(db,bundle.saleSnapshotId)]);
    const requestedMode=clean(input.mode||'full',30).toLowerCase();const currentBeforeBuild=candidateOnly?null:await activeRun(db);const duplicateCandidate=candidateOnly?await db.collection(RUNS).findOne({status:'completed',candidateOnly:true,sourceFifoDatasetId:bundle.fifoDatasetId,sourceFingerprint:bundle.sourceFingerprint,schemaVersion:SCHEMA_VERSION,algorithmVersion:ALGORITHM_VERSION}):null;if(duplicateCandidate)return {ok:true,runId:duplicateCandidate.runId,lineCount:Number(duplicateCandidate.lineCount||0),summaryCount:Number(duplicateCandidate.summaryCount||0),retryCount:Number(duplicateCandidate.retryCount||0),resumeCount:Number(duplicateCandidate.resumeCount||0),sourceFingerprint:bundle.sourceFingerprint,resultFingerprint:duplicateCandidate.resultFingerprint,candidateFingerprint:duplicateCandidate.candidateFingerprint||duplicateCandidate.resultFingerprint,active:false,candidateOnly:true,duplicate:true,buildMode:'candidate-no-source-change',nonPayable:true};if(requestedMode==='incremental'&&currentBeforeBuild?.run?.sourceFingerprint===bundle.sourceFingerprint&&currentBeforeBuild.run.schemaVersion===SCHEMA_VERSION&&currentBeforeBuild.run.algorithmVersion===ALGORITHM_VERSION)return {ok:true,runId:currentBeforeBuild.runId,lineCount:Number(currentBeforeBuild.run.lineCount||0),summaryCount:Number(currentBeforeBuild.run.summaryCount||0),retryCount:0,resumeCount:Number(currentBeforeBuild.run.resumeCount||0),sourceFingerprint:bundle.sourceFingerprint,resultFingerprint:currentBeforeBuild.run.resultFingerprint,active:true,duplicate:true,buildMode:'incremental-no-source-change',nonPayable:true};
    let run=await db.collection(RUNS).findOne({runId});const now=new Date();let resumeCount=0;
    if(run){if(!['building','failed'].includes(run.status))fail('SELLER_FINANCIAL_RUN_IMMUTABLE','فقط Run ناقص یا ناموفق قابل Resume است.',409);if(run.sourceFingerprint&&run.sourceFingerprint!==bundle.sourceFingerprint)fail('SELLER_FINANCIAL_SOURCE_CHANGED','منابع از زمان Run قبلی تغییر کرده‌اند؛ Resume مجاز نیست.',409);resumeCount=Number(run.resumeCount||0)+1;await db.collection(RUNS).updateOne({runId},{$set:{status:'building',candidateOnly,sourceFingerprint:bundle.sourceFingerprint,fastSourceMetadata:fastMetadata.metadata,fastSourceMetadataFingerprint:fastMetadata.fingerprint,sourceRecency:recency,resumeCount,updatedAt:now,lastError:''}});}else{const policyIds=bundle.approvedPolicies.map(row=>row.policyVersionId).sort();run={runId,schemaVersion:SCHEMA_VERSION,algorithmVersion:ALGORITHM_VERSION,moduleVersion:MODULE_VERSION,buildMode:candidateOnly?'candidate-full':requestedMode==='incremental'?'incremental-safe-full-rebuild':'full',status:'building',active:false,candidateOnly,activationStatus:'candidate',sourceFingerprint:bundle.sourceFingerprint,fastSourceMetadata:fastMetadata.metadata,fastSourceMetadataFingerprint:fastMetadata.fingerprint,sourceRecency:recency,sourceFifoDatasetId:bundle.fifoDatasetId,sourceFifoAuthorityRevision:candidateOnly?Number(input.expectedFifoAuthorityRevision):null,sourceFifoSourceFingerprint:clean(bundle.fifoActive.dataset.sourceFingerprint,64),sourceFifoAllocationFingerprint:clean(bundle.fifoActive.dataset.allocationFingerprint,64),sourceFifoCandidateFingerprint:clean(bundle.fifoActive.dataset.candidateFingerprint,64),sourceSaleSnapshotId:bundle.saleSnapshotId,sourceProfitFactsDatasetId:bundle.facts[0]?.profitFactsDatasetId||'',sourcePolicyVersionId:policyIds.length===1?policyIds[0]:'',sourcePolicyVersionIds:policyIds,sourceCategoryCatalogRunId:bundle.catalogMaps.run?.catalogRunId||'',sourceCatalogRunId:bundle.catalogMaps.run?.catalogRunId||'',sourceDiscountVersion:1,sourceDiscountFingerprint:bundle.sourceProjection.discounts.fingerprint,sourceAdjustmentFingerprint:bundle.sourceProjection.adjustments.fingerprint,sourceLedgerFingerprint:bundle.sourceProjection.savedEntries.fingerprint,sourceSellerMappingFingerprint:bundle.sourceProjection.sellerMappings.fingerprint,checkpoint:{linesWritten:0,lastSaleLineIdentity:''},retryCount:0,resumeCount:0,createdBy:current,startedAt:now,createdAt:now,updatedAt:now,diagnostics:[],immutable:true,nonPayable:true,sellerFacing:false};await db.collection(RUNS).insertOne(run);}
    ...
    const previous=candidateOnly?null:await activeRun(db);const completedAt=new Date();memorySamples.push(memorySample('immediately-after-rebuild'));const peakRssBytes=Math.max(...memorySamples.map(row=>row.rssBytes));await db.collection(RUNS).updateOne({runId},{$set:{status:'completed',active:false,candidateOnly,activationStatus:'validated-candidate',candidateFingerprint:resultFingerprint,lineCount:lines.length,summaryCount:summaries.length,availableCommissionLineCount:lines.filter(row=>row.commissionStatus==='available').length,unavailableCommissionLineCount:lines.filter(row=>row.commissionStatus!=='available').length,retryCount:write.retryCount,resumeCount,attemptDiagnostics:write.attempts.slice(-100),diagnostics:[{code:'SOURCE_VALIDATED',at:completedAt,sourceFingerprint:bundle.sourceFingerprint},{code:'FINGERPRINT_REPLAY_VALIDATED',at:completedAt,lineFingerprint,summaryFingerprint,resultFingerprint},{code:'CANDIDATE_VALIDATED',at:completedAt,validation}],validation,lineFingerprint,summaryFingerprint,resultFingerprint,fingerprintContract:{algorithm:'SHA-256',encoding:'lowercase-hex',length:64,canonicalOrdering:true,replayValidated:true},memorySamples,peakRssBytes,completedAt,immutable:true,nonPayable:true,sellerFacing:false,commissionCreated:false,payrollAuthority:false,updatedAt:completedAt}});
    if(candidateOnly){progress(input.jobControl,'Completed',1,1,'Seller Financial Candidate is ready for human validation');return {ok:true,runId,lineCount:lines.length,summaryCount:summaries.length,retryCount:write.retryCount,resumeCount,sourceFingerprint:bundle.sourceFingerprint,resultFingerprint,candidateFingerprint:resultFingerprint,previousActiveRunId:'',active:false,candidateOnly:true,activationStatus:'validated-candidate',nonPayable:true,commissionCreated:false};}
...
function addCategoryGuidFilter(query,input) {
  const values=listParam(input);if(!values.length)return;
  const includeUnresolved=values.includes(UNRESOLVED_CATEGORY_GUID);const resolved=values.filter(value=>value!==UNRESOLVED_CATEGORY_GUID);
  const predicates=[];if(resolved.length)predicates.push({canonicalCategoryGuid:{$in:resolved}});
  if(includeUnresolved)predicates.push({$or:[{canonicalCategoryGuid:''},{canonicalCategoryGuid:null},{canonicalCategoryGuid:{$exists:false}}]});
  if(predicates.length===1)Object.assign(query,predicates[0]);else if(predicates.length)query.$and=[...(query.$and||[]),{$or:predicates}];
}
function queryFromFilters(runId,filters={}) {
  ...
  if(filters.category&&!filters.categoryGuid&&!filters.officialProductCategoryGuid)fail('SELLER_FINANCIAL_CATEGORY_GUID_REQUIRED','فیلتر Product Category باید با GUID رسمی ارسال شود.',400);
  const matchList=(input,field)=>{const values=listParam(input);if(values.length)query[field]={$in:values};};matchList(filters.sellerIdentity,'canonicalSellerId');addCategoryGuidFilter(query,filters.categoryGuid||filters.officialProductCategoryGuid);matchList(filters.ratePool,'commissionRatePool');matchList(filters.store,'storeIdentity');matchList(filters.costStatus,'costCoverageStatus');matchList(filters.provenanceStatus,'profitProvenanceStatus');
...
async function requireRun(db,filters={}) {const runId=clean(filters.runId,100);if(runId){const run=await db.collection(RUNS).findOne({runId,status:'completed'});if(!run)fail('SELLER_FINANCIAL_RUN_NOT_FOUND','Read Model کاندیدا پیدا نشد.',404);return {runId,run,candidateOnly:Boolean(run.candidateOnly)};}const active=await activeRun(db);if(!active)fail('SELLER_FINANCIAL_ACTIVE_RUN_MISSING','Read Model فعال موجود نیست؛ Candidate را انتخاب کنید.',409);return active;}
async function requireActive(db) {return requireRun(db,{});}
...
async function listLines(db,filters={},requestedBy={}) {
  requireRole(requestedBy,READ_ROLES);const selected=await requireRun(db,filters);const query=queryFromFilters(selected.runId,filters);const page=Math.max(1,Number(filters.page||1));const pageSize=Math.max(1,Math.min(Number(filters.pageSize||50),MAX_PAGE_SIZE));const sortFields={date:'saleDate',invoice:'saleInvoiceNumber',profit:'actualFifoProfitNumeric',sale:'grossSaleAmountNumeric'};const sortField=sortFields[filters.sortBy]||'saleDate';const direction=clean(filters.sortDirection).toLowerCase()==='asc'?1:-1;
  const [total,list]=await Promise.all([count(db.collection(LINES),query),db.collection(LINES).find(query).sort({[sortField]:direction,saleLineIdentity:1}).skip((page-1)*pageSize).limit(pageSize).toArray()]);const sourceStatus=selected.run.candidateOnly?{stale:false}:await freshness(db,requestedBy);return {ok:true,runId:selected.runId,page,pageSize,total,list:list.map(row=>decorateAdjustmentEligibility(row,sourceStatus.stale)),readModelStale:sourceStatus.stale,candidateOnly:Boolean(selected.run.candidateOnly),active:Boolean(selected.run.active),serverSide:true,nonPayable:true,sellerFacing:false};
}
async function listInvoiceLines(db,saleInvoiceIdentity,filters={},requestedBy={}) {
  requireRole(requestedBy,READ_ROLES);const active=await requireRun(db,filters);const page=Math.max(1,Number(filters.page||1));const pageSize=Math.max(1,Math.min(Number(filters.pageSize||100),MAX_PAGE_SIZE));const query={runId:active.runId,saleInvoiceIdentity:clean(saleInvoiceIdentity,200)};const [total,list]=await Promise.all([count(db.collection(LINES),query),db.collection(LINES).find(query).sort({saleLineIdentity:1}).skip((page-1)*pageSize).limit(pageSize).toArray()]);if(!total)fail('SELLER_FINANCIAL_INVOICE_NOT_FOUND','فاکتور در Read Model انتخاب‌شده پیدا نشد.',404);return {ok:true,runId:active.runId,saleInvoiceIdentity,page,pageSize,total,list:list.map(row=>decorateAdjustmentEligibility(row,false)),readModelStale:false,candidateOnly:Boolean(active.run.candidateOnly),serverSide:true,nonPayable:true};
}
async function listInvoices(db,filters={},requestedBy={}) {
  requireRole(requestedBy,READ_ROLES);const active=await requireRun(db,filters);const query=queryFromFilters(active.runId,filters);const page=Math.max(1,Number(filters.page||1)),pageSize=Math.max(1,Math.min(Number(filters.pageSize||50),MAX_PAGE_SIZE));const known=field=>({$sum:{$cond:[{$ne:[`$${field}`,null]},1,0]}});const group={$group:{_id:'$saleInvoiceIdentity',saleInvoiceIdentity:{$first:'$saleInvoiceIdentity'},saleInvoiceType:{$first:'$saleInvoiceType'},saleInvoiceNumber:{$first:'$saleInvoiceNumber'},saleDate:{$first:'$saleDate'},sellerIdentity:{$first:'$sellerIdentity'},sellerName:{$first:'$sellerName'},storeName:{$first:'$storeName'},lineCount:{$sum:1},grossSaleAmountNumeric:{$sum:'$grossSaleAmountNumeric'},netSaleAmountNumeric:{$sum:'$netSaleAmountNumeric'},actualFifoProfitNumeric:{$sum:'$actualFifoProfitNumeric'},commissionableProfitNumeric:{$sum:'$commissionableProfitNumeric'},preliminaryCommissionNumeric:{$sum:'$preliminaryCommissionNumeric'},netKnownCount:known('netSaleAmountNumeric'),profitKnownCount:known('actualFifoProfitNumeric'),commissionableKnownCount:known('commissionableProfitNumeric'),commissionKnownCount:known('preliminaryCommissionNumeric'),unavailableLineCount:{$sum:{$cond:[{$gt:[{$size:'$blockers'},0]},1,0]}},categories:{$addToSet:'$officialProductCategoryName'},ratePools:{$addToSet:'$commissionRatePool'}}};
...
async function totals(db,filters={},requestedBy={}) {
  ...
  const scopedAmount=async extra=>(await db.collection(LINES).aggregate([{$match:{$and:[query,extra]}},{$group:{_id:null,lineCount:{$sum:1},saleValueNumeric:{$sum:'$grossSaleAmountNumeric'},costNumeric:{$sum:'$fifoCostNumeric'},profitNumeric:{$sum:'$actualFifoProfitNumeric'}}}]).toArray())[0]||{};
  const [grossSales,saleReturns,partialExposure,unknownExposure]=await Promise.all([scopedAmount({saleInvoiceType:2}),scopedAmount({saleInvoiceType:6}),scopedAmount({profitProvenanceStatus:'PARTIAL'}),scopedAmount({profitProvenanceStatus:'UNKNOWN'})]);
  ...
  const saleValueExact=decimalText(row.saleValueNumeric)||'0.00',provenSaleValueExact=decimalText(proven.saleValueNumeric)||'0.00';
  return {ok:true,runId:active.runId,candidateOnly:Boolean(active.run.candidateOnly),active:Boolean(active.run.active),lineCount,invoiceCount:Number(invoiceGroups[0]?.count||0),unavailableLineCount,unknownCostLineCount,saleQuantityExact:decimalText(row.saleQuantityNumeric)||'0.000000',grossSalesExact:decimalText(grossSales.saleValueNumeric)||'0.00',saleReturnEffectExact:decimalText(saleReturns.saleValueNumeric)||'0.00',saleValueExact,financialNetSalesExact:saleValueExact,netSaleValueExact:Number(row.netKnownCount||0)===lineCount?decimalText(row.netSaleValueNumeric):null,knownFifoCostExact:Number(row.costKnownCount||0)?decimalText(row.fifoCostNumeric):null,fifoCostExact:Number(row.costKnownCount||0)===lineCount?decimalText(row.fifoCostNumeric):null,knownFifoProfitExact:knownProfit,actualFifoProfitExact:completeProfit?knownProfit:null,profitCoverageComplete:completeProfit,provenProfitLineCount:Number(proven.lineCount||0),provenProfitSaleValueExact:provenSaleValueExact,provenFifoProfitExact:decimalText(proven.fifoProfitNumeric)||'0.00',provenProfitCoveragePercent:Number((Math.abs(Number(provenSaleValueExact))*100/Math.max(1,Math.abs(Number(saleValueExact)))).toFixed(4)),partialExposureLineCount:Number(partialExposure.lineCount||0),partialExposureSaleValueExact:decimalText(partialExposure.saleValueNumeric)||'0.00',unknownExposureLineCount:Number(unknownExposure.lineCount||0),unknownExposureSaleValueExact:decimalText(unknownExposure.saleValueNumeric)||'0.00',approvedAdjustmentExact:decimalText(row.approvedAdjustmentNumeric)||'0.00',savedProfitCreditExact:decimalText(row.savedProfitCreditNumeric)||'0.00',savedProfitSubsidyExact:decimalText(row.savedProfitSubsidyNumeric)||'0.00',commissionableProfitExact:Number(row.commissionableKnownCount||0)===lineCount?decimalText(row.commissionableProfitNumeric):null,preliminaryCommissionExact:Number(row.commissionKnownCount||0)===lineCount?decimalText(row.preliminaryCommissionNumeric):null,nonPayable:true,nonPayroll:true,serverSide:true};
}
async function categoryTotals(db,filters={},requestedBy={}) {
  requireRole(requestedBy,READ_ROLES);const selected=await requireRun(db,filters);const query=queryFromFilters(selected.runId,filters);
  const rows=await db.collection(LINES).aggregate([{$match:query},{$group:{
    _id:{canonicalSellerId:'$canonicalSellerId',canonicalCategoryGuid:'$canonicalCategoryGuid'},sellerIdentity:{$first:'$sellerIdentity'},canonicalSellerId:{$first:'$canonicalSellerId'},sellerName:{$first:'$sellerName'},officialProductCategoryGuid:{$first:'$officialProductCategoryGuid'},canonicalCategoryGuid:{$first:'$canonicalCategoryGuid'},officialProductCategoryNumber:{$first:'$officialProductCategoryNumber'},officialProductCategoryName:{$first:'$officialProductCategoryName'},productCategoryResolutionStatus:{$first:'$productCategoryResolutionStatus'},lineCount:{$sum:1},
    grossSalesNumeric:{$sum:{$cond:[{$eq:['$saleInvoiceType',2]},'$grossSaleAmountNumeric',0]}},saleReturnValueNumeric:{$sum:{$cond:[{$eq:['$saleInvoiceType',6]},'$grossSaleAmountNumeric',0]}},financialNetSalesNumeric:{$sum:'$grossSaleAmountNumeric'},
    provenFifoCostNumeric:{$sum:{$cond:[{$eq:['$profitProvenanceStatus','PROVEN']},'$fifoCostNumeric',0]}},provenFifoProfitNumeric:{$sum:{$cond:[{$eq:['$profitProvenanceStatus','PROVEN']},'$actualFifoProfitNumeric',0]}},unknownOrPartialSaleValueNumeric:{$sum:{$cond:[{$ne:['$profitProvenanceStatus','PROVEN']},'$grossSaleAmountNumeric',0]}}
  }},{$sort:{financialNetSalesNumeric:-1,canonicalSellerId:1,canonicalCategoryGuid:1}}]).toArray();
  return {ok:true,runId:selected.runId,candidateOnly:Boolean(selected.run.candidateOnly),active:Boolean(selected.run.active),total:rows.length,list:rows.map(row=>({...row,grossSalesExact:decimalText(row.grossSalesNumeric)||'0.00',saleReturnValueExact:decimalText(row.saleReturnValueNumeric)||'0.00',financialNetSalesExact:decimalText(row.financialNetSalesNumeric)||'0.00',provenFifoCostExact:decimalText(row.provenFifoCostNumeric)||'0.00',provenFifoProfitExact:decimalText(row.provenFifoProfitNumeric)||'0.00',unknownOrPartialSaleValueExact:decimalText(row.unknownOrPartialSaleValueNumeric)||'0.00'})),serverSide:true,nonPayable:true,nonPayroll:true};
}
async function filterOptions(db,filters={},requestedBy={}) {
  if(filters.role&&!requestedBy.role){requestedBy=filters;filters={};}requireRole(requestedBy,READ_ROLES);const active=await requireRun(db,filters);const rows=await db.collection(SUMMARIES).find({runId:active.runId,dimension:'seller-category-month'}).toArray();const storeRows=await db.collection(SUMMARIES).find({runId:active.runId,dimension:'seller-store-month'}).toArray();
  const categoryByGuid=new Map();let unresolved=false;
  for(const row of rows){const guid=clean(row.canonicalCategoryGuid||row.officialProductCategoryGuid,100);if(!guid){unresolved=true;continue;}if(!categoryByGuid.has(guid))categoryByGuid.set(guid,{guid,identity:row.officialProductCategoryIdentity||`guid:${guid}`,number:row.officialProductCategoryNumber||'',name:row.officialProductCategoryName||guid,status:'resolved'});}
  const nameCounts=new Map();for(const row of categoryByGuid.values())nameCounts.set(row.name,(nameCounts.get(row.name)||0)+1);
  const categories=[...categoryByGuid.values()].map(row=>({...row,label:nameCounts.get(row.name)>1?`${row.name} — ${row.number||row.guid}`:row.name}));if(unresolved)categories.push({guid:UNRESOLVED_CATEGORY_GUID,identity:'',number:'',name:'دسته رسمی حل‌نشده',label:'دسته رسمی حل‌نشده',status:'unresolved'});
  return {ok:true,runId:active.runId,candidateOnly:Boolean(active.run.candidateOnly),sellers:[...new Map(rows.map(row=>[row.canonicalSellerId||row.sellerIdentity,{identity:row.canonicalSellerId||row.sellerIdentity,name:row.sellerName}])).values()].sort((a,b)=>a.name.localeCompare(b.name,'fa')),categories:categories.sort((a,b)=>a.label.localeCompare(b.label,'fa')),ratePools:[...new Set((await db.collection(SUMMARIES).find({runId:active.runId,dimension:'seller-rate-pool-month'}).toArray()).map(row=>row.commissionRatePool).filter(Boolean))].sort(),stores:[...new Map(storeRows.map(row=>[row.storeIdentity,{identity:row.storeIdentity,name:row.storeName}])).values()],months:[...new Set(rows.map(row=>row.accountingMonth).filter(Boolean))].sort(),serverSide:true,categoryAuthority:'official-shaygan-category-guid'};
}
async function lineDrilldown(db,saleLineIdentity,requestedBy={},filters={}) {
  requireRole(requestedBy,READ_ROLES);const active=await requireRun(db,filters);const line=await db.collection(LINES).findOne({runId:active.runId,saleLineIdentity:clean(saleLineIdentity,500)});if(!line)fail('SELLER_FINANCIAL_LINE_NOT_FOUND','ردیف گزارش پیدا نشد.',404);
...
async function recordHumanValidation(db,runId,input={},requestedBy={}) {
  const current=requireRole(requestedBy,VALIDATE_ROLES);await ensureIndexes(db);
  const {run,fingerprint}=await requireCandidate(db,runId);const expected=clean(input.candidateFingerprint,64),reason=clean(input.reason,2000),result=clean(input.result,20).toUpperCase();
  ...
  return {ok:true,idempotent:false,validation};
}

async function activateCandidate(db,runId,input={},requestedBy={}) {
  const current=requireRole(requestedBy,ACTIVATE_ROLES);await ensureIndexes(db);
  const {run,fingerprint}=await requireCandidate(db,runId);const expectedFingerprint=clean(input.candidateFingerprint,64),validationId=clean(input.humanValidationId,100),reason=clean(input.reason,2000);
  if(expectedFingerprint!==fingerprint)fail('SELLER_FINANCIAL_ACTIVATION_FINGERPRINT_STALE','Exact Candidate fingerprint is required for activation.',409);
  if(!validationId||!reason)fail('SELLER_FINANCIAL_ACTIVATION_GOVERNANCE_REQUIRED','Human validation ID and activation reason are required.',400);
  const validation=await db.collection(HUMAN_VALIDATIONS).findOne({validationId,runId:run.runId,candidateFingerprint:fingerprint,result:'PASS',immutable:true});
  if(!validation)fail('SELLER_FINANCIAL_HUMAN_VALIDATION_REQUIRED','A fingerprint-bound PASS Human validation record is required.',409);
  const activeFifo=await resolveActiveFifoLineage(db);
  if(clean(run.sourceFifoDatasetId,100)!==activeFifo.datasetId||Number(run.sourceFifoAuthorityRevision)!==activeFifo.authorityRevision||clean(run.sourceFifoSourceFingerprint,64)!==activeFifo.sourceFingerprint||clean(run.sourceFifoAllocationFingerprint,64)!==activeFifo.allocationFingerprint||clean(run.sourceFifoCandidateFingerprint,64)!==activeFifo.candidateFingerprint)fail('SELLER_FINANCIAL_ACTIVATION_FIFO_AUTHORITY_CHANGED','Active FIFO authority or fingerprints changed after Candidate build; rebuild and revalidate Seller Financial.',409);
  const state=await db.collection(STATE).findOne({scopeKey:SCOPE_KEY});const previousActiveSellerFinancialId=clean(state?.activeRunId,100);const expectedPrevious=clean(input.expectedPreviousActiveSellerFinancialId,100);
  if(expectedPrevious!==previousActiveSellerFinancialId)fail('SELLER_FINANCIAL_ACTIVATION_STATE_STALE','Active Seller Financial authority changed; refresh before activation.',409,{expectedPreviousActiveSellerFinancialId:expectedPrevious,currentActiveSellerFinancialId:previousActiveSellerFinancialId});
  const activatedAt=new Date(),activationId=newId('SFACT'),audit={activationId,action:'ACTIVATE_SELLER_FINANCIAL',candidateId:run.runId,candidateFingerprint:fingerprint,humanValidationId:validation.validationId,actor:current,role:current.role,timestamp:activatedAt,reason,previousActiveSellerFinancialId,newActiveSellerFinancialId:run.runId,nonPayable:true,commissionCreated:false,immutable:true};
  const activeFilter=previousActiveSellerFinancialId?{activeRunId:previousActiveSellerFinancialId}:{$or:[{activeRunId:''},{activeRunId:null},{activeRunId:{$exists:false}}]};
  const updated=await db.collection(STATE).updateOne({scopeKey:SCOPE_KEY,...activeFilter},{$set:{scopeKey:SCOPE_KEY,activeRunId:run.runId,previousActiveRunId:previousActiveSellerFinancialId,activeCandidateFingerprint:fingerprint,humanValidationId:validation.validationId,activatedAt,activatedBy:current,activationReason:reason,lastActivationAudit:audit,updatedAt:activatedAt},$push:{activationAuditLog:audit}},{upsert:!state});
  if(!(updated.matchedCount||updated.upsertedCount))fail('SELLER_FINANCIAL_ACTIVATION_STATE_STALE','Active Seller Financial authority changed concurrently; activation stopped.',409);
  await db.collection(ACTIVATION_AUDITS).insertOne(audit);
  if(previousActiveSellerFinancialId&&previousActiveSellerFinancialId!==run.runId)await db.collection(RUNS).updateOne({runId:previousActiveSellerFinancialId},{$set:{status:'superseded',active:false,activationStatus:'superseded',supersededAt:activatedAt,updatedAt:activatedAt}});
  await db.collection(RUNS).updateOne({runId:run.runId,candidateFingerprint:fingerprint},{$set:{active:true,candidateOnly:false,activationStatus:'active',humanValidationId:validation.validationId,activatedAt,activatedBy:current,activationReason:reason,nonPayable:true,payrollAuthority:false,commissionCreated:false,updatedAt:activatedAt}});
