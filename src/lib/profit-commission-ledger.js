...
const MODULE_VERSION = 'profit-commission-ledger-1.2.0';
const RATE_SCHEMA_VERSION = 3;
const RATE_OPEN_STATUSES = Object.freeze(['draft', 'pending', 'returned']);
const RATE_RECORD_CLASSES = Object.freeze(['TEST', 'BUSINESS', 'HISTORICAL']);
...
function exactRate(value) {
  ...
  if(!/^\d+(?:\.\d{1,8})?$/.test(text))fail('RATE_INVALID','نرخ باید رشته اعشاری مثبت با حداکثر ۸ رقم اعشار باشد.');
  return exact(text,8);
}
...
async function ensureIndexes(db) {
  await commissionPolicies.ensureIndexes(db);
  ...
  await db.collection(RATE_VERSIONS).createIndex({ policyVersionId:1, sellerIdentity:1, rateScope:1, officialProductCategoryIdentity:1, commissionRatePool:1, effectiveFrom:1, effectiveTo:1, status:1 });
  await db.collection(RATE_VERSIONS).createIndex({ supersedesRateVersionId:1 }, { sparse:true });
...
function resolveCategoryFromMappings(candidates, saleLine, saleDate) {
  ...
  for (const [identityType, identityValue] of ordered) {
    ...
    if (rows.length === 1) return { ...productCategory, category:productCategory.officialProductCategoryName, commissionRatePool:mappingRatePool(rows[0]), mappingId:rows[0].mappingId, mappingContentHash:rows[0].contentHash||sha256(stable({mappingId:rows[0].mappingId,policyVersionId:rows[0].policyVersionId||'',officialProductCategoryGuid:rows[0].officialProductCategoryGuid||'',commissionRatePool:mappingRatePool(rows[0]),effectiveFrom:rows[0].effectiveFrom,effectiveTo:rows[0].effectiveTo||''})), policyVersionId:rows[0].policyVersionId||'', status:mappingRatePool(rows[0])==='UNRESOLVED'?'unresolved':'resolved', legacyRecord:!rows[0].commissionRatePool };
...
async function resolveCategory(db, saleLine, saleDate) {
  const candidates = await commissionPolicies.attachPolicyBindings(db,'category_mapping',await db.collection(CATEGORY_MAPPINGS).find({ status:'approved' }).toArray());
...
async function createCategoryMapping(db,input,requestedBy){
  const current=requireRole(requestedBy,EDIT_ROLES);await ensureIndexes(db);const policy=await commissionPolicies.requireSelectablePolicy(db,input.policyVersionId);const identity=categoryIdentity(input);const classification=await officialCategoryForMapping(db,input,identity);
  if(!classification)fail('OFFICIAL_PRODUCT_CATEGORY_REQUIRED','Mapping جدید باید Product Category رسمی Main Group و Commission Rate Pool جداگانه داشته باشد.',409);
  ...
  const mappingId=newId('CMAP');const lockKey=`create|${policy.policyVersionId}|${classification.officialProductCategoryGuid}|${effectiveFrom}|${effectiveTo||'open'}`;let lock=null;
  try{
    lock=await acquireApprovalLock(db,CATEGORY_APPROVAL_LOCKS,lockKey,mappingId);
    const duplicate=await db.collection(CATEGORY_MAPPINGS).findOne({policyVersionId:policy.policyVersionId,officialProductCategoryGuid:classification.officialProductCategoryGuid,effectiveFrom,effectiveTo,status:{$in:['draft','pending','returned']}});
    if(duplicate)fail('CATEGORY_MAPPING_DUPLICATE_NON_TERMINAL','برای این Policy، Main Group GUID و دوره یک Mapping باز وجود دارد.',409,{details:{existingMappingId:duplicate.mappingId,policyVersionId:policy.policyVersionId,officialProductCategoryGuid:classification.officialProductCategoryGuid,effectiveFrom,effectiveTo}});
    const now=new Date();const row={mappingId,schemaVersion:classification?GOVERNANCE_SCHEMA_VERSION:SCHEMA_VERSION,policyVersionId:policy.policyVersionId,...identity,...(classification||{}),commissionCategory:legacyCommissionCategory,effectiveFrom,effectiveTo,description:clean(input.description,2000),source:clean(input.source||'accounting-review',200),sourceReference:clean(input.sourceReference,500),reason:clean(input.reason,2000),evidenceMetadata:input.evidenceMetadata&&typeof input.evidenceMetadata==='object'?JSON.parse(JSON.stringify(input.evidenceMetadata)):null,status:'draft',revision:1,createdBy:current,approvedBy:null,auditLog:[audit('mapping-created',current,{policyVersionId:policy.policyVersionId,officialProductCategoryIdentity:classification?.officialProductCategoryIdentity||'',officialProductCategoryGuid:classification?.officialProductCategoryGuid||'',officialProductCategoryName:classification?.officialProductCategoryName||'',commissionRatePool:classification?.commissionRatePool||compatibilityRatePool(legacyCommissionCategory),legacyCommissionCategory,effectiveFrom,effectiveTo,...identity})],createdAt:now,updatedAt:now,softDeleteOnly:true};await db.collection(CATEGORY_MAPPINGS).insertOne(row);return{ok:true,mapping:row,automaticApproval:false};
  }finally{await releaseApprovalLock(lock);}
}
...
function mappingEvidence(row,input={}){const reason=clean(input.reason||row.reason,2000);const sourceReference=clean(input.sourceReference||row.sourceReference,500);const evidenceMetadata=input.evidenceMetadata||row.evidenceMetadata;const evidenceUnavailableReason=clean(input.evidenceUnavailableReason||evidenceMetadata?.evidenceUnavailableReason,1000);return{reason,sourceReference,evidenceMetadata:evidenceMetadata||null,evidenceUnavailableReason};}
async function approvedOperationalMappingOverlap(db,row){
  const categoryQuery={status:'approved'};
  if(clean(row.officialProductCategoryGuid,100))categoryQuery.officialProductCategoryGuid=clean(row.officialProductCategoryGuid,100);
  else Object.assign(categoryQuery,{identityType:row.identityType,identityValue:row.identityValue});
  const candidates=await db.collection(CATEGORY_MAPPINGS).find(categoryQuery).toArray();
  const policyIds=[...new Set(candidates.map(candidate=>clean(candidate.policyVersionId,100)).filter(Boolean))];
  if(!policyIds.length)return false;
  const policies=await db.collection('commissionPolicyVersions').find({policyVersionId:{$in:policyIds},status:'approved'}).toArray();
  const operationalPolicies=new Map(policies.map(policy=>[policy.policyVersionId,policy]));
  return candidates.some(other=>{
    const policy=operationalPolicies.get(other.policyVersionId);
    return other.mappingId!==row.mappingId&&policy&&rangesOverlap(row.effectiveFrom,row.effectiveTo,other.effectiveFrom,other.effectiveTo)&&rangesOverlap(other.effectiveFrom,other.effectiveTo,policy.effectiveFrom,policy.effectiveTo);
  });
}
async function transitionCategoryMapping(db,mappingId,action,input={},requestedBy={}){
  const current=requireRole(requestedBy,['approve','reject','return'].includes(action)?APPROVE_ROLES:EDIT_ROLES);const row=await db.collection(CATEGORY_MAPPINGS).findOne({mappingId:clean(mappingId,100)});if(!row)fail('CATEGORY_MAPPING_NOT_FOUND','Mapping پیدا نشد.',404);if(Number(input.revision)!==Number(row.revision))fail('CATEGORY_MAPPING_CONFLICT','Revision تغییر کرده است.',409);const rules={submit:{from:['draft','returned'],to:'pending'},cancel:{from:['draft','returned'],to:'cancelled'},approve:{from:['pending'],to:'approved'},reject:{from:['pending'],to:'rejected'},return:{from:['pending'],to:'returned'}};const rule=rules[action];if(!rule||!rule.from.includes(row.status))fail('CATEGORY_MAPPING_STATUS_INVALID','انتقال وضعیت Mapping مجاز نیست.',409);if(action==='approve'&&row.createdBy?.username===current.username)fail('CATEGORY_MAPPING_SELF_APPROVAL','تفکیک نقش نقض شده است.',403);const ev=['submit','approve','reject','return'].includes(action)?mappingEvidence(row,input):null;let lock=null;try{if(action==='approve'){lock=await acquireApprovalLock(db,CATEGORY_APPROVAL_LOCKS,`${row.identityType}|${row.identityValue}`,row.mappingId);if(await approvedOperationalMappingOverlap(db,row))fail('CATEGORY_MAPPING_OVERLAP','Mapping approved هم‌پوشان برای Policy عملیاتی وجود دارد.',409);}const now=new Date();const patch={status:rule.to,revision:Number(row.revision)+1,updatedAt:new Date(),auditLog:[...(row.auditLog||[]),audit(`mapping-${action}`,current,{reason:ev?.reason,sourceReference:ev?.sourceReference})].slice(-300)};if(action==='approve')Object.assign(patch,{approvedBy:current,approvedAt:now,approvalReason:ev.reason,approvalEvidenceReference:ev.sourceReference});if(action==='submit')patch.submittedBy=current;if(action==='reject')patch.rejectedBy=current;if(action==='return')patch.returnedBy=current;const result=await db.collection(CATEGORY_MAPPINGS).updateOne({mappingId:row.mappingId,status:row.status,revision:row.revision},{$set:patch});if(!result.matchedCount)fail('CATEGORY_MAPPING_CONFLICT','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,mapping:{...row,...patch},automaticApproval:false,approvalSerialized:action==='approve'};}finally{await releaseApprovalLock(lock);}
}
...
function rateEnvironment(){
  const explicit=clean(process.env.MKCRM_ENV||process.env.APP_ENV||process.env.DEPLOYMENT_ENV,50).toLowerCase();
  if(['production','prod'].includes(explicit))return'PRODUCTION';
  if(['staging','stage','test','development','dev'].includes(explicit))return'STAGING';
  if(/mkcrm[_-]?staging/i.test(String(process.env.MONGO_URI||''))||['1386','11386'].includes(clean(process.env.PORT,10)))return'STAGING';
  return clean(process.env.NODE_ENV,50).toLowerCase()==='production'?'PRODUCTION':'STAGING';
}
function inferredRateRecordClass(row={}){
  const stored=clean(row.recordClass,20).toUpperCase();
  if(RATE_RECORD_CLASSES.includes(stored))return stored;
  if(row.policyVersionId===commissionPolicies.LEGACY_POLICY_ID||row.legacyPolicy||row.historicalFrozen)return'HISTORICAL';
  return rateEnvironment()==='PRODUCTION'?'BUSINESS':'TEST';
}
function rateTargetIdentity(row={}){
  const rateScope=storedRateScope(row);
  return rateScope==='product_category'
    ? clean(row.officialProductCategoryGuid||row.officialProductCategoryIdentity,500)
    : clean(RATE_POOLS.includes(clean(row.commissionRatePool,50).toUpperCase())?row.commissionRatePool:compatibilityRatePool(row.commissionCategory),50);
}
function rateBusinessContent(row={}){
  const rateScope=storedRateScope(row),pool=rateScope==='rate_pool'?(RATE_POOLS.includes(clean(row.commissionRatePool,50).toUpperCase())?clean(row.commissionRatePool,50).toUpperCase():compatibilityRatePool(row.commissionCategory)):'';
  return{policyVersionId:clean(row.policyVersionId,100),sellerIdentity:clean(row.sellerIdentity,100)||'*',rateScope,targetIdentity:rateTargetIdentity(row),officialProductCategoryGuid:clean(row.officialProductCategoryGuid,100),officialProductCategoryIdentity:clean(row.officialProductCategoryIdentity,500),commissionRatePool:pool,rate:clean(row.rate,50),effectiveFrom:clean(row.effectiveFrom,8),effectiveTo:clean(row.effectiveTo,8),supersedesRateVersionId:clean(row.supersedesRateVersionId,100),supersessionReason:clean(row.supersessionReason,2000),description:clean(row.description||row.reason,2000),recordClass:inferredRateRecordClass(row),environment:clean(row.environment,20)||rateEnvironment()};
}
function rateContentHash(row={}){return sha256(stable(rateBusinessContent(row)));}
function rateSourceFingerprint(row={}){return sha256(stable({policyVersionId:clean(row.policyVersionId,100),contentHash:row.contentHash||rateContentHash(row),schemaVersion:Number(row.schemaVersion||1),recordClass:inferredRateRecordClass(row)}));}
function normalizedStoredRate(row={}){
  const rateScope=storedRateScope(row);
  const normalized={...row,rateScope,commissionRatePool:rateScope==='rate_pool'?(RATE_POOLS.includes(clean(row.commissionRatePool,50).toUpperCase())?clean(row.commissionRatePool,50).toUpperCase():compatibilityRatePool(row.commissionCategory)):'',officialProductCategoryIdentity:clean(row.officialProductCategoryIdentity,500),officialProductCategoryName:clean(row.officialProductCategoryName||(rateScope==='product_category'?row.commissionCategory:''),300),legacyCompatibility:!row.rateScope,recordClass:inferredRateRecordClass(row),environment:clean(row.environment,20)||rateEnvironment(),lifecycleStatus:clean(row.lifecycleStatus,30)||(row.status==='approved'?'operational':row.status)};
  normalized.contentHash=row.contentHash||rateContentHash(normalized);normalized.sourceFingerprint=row.sourceFingerprint||rateSourceFingerprint(normalized);return normalized;
}
async function rateScopeForInput(db,input={},existing={}){
  ...
  if(requested&&!RATE_SCOPES.includes(requested))fail('RATE_TARGET_INVALID','Target Type فقط Product Category یا Rate Pool است.');
  ...
  if(rateScope==='rate_pool'){
    ...
    return{policyVersionId:clean(input.policyVersionId||existing.policyVersionId,100),rateScope,commissionRatePool:normalized,officialProductCategoryIdentity:'',officialProductCategoryGuid:'',officialProductCategoryNumber:'',officialProductCategoryName:'',commissionCategory:normalized};
  }
  ...
  return{policyVersionId:clean(input.policyVersionId||existing.policyVersionId,100),rateScope,commissionRatePool:'',...category,commissionCategory:category.officialProductCategoryName};
}
async function requireRatePolicyForPeriod(db,policyVersionId,effectiveFrom,effectiveTo,stage){
  let policy;
  try{policy=await commissionPolicies.getPolicy(db,policyVersionId);}catch(error){if(error.code==='COMMISSION_POLICY_NOT_FOUND')fail('RATE_POLICY_NOT_APPROVED','سیاست Rate پیدا نشد یا مصوب نیست.',409);throw error;}
  if(policy.status!=='approved'||policy.policyVersionId===commissionPolicies.LEGACY_POLICY_ID)fail('RATE_POLICY_NOT_APPROVED',`سیاست Rate در مرحله ${stage} باید approved و operational باشد.`,409);
  if(effectiveFrom<policy.effectiveFrom||(policy.effectiveTo&&effectiveTo>policy.effectiveTo))fail('RATE_EFFECTIVE_PERIOD_INVALID','بازه Rate باید کاملاً داخل بازه Policy باشد.',409,{policyEffectiveFrom:policy.effectiveFrom,policyEffectiveTo:policy.effectiveTo});
  return policy;
}
function requiredRatePeriod(input={},existing={}){
  let effectiveFrom,effectiveTo;
  try{effectiveFrom=date8(input.effectiveFrom??existing.effectiveFrom,'effectiveFrom');effectiveTo=date8(input.effectiveTo??existing.effectiveTo,'effectiveTo');}
  catch(error){fail('RATE_EFFECTIVE_PERIOD_INVALID','effectiveFrom و effectiveTo باید تاریخ شمسی معتبر و هشت‌رقمی باشند.',400,{causeCode:error.code||''});}
  if(effectiveTo<effectiveFrom)fail('RATE_EFFECTIVE_PERIOD_INVALID','بازه نرخ معتبر نیست.');
  return{effectiveFrom,effectiveTo};
}
function canonicalOpenRateQuery(row){
  const normalized=normalizedStoredRate(row);return{policyVersionId:clean(row.policyVersionId,100),sellerIdentity:clean(row.sellerIdentity,100)||'*',rateScope:normalized.rateScope,...(normalized.rateScope==='product_category'?{officialProductCategoryGuid:clean(normalized.officialProductCategoryGuid,100),officialProductCategoryIdentity:clean(normalized.officialProductCategoryIdentity,500)}:{commissionRatePool:normalized.commissionRatePool}),effectiveFrom:row.effectiveFrom,effectiveTo:row.effectiveTo,status:{$in:RATE_OPEN_STATUSES}};
}
async function findOpenRate(db,row){const query=canonicalOpenRateQuery(row);let rows=await db.collection(RATE_VERSIONS).find(query).toArray();rows=rows.filter(candidate=>rateTargetIdentity(candidate)===rateTargetIdentity(row));return rows.find(candidate=>candidate.rateVersionId!==row.rateVersionId)||null;}
async function createRateVersion(db,input,requestedBy,internal={}){
  if(input.supersedesRateVersionId&&!internal.allowSupersession)fail('RATE_SUPERSESSION_INVALID','جایگزینی فقط از workflow نسخه approved مجاز است.',409);
  const current=requireRole(requestedBy,EDIT_ROLES);await ensureIndexes(db);if(input.recordClass!=null||input.environment!=null)fail('RATE_RECORD_CLASS_INVALID','نوع رکورد و محیط توسط Backend تعیین می‌شود و ورودی کاربر نیست.',400);const scope=await rateScopeForInput(db,input);if(input.rate==null||clean(input.rate)==='')fail('RATE_REQUIRED','نرخ خالی با صفر یکسان نیست.');const rate=exactRate(input.rate);if(compare(rate,'0',8)<0||compare(rate,'1',8)>0)fail('RATE_INVALID','نرخ باید بین صفر و یک باشد.');const {effectiveFrom,effectiveTo}=requiredRatePeriod(input);await requireRatePolicyForPeriod(db,scope.policyVersionId,effectiveFrom,effectiveTo,'create');
  const rateVersionId=newId('CRATE'),now=new Date();const row={rateVersionId,schemaVersion:RATE_SCHEMA_VERSION,sellerIdentity:clean(input.sellerIdentity,100)||'*',...scope,effectiveFrom,effectiveTo,rate,description:clean(input.description??input.reason,2000),contractType:clean(input.contractType||'governed-rate',100),sourceDocumentType:clean(input.sourceDocumentType,100),sourceReference:clean(input.sourceReference,500),reason:clean(input.reason,2000),evidenceMetadata:input.evidenceMetadata&&typeof input.evidenceMetadata==='object'?JSON.parse(JSON.stringify(input.evidenceMetadata)):null,supersedesRateVersionId:clean(input.supersedesRateVersionId,100),supersessionReason:clean(input.supersessionReason,2000),supersededByRateVersionId:'',supersessionEffectiveFrom:'',retirementEffectiveFrom:'',lifecycleStatus:'draft',recordClass:rateEnvironment()==='PRODUCTION'?'BUSINESS':'TEST',environment:rateEnvironment(),status:'draft',revision:1,createdBy:current,approvedBy:null,createdAt:now,updatedAt:now,softDeleteOnly:true};row.contentHash=rateContentHash(row);row.sourceFingerprint=rateSourceFingerprint(row);row.auditLog=[audit('rate-created',current,{policyVersionId:scope.policyVersionId,rate,rateScope:scope.rateScope,targetIdentity:rateTargetIdentity(row),effectiveFrom,effectiveTo,recordClass:row.recordClass,supersedesRateVersionId:row.supersedesRateVersionId})];
  const lockKey=`create|${scope.policyVersionId}|${row.sellerIdentity}|${scope.rateScope}|${rateTargetIdentity(row)}|${effectiveFrom}|${effectiveTo}`;let lock=null;try{lock=await acquireApprovalLock(db,RATE_APPROVAL_LOCKS,lockKey,rateVersionId);const duplicate=await findOpenRate(db,row);if(duplicate)fail('RATE_DUPLICATE_OPEN_WORKFLOW','برای این Rate workflow یک رکورد باز وجود دارد.',409,{details:{existingRateVersionId:duplicate.rateVersionId}});await db.collection(RATE_VERSIONS).insertOne(row);return{ok:true,rateVersion:row,automaticApproval:false};}finally{await releaseApprovalLock(lock);}
}
async function updateRateVersion(db,rateVersionId,input,requestedBy){const current=requireRole(requestedBy,EDIT_ROLES);const row=await db.collection(RATE_VERSIONS).findOne({rateVersionId:clean(rateVersionId,100)});if(!row)fail('RATE_NOT_FOUND','Rate version پیدا نشد.',404);if(row.status==='approved')fail('RATE_CLOSED_PERIOD','Rate approved فقط از مسیر supersession یا retirement قابل تغییر است.',409);if(!['draft','returned'].includes(row.status))fail('RATE_EDIT_INVALID','فقط draft یا returned قابل ویرایش است.',409);if(Number(input.revision)!==Number(row.revision))fail('RATE_STALE_REVISION','Revision تغییر کرده است.',409);if(input.recordClass!=null||input.environment!=null||input.supersedesRateVersionId!=null)fail('RATE_RECORD_CLASS_INVALID','فیلدهای سیستمی Rate قابل ویرایش نیستند.',400);const scope=await rateScopeForInput(db,{...row,...input},row),rate=input.rate==null?row.rate:exactRate(input.rate);if(compare(rate,'0',8)<0||compare(rate,'1',8)>0)fail('RATE_INVALID','نرخ باید بین صفر و یک باشد.');const {effectiveFrom,effectiveTo}=requiredRatePeriod(input,row);await requireRatePolicyForPeriod(db,row.policyVersionId,effectiveFrom,effectiveTo,'edit');const patch={sellerIdentity:clean(input.sellerIdentity??row.sellerIdentity,100)||'*',...scope,rate,effectiveFrom,effectiveTo,description:clean(input.description??input.reason??row.description??row.reason,2000),reason:clean(input.reason??row.reason,2000),status:'draft',lifecycleStatus:'draft',revision:Number(row.revision)+1,updatedAt:new Date()};patch.contentHash=rateContentHash({...row,...patch});patch.sourceFingerprint=rateSourceFingerprint({...row,...patch});patch.auditLog=[...(row.auditLog||[]),audit('rate-updated',current,{oldContentHash:row.contentHash||rateContentHash(row),newContentHash:patch.contentHash,oldRate:row.rate,newRate:rate,oldEffectiveFrom:row.effectiveFrom,newEffectiveFrom:effectiveFrom,oldEffectiveTo:row.effectiveTo,newEffectiveTo:effectiveTo,rateScope:scope.rateScope,targetIdentity:rateTargetIdentity({...row,...patch})})].slice(-300);const duplicate=await findOpenRate(db,{...row,...patch});if(duplicate)fail('RATE_DUPLICATE_OPEN_WORKFLOW','برای این Rate workflow یک رکورد باز وجود دارد.',409,{details:{existingRateVersionId:duplicate.rateVersionId}});const result=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:row.rateVersionId,status:row.status,revision:row.revision},{$set:patch});if(!result.matchedCount)fail('RATE_STALE_REVISION','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,rateVersion:{...row,...patch}};}
...
async function transitionRateVersion(db,rateVersionId,action,input={},requestedBy={}){
  const current=requireRole(requestedBy,['approve','reject','return','retire'].includes(action)?APPROVE_ROLES:EDIT_ROLES);await ensureIndexes(db);const row=await db.collection(RATE_VERSIONS).findOne({rateVersionId:clean(rateVersionId,100)});if(!row)fail('RATE_NOT_FOUND','Rate version پیدا نشد.',404);if(Number(input.revision)!==Number(row.revision))fail('RATE_STALE_REVISION','Revision تغییر کرده است.',409);const rules={submit:{from:['draft','returned'],to:'pending'},cancel:{from:['draft','returned'],to:'cancelled'},approve:{from:['pending'],to:'approved'},reject:{from:['pending'],to:'rejected'},return:{from:['pending'],to:'returned'},retire:{from:['approved'],to:'approved'}};const rule=rules[action];if(!rule||!rule.from.includes(row.status))fail('RATE_STATUS_INVALID','انتقال وضعیت Rate مجاز نیست.',409);if(action==='approve'&&row.createdBy?.username===current.username)fail('RATE_SELF_APPROVAL_FORBIDDEN','تفکیک نقش نقض شده است.',403);if(action==='retire'){const reason=clean(input.reason,2000);if(!reason)fail('RATE_SUPERSESSION_INVALID','دلیل بازنشستگی الزامی است.');const retirementEffectiveFrom=date8(input.retirementEffectiveFrom,'retirementEffectiveFrom');if(retirementEffectiveFrom<row.effectiveFrom||retirementEffectiveFrom>row.effectiveTo)fail('RATE_EFFECTIVE_PERIOD_INVALID','تاریخ بازنشستگی باید داخل بازه Rate باشد.');const patch={lifecycleStatus:'retired',retirementEffectiveFrom,retirementReason:reason,retiredBy:current,retiredAt:new Date(),revision:Number(row.revision)+1,updatedAt:new Date(),auditLog:[...(row.auditLog||[]),audit('rate-retired',current,{reason,retirementEffectiveFrom})].slice(-300)};const result=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:row.rateVersionId,status:'approved',revision:row.revision},{$set:patch});if(!result.matchedCount)fail('RATE_STALE_REVISION','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,rateVersion:{...row,...patch},automaticApproval:false};}
  if(['submit','approve'].includes(action))await requireRatePolicyForPeriod(db,row.policyVersionId,row.effectiveFrom,row.effectiveTo,action);const ev=['submit','approve','reject','return'].includes(action)?rateEvidence(row,input):null;if(action!=='approve'){const patch={status:rule.to,lifecycleStatus:rule.to,revision:Number(row.revision)+1,updatedAt:new Date(),auditLog:[...(row.auditLog||[]),audit(`rate-${action}`,current,{reason:ev?.reason})].slice(-300)};if(action==='submit')patch.submittedBy=current;if(action==='reject')patch.rejectedBy=current;if(action==='return')patch.returnedBy=current;const result=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:row.rateVersionId,status:row.status,revision:row.revision},{$set:patch});if(!result.matchedCount)fail('RATE_STALE_REVISION','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,rateVersion:{...row,...patch},automaticApproval:false};}
  ...
  try{
    ...
    const rows=(await commissionPolicies.attachPolicyBindings(db,'rate_version',await db.collection(RATE_VERSIONS).find({status:'approved',sellerIdentity:row.sellerIdentity}).toArray())).map(normalizedStoredRate);const policyIds=[...new Set(rows.map(other=>other.policyVersionId).filter(Boolean))];const policyRows=policyIds.length?await db.collection(commissionPolicies.POLICIES).find({policyVersionId:{$in:policyIds},status:'approved'}).toArray():[];const operationalPolicies=new Map(policyRows.map(policy=>[policy.policyVersionId,policy]));const conflict=rows.some(other=>{const owner=operationalPolicies.get(other.policyVersionId);if(!owner||other.rateVersionId===row.rateVersionId||other.rateVersionId===row.supersedesRateVersionId||inferredRateRecordClass(other)==='HISTORICAL'||other.rateScope!==normalized.rateScope||rateTargetIdentity(other)!==rateTargetIdentity(normalized))return false;const operationalFrom=other.effectiveFrom>owner.effectiveFrom?other.effectiveFrom:owner.effectiveFrom,operationalTo=[operationalRateEnd(other),owner.effectiveTo||'99999999'].sort()[0];return operationalFrom<=operationalTo&&rangesOverlap(row.effectiveFrom,row.effectiveTo,operationalFrom,operationalTo);});
    if(conflict)fail('RATE_OVERLAP','نرخ approved هم‌پوشان زیر Policy عملیاتی وجود دارد.',409);
    let superseded=null;if(row.supersedesRateVersionId){superseded=await db.collection(RATE_VERSIONS).findOne({rateVersionId:row.supersedesRateVersionId,status:'approved'});if(!superseded)fail('RATE_SUPERSESSION_INVALID','نسخه مبنای جایگزینی approved پیدا نشد.',409);if(!clean(row.supersessionReason))fail('RATE_SUPERSESSION_INVALID','دلیل جایگزینی الزامی است.');if(rateTargetIdentity(superseded)!==rateTargetIdentity(row)||storedRateScope(superseded)!==normalized.rateScope||clean(superseded.sellerIdentity,100)!==clean(row.sellerIdentity,100))fail('RATE_SUPERSESSION_INVALID','نسخه جایگزین باید همان Seller/Scope/Target را حفظ کند.',409);}
    const approvedAt=new Date();const patch={status:'approved',lifecycleStatus:'operational',revision:Number(row.revision)+1,approvedBy:current,approvedAt,approvalReason:ev.reason,updatedAt:approvedAt,approvedContentHash:row.contentHash||rateContentHash(row),auditLog:[...(row.auditLog||[]),audit('rate-approved',current,{reason:ev.reason,supersedesRateVersionId:row.supersedesRateVersionId})].slice(-300)};const updated=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:row.rateVersionId,status:row.status,revision:row.revision},{$set:patch});if(!updated.matchedCount)fail('RATE_STALE_REVISION','Rate version هم‌زمان تغییر کرده است.',409);const approvedRow={...row,...patch};
    if(superseded){const oldPatch={supersededByRateVersionId:row.rateVersionId,supersessionEffectiveFrom:row.effectiveFrom,lifecycleStatus:'superseded',revision:Number(superseded.revision||1)+1,updatedAt:approvedAt,auditLog:[...(superseded.auditLog||[]),audit('rate-superseded',current,{replacementRateVersionId:row.rateVersionId,supersessionEffectiveFrom:row.effectiveFrom,reason:row.supersessionReason})].slice(-300)};const oldUpdated=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:superseded.rateVersionId,status:'approved',revision:superseded.revision},{$set:oldPatch});if(!oldUpdated.matchedCount)fail('RATE_SUPERSESSION_INVALID','نسخه مبنا هم‌زمان تغییر کرده است؛ بازبینی لازم است.',409);}
    return{ok:true,rateVersion:approvedRow,approvalSerialized:true,supersededRateVersionId:superseded?.rateVersionId||''};
...
function operationalRateEnd(row={}){let end=clean(row.effectiveTo,8)||'99999999';for(const cutoff of [row.supersessionEffectiveFrom,row.retirementEffectiveFrom].map(value=>clean(value,8)).filter(Boolean)){const previous=shiftJalaliDate(cutoff,-1,{field:'rateLifecycleDate'});if(previous<end)end=previous;}return end;}
function operationalRateAt(row,saleDate){return row.effectiveFrom<=saleDate&&operationalRateEnd(row)>=saleDate;}
function resolveRateFromRows(rows,sellerIdentity,classification,saleDate){
  ...
  const expectedClass=rateEnvironment()==='PRODUCTION'?'BUSINESS':'TEST';const eligible=rows.map(normalizedStoredRate).filter(row=>operationalRateAt(row,saleDate)&&row.recordClass===expectedClass&&(!value.policyVersionId||row.policyVersionId===value.policyVersionId));
  ...
  for(const level of levels){const matches=eligible.filter(level.test);if(matches.length===1)return{status:'resolved',code:null,rateVersion:matches[0],appliedRateVersionId:matches[0].rateVersionId,appliedRateExact:matches[0].rate,appliedRateScope:matches[0].rateScope,appliedPolicyVersionId:matches[0].policyVersionId||value.policyVersionId||'',precedence:level.precedence,rateResolutionPrecedence:level.precedence,rateContentHash:matches[0].contentHash,governanceSourceFingerprint:matches[0].sourceFingerprint};if(matches.length>1)return{status:'ambiguous',code:'RATE_CONFLICT',rateVersion:null,appliedRateVersionId:'',appliedPolicyVersionId:value.policyVersionId||'',appliedRateScope:'',precedence:level.precedence};}
  return{status:'missing',code:'RATE_NOT_FOUND',rateVersion:null,appliedRateVersionId:'',appliedRateExact:null,appliedRateScope:'',precedence:''};
}
...
async function listRateVersions(db,filters={}){await ensureIndexes(db);let rows=await db.collection(RATE_VERSIONS).find({}).toArray();if(filters.status)rows=rows.filter(row=>row.status===clean(filters.status));if(filters.policyVersionId)rows=rows.filter(row=>row.policyVersionId===clean(filters.policyVersionId));if(filters.sellerIdentity)rows=rows.filter(row=>row.sellerIdentity===clean(filters.sellerIdentity));rows=rows.map(normalizedStoredRate);return{ok:true,...pageRows(rows,filters),rateScopes:RATE_SCOPES,ratePools:RATE_POOLS,recordClasses:RATE_RECORD_CLASSES,environment:rateEnvironment(),precedence:['seller/category','seller/rate-pool','category-default','rate-pool-default'],historicalVersionPinned:true,historicalRecordsPreserved:true,missingRateIsZero:false,commissionPayable:false};}
async function createSupersedingRateVersion(db,rateVersionId,input,requestedBy){const current=requireRole(requestedBy,EDIT_ROLES);const old=await db.collection(RATE_VERSIONS).findOne({rateVersionId:clean(rateVersionId,100)});if(!old||old.status!=='approved')fail('RATE_SUPERSESSION_INVALID','فقط Rate approved قابل جایگزینی است.',409);const reason=clean(input.reason||input.supersessionReason,2000);if(!reason)fail('RATE_SUPERSESSION_INVALID','دلیل جایگزینی الزامی است.');const normalized=normalizedStoredRate(old);return createRateVersion(db,{policyVersionId:old.policyVersionId,sellerIdentity:old.sellerIdentity,rateScope:normalized.rateScope,commissionRatePool:normalized.commissionRatePool,officialProductCategoryIdentity:normalized.officialProductCategoryIdentity,officialProductCategoryName:normalized.officialProductCategoryName,rate:input.rate,effectiveFrom:input.effectiveFrom,effectiveTo:input.effectiveTo,description:input.description,supersedesRateVersionId:old.rateVersionId,supersessionReason:reason},current,{allowSupersession:true});}
...
async function governedLegacyRecord(db,recordType,id){
  const collection=recordType==='category_mapping'?CATEGORY_MAPPINGS:RATE_VERSIONS;
  const field=recordType==='category_mapping'?'mappingId':'rateVersionId';
  const row=await db.collection(collection).findOne({[field]:clean(id,100)});
  if(!row)fail(recordType==='category_mapping'?'CATEGORY_MAPPING_NOT_FOUND':'RATE_NOT_FOUND','رکورد پیدا نشد.',404);
  if(!row.policyVersionId)fail('LEGACY_POLICY_RECORD_IMMUTABLE','رکورد تاریخی متصل به LEGACY_PRE_POLICY فقط خواندنی است.',409);
  await commissionPolicies.requireSelectablePolicy(db,row.policyVersionId);
  return row;
}
async function governedUpdateCategoryMapping(db,id,input,requestedBy){const row=await governedLegacyRecord(db,'category_mapping',id);if(input.policyVersionId&&input.policyVersionId!==row.policyVersionId)fail('CATEGORY_POLICY_IMMUTABLE','سیاست Mapping پس از ایجاد قابل تغییر نیست.',409);return updateCategoryMapping(db,id,input,requestedBy);}
async function governedTransitionCategoryMapping(db,id,action,input,requestedBy){await governedLegacyRecord(db,'category_mapping',id);return transitionCategoryMapping(db,id,action,input,requestedBy);}
async function governedListCategoryMappings(db,filters={}){const result=await listCategoryMappings(db,filters);result.list=(await commissionPolicies.attachPolicyBindings(db,'category_mapping',result.list)).map(row=>({...row,historicalFrozen:row.policyVersionId===commissionPolicies.LEGACY_POLICY_ID}));return result;}
async function requireGovernedRatePolicy(db,policyVersionId){try{return await commissionPolicies.requireSelectablePolicy(db,policyVersionId);}catch(error){if(['COMMISSION_POLICY_REQUIRED','COMMISSION_POLICY_NOT_APPROVED','COMMISSION_POLICY_NOT_FOUND','LEGACY_POLICY_NOT_SELECTABLE'].includes(error.code))fail('RATE_POLICY_NOT_APPROVED','سیاست Rate باید approved و operational باشد.',409);throw error;}}
async function governedCreateRateVersion(db,input,requestedBy){const policy=await requireGovernedRatePolicy(db,input.policyVersionId);return createRateVersion(db,{...input,policyVersionId:policy.policyVersionId},requestedBy);}
async function governedUpdateRateVersion(db,id,input,requestedBy){const row=await governedLegacyRecord(db,'rate_version',id);if(input.policyVersionId&&input.policyVersionId!==row.policyVersionId)fail('RATE_POLICY_IMMUTABLE','سیاست Rate پس از ایجاد قابل تغییر نیست.',409);return updateRateVersion(db,id,input,requestedBy);}
async function governedTransitionRateVersion(db,id,action,input,requestedBy){const row=await db.collection(RATE_VERSIONS).findOne({rateVersionId:clean(id,100)});if(!row)fail('RATE_NOT_FOUND','رکورد پیدا نشد.',404);if(!row.policyVersionId)fail('LEGACY_POLICY_RECORD_IMMUTABLE','رکورد تاریخی فقط خواندنی است.',409);if(['submit','approve','retire'].includes(action))await requireGovernedRatePolicy(db,row.policyVersionId);return transitionRateVersion(db,id,action,input,requestedBy);}
async function governedCreateSupersedingRateVersion(db,id,input,requestedBy){const row=await governedLegacyRecord(db,'rate_version',id);return createSupersedingRateVersion(db,row.rateVersionId,input,requestedBy);}
async function governedListRateVersions(db,filters={}){const result=await listRateVersions(db,filters);result.list=(await commissionPolicies.attachPolicyBindings(db,'rate_version',result.list)).map(row=>({...row,historicalFrozen:row.policyVersionId===commissionPolicies.LEGACY_POLICY_ID}));return result;}
async function governedResolveRate(db,sellerIdentity,classification,saleDate){const normalizedDate=date8(saleDate,'saleDate');const policyVersionId=clean(classification?.policyVersionId,100);const query={status:'approved',sellerIdentity:{$in:[clean(sellerIdentity,100),'*']},effectiveFrom:{$lte:normalizedDate},effectiveTo:{$gte:normalizedDate}};if(policyVersionId)query.policyVersionId=policyVersionId;let rows=await commissionPolicies.attachPolicyBindings(db,'rate_version',await db.collection(RATE_VERSIONS).find(query).toArray());const policyIds=[...new Set(rows.map(row=>row.policyVersionId).filter(Boolean))];const operational=policyIds.length?await db.collection(commissionPolicies.POLICIES).find({policyVersionId:{$in:policyIds},status:'approved'}).toArray():[];const operationalIds=new Set(operational.map(row=>row.policyVersionId));rows=rows.filter(row=>operationalIds.has(row.policyVersionId));return resolveRateFromRows(rows,sellerIdentity,classification,normalizedDate);}
async function disabledSeedTirRateCandidates(){fail('RATE_CANDIDATE_SEED_RETIRED','نرخ‌های ۱۴٪ و ۲۰٪ دیگر به‌صورت پنهان ایجاد نمی‌شوند؛ نرخ باید زیر Policy مصوب ثبت شود.',409);}
...
module.exports={
  ...
  SCHEMA_VERSION,GOVERNANCE_SCHEMA_VERSION,RATE_SCHEMA_VERSION,MODULE_VERSION,POOLS,RATE_POOLS,RATE_SCOPES,RATE_OPEN_STATUSES,RATE_RECORD_CLASSES,CATEGORIES,ADJUSTMENT_TYPES,ADJUSTMENT_STATUSES,SAVED_ENTRY_TYPES,SUPPLIER_INCENTIVE_TYPES,IMMUTABLE_EXPORT_FIELDS,LEGACY_IMMUTABLE_EXPORT_FIELDS,EDITABLE_EXPORT_FIELDS,TIR_ISSUES,
  ensureIndexes,materializeFifoProfitFacts,listFacts,createAdjustment,updateAdjustmentDraft,transitionAdjustment,reverseAdjustment,listAdjustments,listSavedLedger,savedBalance,listSupplierIncentives,createCategoryMapping,updateCategoryMapping:governedUpdateCategoryMapping,transitionCategoryMapping:governedTransitionCategoryMapping,approveCategoryMapping:(db,id,input,user)=>governedTransitionCategoryMapping(db,id,'approve',input,user),listCategoryMappings:governedListCategoryMappings,resolveCategory,createRateVersion:governedCreateRateVersion,createSupersedingRateVersion:governedCreateSupersedingRateVersion,updateRateVersion:governedUpdateRateVersion,transitionRateVersion:governedTransitionRateVersion,approveRateVersion:(db,id,input,user)=>governedTransitionRateVersion(db,id,'approve',input,user),listRateVersions:governedListRateVersions,resolveRate:governedResolveRate,seedTirRateCandidates:disabledSeedTirRateCandidates,extractInvoiceDiscountFacts,listDiscountFacts,calculateDraftCommission,commissionReport,createExcelExport,importExcelEdits,listExcelAudit,buildTirReconstruction,readTirReconstruction,health,
  _stable:stable,_sha256:sha256,_exact:exact,_add:add,_subtract:subtract,_multiplyMoneyRate:multiplyMoneyRate,_immutableExportProjection:immutableExportProjection,_normalizeAdjustment:normalizeAdjustment,_resolveCategoryFromMappings:resolveCategoryFromMappings,_resolveRateFromRows:resolveRateFromRows,_mappingRatePool:mappingRatePool,_normalizedStoredRate:normalizedStoredRate,_rateContentHash:rateContentHash,_rateEnvironment:rateEnvironment,_operationalRateEnd:operationalRateEnd
};
