...
async function materializeFifoProfitFacts(db, input = {}, requestedBy = {}) {
  ...
  for (const [saleLineIdentity, rows] of grouped) {
    ...
    const category = resolveCategoryFromMappings(approvedCategoryMappings, { ...saleLine, ...first }, clean(first.saleDate, 8));
...
async function createCategoryMapping(db,input,requestedBy){
  ...
  const now=new Date();const row={mappingId:newId('CMAP'),schemaVersion:SCHEMA_VERSION,...identity,commissionCategory,effectiveFrom,effectiveTo,source:clean(input.source||'accounting-review',200),sourceReference:clean(input.sourceReference,500),reason:clean(input.reason,2000),evidenceMetadata:input.evidenceMetadata&&typeof input.evidenceMetadata==='object'?JSON.parse(JSON.stringify(input.evidenceMetadata)):null,status:'draft',revision:1,createdBy:current,approvedBy:null,auditLog:[audit('mapping-created',current,{commissionCategory,...identity})],createdAt:now,updatedAt:now,softDeleteOnly:true};await db.collection(CATEGORY_MAPPINGS).insertOne(row);return{ok:true,mapping:row,automaticApproval:false};
}
async function updateCategoryMapping(db,mappingId,input,requestedBy){
  const current=requireRole(requestedBy,EDIT_ROLES);const row=await db.collection(CATEGORY_MAPPINGS).findOne({mappingId:clean(mappingId,100)});if(!row)fail('CATEGORY_MAPPING_NOT_FOUND','Mapping پیدا نشد.',404);if(!['draft','returned'].includes(row.status))fail('CATEGORY_MAPPING_EDIT_INVALID','فقط draft یا returned قابل ویرایش است.',409);if(Number(input.revision)!==Number(row.revision))fail('CATEGORY_MAPPING_CONFLICT','Revision تغییر کرده است.',409);const identity=categoryIdentity({...row,...input});const commissionCategory=clean(input.commissionCategory||row.commissionCategory).toUpperCase();if(!CATEGORIES.includes(commissionCategory))fail('CATEGORY_INVALID','دسته کمیسیون معتبر نیست.');const effectiveFrom=date8(input.effectiveFrom||row.effectiveFrom,'effectiveFrom');const effectiveTo=date8(input.effectiveTo??row.effectiveTo,'effectiveTo',true);if(effectiveTo&&effectiveTo<effectiveFrom)fail('CATEGORY_RANGE_INVALID','بازه دسته‌بندی معتبر نیست.');const patch={...identity,commissionCategory,effectiveFrom,effectiveTo,source:clean(input.source??row.source,200),sourceReference:clean(input.sourceReference??row.sourceReference,500),reason:clean(input.reason??row.reason,2000),evidenceMetadata:input.evidenceMetadata??row.evidenceMetadata??null,status:'draft',revision:Number(row.revision)+1,updatedAt:new Date(),auditLog:[...(row.auditLog||[]),audit('mapping-updated',current,{reason:input.reason})].slice(-300)};const result=await db.collection(CATEGORY_MAPPINGS).updateOne({mappingId:row.mappingId,status:row.status,revision:row.revision},{$set:patch});if(!result.matchedCount)fail('CATEGORY_MAPPING_CONFLICT','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,mapping:{...row,...patch}};
}
...
async function createRateVersion(db,input,requestedBy){
  const current=requireRole(requestedBy,EDIT_ROLES);await ensureIndexes(db);const commissionCategory=clean(input.commissionCategory).toUpperCase();if(!CATEGORIES.includes(commissionCategory)||commissionCategory==='UNKNOWN')fail('RATE_CATEGORY_INVALID','دسته نرخ معتبر نیست.');if(input.rate==null||clean(input.rate)==='')fail('RATE_REQUIRED','نرخ خالی با صفر یکسان نیست.');const rate=exact(input.rate,8);if(compare(rate,'0',8)<0||compare(rate,'1',8)>0)fail('RATE_INVALID','نرخ باید بین صفر و یک باشد.');const effectiveFrom=date8(input.effectiveFrom,'effectiveFrom');const effectiveTo=date8(input.effectiveTo,'effectiveTo',true);if(effectiveTo&&effectiveTo<effectiveFrom)fail('RATE_RANGE_INVALID','بازه نرخ معتبر نیست.');
  const now=new Date();const row={rateVersionId:newId('CRATE'),schemaVersion:SCHEMA_VERSION,sellerIdentity:clean(input.sellerIdentity,100)||'*',commissionCategory,effectiveFrom,effectiveTo,rate,contractType:clean(input.contractType||'candidate',100),sourceDocumentType:clean(input.sourceDocumentType,100),sourceReference:clean(input.sourceReference,500),reason:clean(input.reason,2000),evidenceMetadata:input.evidenceMetadata&&typeof input.evidenceMetadata==='object'?JSON.parse(JSON.stringify(input.evidenceMetadata)):null,status:'draft',revision:1,createdBy:current,approvedBy:null,auditLog:[audit('rate-created',current,{rate,commissionCategory,effectiveFrom,effectiveTo})],createdAt:now,updatedAt:now,softDeleteOnly:true};await db.collection(RATE_VERSIONS).insertOne(row);return{ok:true,rateVersion:row,automaticApproval:false};
}
async function updateRateVersion(db,rateVersionId,input,requestedBy){const current=requireRole(requestedBy,EDIT_ROLES);const row=await db.collection(RATE_VERSIONS).findOne({rateVersionId:clean(rateVersionId,100)});if(!row)fail('RATE_NOT_FOUND','Rate version پیدا نشد.',404);if(!['draft','returned'].includes(row.status))fail('RATE_EDIT_INVALID','فقط draft یا returned قابل ویرایش است.',409);if(Number(input.revision)!==Number(row.revision))fail('RATE_CONFLICT','Revision تغییر کرده است.',409);const commissionCategory=clean(input.commissionCategory||row.commissionCategory).toUpperCase();const rate=input.rate==null?row.rate:exact(input.rate,8);if(!CATEGORIES.includes(commissionCategory)||commissionCategory==='UNKNOWN')fail('RATE_CATEGORY_INVALID','دسته نرخ معتبر نیست.');if(compare(rate,'0',8)<0||compare(rate,'1',8)>0)fail('RATE_INVALID','نرخ باید بین صفر و یک باشد.');const effectiveFrom=date8(input.effectiveFrom||row.effectiveFrom,'effectiveFrom');const effectiveTo=date8(input.effectiveTo??row.effectiveTo,'effectiveTo',true);if(effectiveTo&&effectiveTo<effectiveFrom)fail('RATE_RANGE_INVALID','بازه نرخ معتبر نیست.');const patch={sellerIdentity:clean(input.sellerIdentity??row.sellerIdentity,100)||'*',commissionCategory,rate,effectiveFrom,effectiveTo,contractType:clean(input.contractType??row.contractType,100),sourceDocumentType:clean(input.sourceDocumentType??row.sourceDocumentType,100),sourceReference:clean(input.sourceReference??row.sourceReference,500),reason:clean(input.reason??row.reason,2000),evidenceMetadata:input.evidenceMetadata??row.evidenceMetadata??null,status:'draft',revision:Number(row.revision)+1,updatedAt:new Date(),auditLog:[...(row.auditLog||[]),audit('rate-updated',current,{reason:input.reason})].slice(-300)};const result=await db.collection(RATE_VERSIONS).updateOne({rateVersionId:row.rateVersionId,status:row.status,revision:row.revision},{$set:patch});if(!result.matchedCount)fail('RATE_CONFLICT','Revision هم‌زمان تغییر کرده است.',409);return{ok:true,rateVersion:{...row,...patch}};}
...
function resolveRateFromRows(rows,sellerIdentity,category,saleDate){const eligible=rows.filter(row=>row.commissionCategory===category&&(row.sellerIdentity===sellerIdentity||row.sellerIdentity==='*')&&row.effectiveFrom<=saleDate&&(!row.effectiveTo||row.effectiveTo>=saleDate)).sort((a,b)=>(a.sellerIdentity==='*')-(b.sellerIdentity==='*'));if(eligible.length!==1&&!(eligible.length>1&&eligible[0].sellerIdentity!=='*'&&eligible[1].sellerIdentity==='*'))return{status:eligible.length?'ambiguous':'missing',rateVersion:null};return{status:'resolved',rateVersion:eligible[0]};}
async function resolveRate(db,sellerIdentity,category,saleDate){const rows=await db.collection(RATE_VERSIONS).find({status:'approved',commissionCategory:category}).toArray();return resolveRateFromRows(rows,sellerIdentity,category,saleDate);}
...
async function seedTirRateCandidates(db,requestedBy){const current=requireRole(requestedBy,EDIT_ROLES);const seeds=[{sellerIdentity:'*',commissionCategory:'NOTEBOOK',rate:'0.14000000'},{sellerIdentity:'*',commissionCategory:'COMPONENT',rate:'0.20000000'}];const created=[];for(const seed of seeds){const existing=await db.collection(RATE_VERSIONS).findOne({...seed,effectiveFrom:'14050401',effectiveTo:'14050431',sourceReference:'TIR-1405-WORKBOOK-EVIDENCE'});if(existing){created.push(existing);continue;}const draft=(await createRateVersion(db,{...seed,effectiveFrom:'14050401',effectiveTo:'14050431',contractType:'tir-workbook-candidate',sourceDocumentType:'Tir 1405 analyzed workbook',sourceReference:'TIR-1405-WORKBOOK-EVIDENCE',reason:'Workbook-derived candidate; human approval required.',evidenceMetadata:{candidateOnly:true}},current)).rateVersion;const pending=(await transitionRateVersion(db,draft.rateVersionId,'submit',{revision:draft.revision,reason:draft.reason,sourceReference:draft.sourceReference,evidenceMetadata:draft.evidenceMetadata},current)).rateVersion;created.push(pending);}return{ok:true,list:created,approved:0,evidenceOnly:true};}
...
module.exports={
  ...
  _stable:stable,_sha256:sha256,_exact:exact,_add:add,_subtract:subtract,_multiplyMoneyRate:multiplyMoneyRate,_immutableExportProjection:immutableExportProjection,_normalizeAdjustment:normalizeAdjustment,_resolveCategoryFromMappings:resolveCategoryFromMappings,_resolveRateFromRows:resolveRateFromRows
};
