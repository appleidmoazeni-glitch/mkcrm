...
function resolveCategoryFromMappings(candidates, saleLine, saleDate) {
  ...
  for (const [identityType, identityValue] of ordered) {
    ...
    if (rows.length === 1) return { ...productCategory, category:productCategory.officialProductCategoryName, commissionRatePool:mappingRatePool(rows[0]), mappingId:rows[0].mappingId, policyVersionId:rows[0].policyVersionId||'', status:mappingRatePool(rows[0])==='UNRESOLVED'?'unresolved':'resolved', legacyRecord:!rows[0].commissionRatePool };
...
async function listAdjustments(db, filters = {}) {
  ...
  rows=rows.map(row=>recordClassification.decorate('adjustment',row));rows.sort((a,b)=>new Date(b.updatedAt||0)-new Date(a.updatedAt||0));
...
async function listSavedLedger(db, filters={}){
  await ensureIndexes(db);let rows=await db.collection(SAVED_LEDGER).find({}).toArray();if(filters.pool)rows=rows.filter(row=>row.pool===clean(filters.pool).toUpperCase());rows=rows.map(row=>recordClassification.decorate('ledger',row));rows.sort((a,b)=>new Date(a.postedAt)-new Date(b.postedAt));
...
async function createCategoryMapping(db,input,requestedBy){
  ...
  try{
    ...
    if(duplicate)return{ok:true,mapping:duplicate,created:false,openedExisting:true,automaticApproval:false,message:'پیش‌نویس باز موجود شد؛ رکورد جدیدی ایجاد نشد.'};
...
function resolveRateFromRows(rows,sellerIdentity,classification,saleDate){
  ...
  const eligible=rows.map(normalizedStoredRate).filter(row=>row.effectiveFrom<=saleDate&&(!row.effectiveTo||row.effectiveTo>=saleDate)&&(!value.policyVersionId||row.policyVersionId===value.policyVersionId));
  ...
  for(const level of levels){const matches=eligible.filter(level.test);if(matches.length===1)return{status:'resolved',rateVersion:matches[0],appliedRateVersionId:matches[0].rateVersionId,appliedPolicyVersionId:matches[0].policyVersionId||value.policyVersionId||'',appliedRateScope:matches[0].rateScope,precedence:level.precedence};if(matches.length>1)return{status:'ambiguous',rateVersion:null,appliedRateVersionId:'',appliedPolicyVersionId:value.policyVersionId||'',appliedRateScope:'',precedence:level.precedence};}
...
async function governedListCategoryMappings(db,filters={}){const result=await listCategoryMappings(db,filters);const policies=await db.collection(commissionPolicies.POLICIES).find({}).toArray(),names=new Map(policies.map(row=>[row.policyVersionId,row.name]));result.list=(await commissionPolicies.attachPolicyBindings(db,'category_mapping',result.list)).map(row=>recordClassification.decorate('mapping',{...row,historicalFrozen:row.policyVersionId===commissionPolicies.LEGACY_POLICY_ID},{policyName:names.get(row.policyVersionId)||''}));return result;}
async function governedCreateRateVersion(db,input,requestedBy){const policy=await commissionPolicies.requireSelectablePolicy(db,input.policyVersionId);return createRateVersion(db,{...input,policyVersionId:policy.policyVersionId},requestedBy);}
...
async function governedTransitionRateVersion(db,id,action,input,requestedBy){await governedLegacyRecord(db,'rate_version',id);return transitionRateVersion(db,id,action,input,requestedBy);}
async function governedListRateVersions(db,filters={}){const result=await listRateVersions(db,filters);const policies=await db.collection(commissionPolicies.POLICIES).find({}).toArray(),names=new Map(policies.map(row=>[row.policyVersionId,row.name]));result.list=(await commissionPolicies.attachPolicyBindings(db,'rate_version',result.list)).map(row=>recordClassification.decorate('rate',{...row,historicalFrozen:row.policyVersionId===commissionPolicies.LEGACY_POLICY_ID},{policyName:names.get(row.policyVersionId)||''}));return result;}
async function governedResolveRate(db,sellerIdentity,classification,saleDate){let rows=await commissionPolicies.attachPolicyBindings(db,'rate_version',await db.collection(RATE_VERSIONS).find({status:'approved'}).toArray());if(classification?.policyVersionId)rows=rows.filter(row=>row.policyVersionId===classification.policyVersionId);return resolveRateFromRows(rows,sellerIdentity,classification,saleDate);}
