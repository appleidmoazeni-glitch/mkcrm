...
async function createExcelExport(db,input={},requestedBy={}){
  const current=requireRole(requestedBy,EDIT_ROLES);await ensureIndexes(db);const fifoDatasetId=clean(input.fifoDatasetId,100)||(await fifoShadow.activeDataset(db))?.datasetId;let facts=await db.collection(FIFO_FACTS).find({fifoDatasetId}).toArray();if(input.periodFrom)facts=facts.filter(row=>row.saleDate>=date8(input.periodFrom,'periodFrom'));if(input.periodTo)facts=facts.filter(row=>row.saleDate<=date8(input.periodTo,'periodTo'));facts.sort((a,b)=>a.saleLineIdentity.localeCompare(b.saleLineIdentity));const exportBatchId=newId('XEXP');const adjustments=await db.collection(ADJUSTMENTS).find({fifoDatasetId,status:'approved'}).toArray();const adjustmentsByLine=new Map();for(const row of adjustments){if(!adjustmentsByLine.has(row.saleLineIdentity))adjustmentsByLine.set(row.saleLineIdentity,[]);adjustmentsByLine.get(row.saleLineIdentity).push(row);}const approvedCategoryMappings=await db.collection(CATEGORY_MAPPINGS).find({status:'approved'}).toArray();const approvedRates=await db.collection(RATE_VERSIONS).find({status:'approved'}).toArray();const rows=[];
  for(const fact of facts){const currentApprovedAdjustmentsExact=add((adjustmentsByLine.get(fact.saleLineIdentity)||[]).map(row=>row.approvedAmountExact||0));const remapped=fact.commissionCategory==='UNKNOWN'?resolveCategoryFromMappings(approvedCategoryMappings,fact,fact.saleDate):{category:fact.commissionCategory,status:'resolved'};const effectiveCategory=remapped.status==='resolved'?remapped.category:fact.commissionCategory;const rate=resolveRateFromRows(approvedRates,fact.sellerIdentity,effectiveCategory,fact.saleDate);const base={exportBatchId,rowId:deterministicId('XROW',`${exportBatchId}|${fact.saleLineIdentity}`),fifoDatasetId,fifoAlgorithmVersion:fact.fifoAlgorithmVersion,saleInvoiceIdentity:fact.saleInvoiceIdentity,saleLineIdentity:fact.saleLineIdentity,itemGuid:fact.itemGuid,itemCode:fact.itemCode,sellerIdentity:fact.sellerIdentity,commissionCategory:effectiveCategory,quantityExact:fact.quantityExact,saleAmountExact:fact.saleAmountExact,invoiceDiscountExact:fact.invoiceDiscountExact,fifoCostExact:fact.fifoCostExact,actualFifoProfitExact:fact.actualFifoProfitExact,costCoverageStatus:fact.costCoverageStatus,currentApprovedAdjustmentsExact,savedProfitPool:POOLS.includes(effectiveCategory)?effectiveCategory:'',commissionRateVersionId:rate.rateVersion?.rateVersionId||''};const integrityHash=sha256(stable(immutableExportProjection(base)));rows.push({...base,integrityHash,...Object.fromEntries(EDITABLE_EXPORT_FIELDS.map(field=>[field,'']))});}
