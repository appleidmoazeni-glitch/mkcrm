...
async function apply(db, authoritativeRows = [], options = {}) {
  ...
  try {
    for (const rename of current.renames) {
      ...
      // Legacy catalog rows may predate normalizedItemCode/canonicalItemGuid fields. The row was
      // resolved by immutable _id and normalized in memory above; raw GUID/code equality keeps the
      // write concurrency-safe without requiring those newer fields to exist already.
      const write=await db.collection(catalog.CATALOG).updateOne({ _id:before._id, itemGuid:before.itemGuid, itemCode:before.itemCode },{ $set:update,$push:{ codeHistory:history } });
