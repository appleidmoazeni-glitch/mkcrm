...
function flattenStatementResult(result = []) {
  ...
  for (const acc of result || []) {
    ...
    for (const r of rows) {
      ...
      out.push({ ...r, InvNo:r.InvNo ?? r.InvoiceNumber ?? r.DocumentNumber ?? '', InvTyp:Number(r.InvTyp ?? r.InvoiceType ?? 0)||0, AccountNumber: r.AccountNumber || accountNumber, AccountName: r.AccountName || accountName, AccountGuId: r.AccountGuId || accountGuId });
    }
  }
  return out;
}
