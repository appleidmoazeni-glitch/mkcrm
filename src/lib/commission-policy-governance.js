...
async function listPolicies(db,filters={},requestedBy={}){requireRole(requestedBy,READ_ROLES);await ensureIndexes(db);let rows=await db.collection(POLICIES).find({}).sort({createdAt:-1}).toArray();if(filters.status)rows=rows.filter(row=>row.status===clean(filters.status));rows=rows.map(row=>recordClassification.decorate('policy',row));return{ok:true,list:rows,total:rows.length,selectable:rows.filter(row=>row.status==='approved'&&row.policyVersionId!==LEGACY_POLICY_ID)};}
