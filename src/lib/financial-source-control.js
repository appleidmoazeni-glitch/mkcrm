...
async function purchaseDelta(db,filters={},by={}){
  ...
  const [oldRows,newRows]=await Promise.all([
    db.collection(purchase.LAYERS).find(canonicalLayerContract.canonicalLayerQuery({datasetId:context.activePurchaseId})).toArray(),
    db.collection(purchase.LAYERS).find(canonicalLayerContract.canonicalLayerQuery({datasetId:context.purchaseCandidate.datasetId})).toArray()
  ]);
...
async function sourceGaps(db,filters={},by={}){
  ...
  const [unknown,recovery,manuals,purchaseRows,returns]=await Promise.all([
    ...
    db.collection(purchase.LAYERS).find(canonicalLayerContract.canonicalLayerQuery({datasetId:context.purchaseCandidate?.datasetId||context.activePurchaseId})).toArray(),
    db.collection(purchase.LAYERS).find(canonicalLayerContract.canonicalPurchaseReturnQuery({datasetId:context.purchaseCandidate?.datasetId||context.activePurchaseId})).toArray()
  ]);
...
async function fifoFacts(db,datasetId,filters={}){
  ...
  const [manuals,opening,purchaseLayers]=await Promise.all([
    ...
    purchaseDatasetIds.length?db.collection(purchase.LAYERS).find(canonicalLayerContract.canonicalLayerQuery({datasetId:{$in:purchaseDatasetIds}})).toArray():[]
  ]);
