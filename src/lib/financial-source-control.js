...
async function fifoFacts(db,datasetId,filters={}){
  ...
  if(clean(filters.invoice)&&/^\d+$/.test(clean(filters.invoice)))query.saleInvoiceNo=Number(filters.invoice);
  const allocations=await db.collection(fifo.ALLOCATIONS).find(query).toArray();
  ...
  return fifo._provenanceFacts(allocations,manuals).map(fact=>{
    ...
    return {...fact,saleDate:clean(first.saleDate,8),sellerName:clean(first.sellerName,200),itemGuid:clean(first.itemGuid,100),itemDescription:clean(first.itemDescription,500),officialProductCategoryGuid:clean(first.officialProductCategoryGuid||first.productCategoryGuid,100),officialProductCategoryName:clean(first.officialProductCategoryName||fact.productCategory,300),
...
function filterFacts(rows,filters={}){let out=rows;if(clean(filters.status))out=out.filter(row=>row.profitProvenanceStatus===clean(filters.status).toUpperCase());if(clean(filters.invoice)){const invoice=clean(filters.invoice);out=out.filter(row=>String(row.saleInvoiceNumber||row.saleInvoiceNo||'')===invoice);}if(clean(filters.item))out=out.filter(row=>regex(filters.item).test(`${row.itemCode} ${row.itemGuid||''} ${row.itemDescription||''}`));if(clean(filters.sellerIdentity))out=out.filter(row=>clean(row.sellerIdentity,100)===clean(filters.sellerIdentity,100));else if(clean(filters.seller))out=out.filter(row=>regex(filters.seller).test(`${row.sellerIdentity} ${row.sellerName||''}`));if(clean(filters.categoryGuid))out=out.filter(row=>clean(row.officialProductCategoryGuid||row.productCategoryGuid,100)===clean(filters.categoryGuid,100));else if(clean(filters.category))out=out.filter(row=>regex(filters.category).test(row.productCategory));if(clean(filters.costSourceType))out=out.filter(row=>row.costSourceType===clean(filters.costSourceType).toUpperCase());if(clean(filters.dateFrom))out=out.filter(row=>row.saleDate>=fifoFilterDate(filters.dateFrom,'dateFrom'));if(clean(filters.dateTo))out=out.filter(row=>row.saleDate<=fifoFilterDate(filters.dateTo,'dateTo'));if(clean(filters.saleValueMin))out=out.filter(row=>numeric(row.saleValueExact)>=numeric(filters.saleValueMin));if(clean(filters.saleValueMax))out=out.filter(row=>numeric(row.saleValueExact)<=numeric(filters.saleValueMax));if(clean(filters.profitMin))out=out.filter(row=>row.fifoProfitExact!=null&&numeric(row.fifoProfitExact)>=numeric(filters.profitMin));if(clean(filters.profitMax))out=out.filter(row=>row.fifoProfitExact!=null&&numeric(row.fifoProfitExact)<=numeric(filters.profitMax));if(clean(filters.marginMin))out=out.filter(row=>row.marginPercent!=null&&row.marginPercent>=numeric(filters.marginMin));if(clean(filters.marginMax))out=out.filter(row=>row.marginPercent!=null&&row.marginPercent<=numeric(filters.marginMax));return out;}
async function fifoLines(db,filters={},by={}){assertRole(by,READ_ROLES);const context=await contexts(db,filters),selected=fifoSelection(context,filters),datasetId=selected.datasetId;if(!datasetId)return {ok:true,readOnly:true,datasetId:'',datasetRole:'unavailable',...paginate([],filters)};let rows=filterFacts(await fifoFacts(db,datasetId,filters),filters);rows.sort((a,b)=>Number(b.saleInvoiceNumber||b.saleInvoiceNo||0)-Number(a.saleInvoiceNumber||a.saleInvoiceNo||0)||a.saleLineId.localeCompare(b.saleLineId));return {ok:true,readOnly:true,datasetId,datasetRole:selected.datasetRole,dataset:datasetSummary(selected.dataset,context.activeFifoId),...paginate(rows,filters)};}
