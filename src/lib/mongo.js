...
async function initMongo() {
  ...
  await database.collection('inventoryDiscoveryVerificationQueue').createIndex({ status:1, nextEligibleAt:1, firstQueuedAt:1, lastAttemptAt:1 });
  ...
  await database.collection('itemInventoryCatalog').createIndex({ needsLiveVerify:1, nextLiveVerifyEligibleAt:1, lastLiveAttemptAt:1, firstMissingInStockAt:1 });
  ...
  await database.collection('openingAccountingCostBasis').createIndex({ datasetId:1, status:1, saleExposure:-1 });
  await database.collection('openingAccountingCostBasis').createIndex({ canonicalIdentity:1, effectiveOpeningDate:-1, status:1 });
  await database.collection('openingAccountingEvidenceDatasets').createIndex({ datasetId:1 }, { unique:true });
  await database.collection('openingAccountingEvidenceDatasets').createIndex({ status:1, createdAt:-1 });
  ...
  await database.collection('openingAccountingEligibilityPreview').createIndex({ datasetId:1, itemCode:1, saleDate:1 });
  await database.collection('openingAccountingExtractionLeases').createIndex({ scopeKey:1 }, { unique:true });
  await database.collection('openingAccountingExtractionLeases').createIndex({ expiresAt:1 });
  await database.collection('openingAccountingExtractionRuntime').createIndex({ datasetId:1 }, { unique:true });
  await database.collection('openingAccountingExtractionRuntime').createIndex({ state:1, updatedAt:-1 });
  await database.collection('openingAccountingExtractionEvents').createIndex({ eventId:1 }, { unique:true });
  await database.collection('openingAccountingExtractionEvents').createIndex({ datasetId:1, at:-1 });
