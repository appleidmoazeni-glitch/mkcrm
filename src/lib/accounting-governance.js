...
async function groupReviewMatrix(db, filters={}, requestedBy={}) {
  ...
  for(const row of grouped.values()){
    row.officialProductCategoryIdentity=row.mainGroupIdentity;row.officialProductCategoryGuid=row.mainGroupGuid;row.officialProductCategoryNumber=row.mainGroupNumber;row.officialProductCategoryName=row.mainGroupName;
    ...
    const approvedMapping=relevant.find(m=>m.status==='approved');row.currentCommissionRatePool=approvedMapping?ledger._mappingRatePool(approvedMapping):'UNRESOLVED';row.currentCommissionCategory=row.currentCommissionRatePool;
    ...
    row.suggestedCommissionRatePool=/notebook|laptop|نوت|لپ/.test(haystack)?'NOTEBOOK':row.officialEvidence?'COMPONENT':'UNRESOLVED';row.suggestedCommissionCategory=row.suggestedCommissionRatePool;
    row.suggestionEvidence=row.suggestedCommissionRatePool==='UNRESOLVED'?'Official hierarchy unresolved; prefix fallback is diagnostic-only and human evidence is required.':'Rate Pool suggestion derived from the official Main Group name; Product Category remains the exact Shaygan Main Group and is never collapsed.';
...
async function rateReviewMatrix(db,filters={},requestedBy={}){
  ...
  for(const fact of facts){
    ...
    const classification=projected.officialProductCategoryIdentity?projected:ledger._resolveCategoryFromMappings([],enriched,fact.saleDate);const key=(fact.sellerIdentity||'UNRESOLVED')+'|'+classification.officialProductCategoryIdentity+'|'+classification.commissionRatePool;
    const row=matrix.get(key)||{sellerIdentity:fact.sellerIdentity||'UNRESOLVED',sellerName:fact.sellerName||'',officialProductCategoryIdentity:classification.officialProductCategoryIdentity,officialProductCategoryGuid:classification.officialProductCategoryGuid,officialProductCategoryNumber:classification.officialProductCategoryNumber,officialProductCategoryName:classification.officialProductCategoryName,commissionRatePool:classification.commissionRatePool,commissionCategory:classification.officialProductCategoryName,saleLineCount:0,saleValue:'0.00',mappingStatus:approved.status==='resolved'?'approved':projected.status==='resolved'?'projected':'missing'};
    row.saleLineCount++;row.saleValue=add([row.saleValue,fact.saleAmountExact||0]);matrix.set(key,row);
  }
  for(const row of matrix.values()){
    let preferred=null,resolution=null,rateStatus='missing';for(const status of ['approved','pending','draft']){const candidates=rates.filter(rate=>rate.status===status&&rate.effectiveFrom<=periodTo&&(!rate.effectiveTo||rate.effectiveTo>=periodFrom));const result=ledger._resolveRateFromRows(candidates,row.sellerIdentity,row,periodFrom);if(result.status!=='missing'){resolution=result;preferred=result.rateVersion;rateStatus=status;break;}}
    row.proposedRate=preferred?.rate??null;row.rateVersionId=preferred?.rateVersionId||'';row.rateStatus=resolution?.status==='ambiguous'?'ambiguous':rateStatus;row.rateClassification=resolution?.precedence||'missing';row.appliedRateScope=resolution?.appliedRateScope||'';row.evidence=preferred?.sourceReference||'';row.ambiguityNotes=resolution?.status==='ambiguous'?'multiple-candidates-at-same-precedence':'';
  }
  const rows=[...matrix.values()].sort((a,b)=>Number(b.saleValue)-Number(a.saleValue)||a.sellerIdentity.localeCompare(b.sellerIdentity));
  return{ok:true,fifoDatasetId,periodFrom,periodTo,...pageRows(rows,filters),rateResolutionPrecedence:['seller/category','seller/rate-pool','category-default','rate-pool-default'],blankRateIsZero:false,automaticApproval:false};
}
...
async function readiness(db,input={},requestedBy={}) {
  ...
  const exceptionalRateCandidates=rates.filter(row=>row.sellerIdentity!=='*'&&['draft','pending'].includes(row.status)&&row.effectiveFrom<=periodTo&&(!row.effectiveTo||row.effectiveTo>=periodFrom)).map(row=>({rateVersionId:row.rateVersionId,sellerIdentity:row.sellerIdentity,rateScope:row.rateScope||'legacy',officialProductCategoryIdentity:row.officialProductCategoryIdentity||'',officialProductCategoryName:row.officialProductCategoryName||'',commissionRatePool:row.commissionRatePool||ledger._mappingRatePool(row),rate:row.rate,status:row.status}));return{ok:true,fifoDatasetId,periodFrom,periodTo,policy,metrics,openingReadiness,missingRateSellers:[...missingRates],exceptionalRateCandidates,rateResolutionPrecedence:['seller/category','seller/rate-pool','category-default','rate-pool-default'],blockers,normalExportReady:blockers.length===0,diagnosticExportPayable:false,profitRoiCommissionEnabled:false};
}
